...
def set_data(df_polars, lib_name, ctrl_name, cur_data, prev_data):
    ...
    try:
        # FIX: sas.df2sd() returns None or a result dict depending on version
        # Let's use try-except and also check what it returns
        result = sas.df2sd(df_pandas, table=cur_data, libref='work')
        
        # Check what was returned
        print(f"  sas.df2sd() returned: {type(result)}")
        
        # Different saspy versions return different things
        # Some return a dict with LOG, some return None
        if result is None:
            print(f"  sas.df2sd() returned None (normal for some saspy versions)")
            # Check if dataset was created by trying to access it
            try:
                test_ds = sas.sasdata(cur_data, libref='work')
                row_count = test_ds.shape[0] if hasattr(test_ds, 'shape') else "unknown"
                print(f"  Verified work.{cur_data} created with {row_count} rows")
            except:
                print(f"  Warning: Could not verify work.{cur_data} creation")
        elif isinstance(result, dict) and "LOG" in result:
            print(f"  sas.df2sd() returned dict with LOG key")
            if "ERROR" in result["LOG"]:
                print(f"  ERROR creating work.{cur_data}:")
                print(result["LOG"][:500])
                return result
        else:
            print(f"  sas.df2sd() returned unexpected type: {type(result)}")
            
    except Exception as e:
        print(f"  Exception in sas.df2sd(): {e}")
        # Continue anyway, the dataset might still have been created
    
    # Get metadata from control table
    log_result = sas.submit(f"""
        proc sql noprint;
    ...
    if isinstance(log_result, dict) and "LOG" in log_result:
        log_text = log_result["LOG"]
        if "ERROR" in log_text or "Table does not exist" in log_text:
        ...
        print(f"  SAS submit returned None, assuming successful")
    else:
        print(f"  SAS submit returned unexpected type: {type(log_result)}")
    ...
    try:
        ...
        print(f"  Error getting metadata: {meta_error}")
    ...
    if log_result and isinstance(log_result, dict) and "LOG" in log_result:
        log_text = log_result["LOG"]
        print(f"  SAS Log (first 500 chars): {log_text[:500]}...")
        if "ERROR" in log_text:
            print(f"  WARNING: Errors in SAS log!")
    elif log_result is None:
        print(f"  SAS operation completed (no log returned)")
    else:
        print(f"  SAS operation completed, log type: {type(log_result)}")
...
# Create a simplified SAS transfer function that doesn't rely on control tables
def simple_sas_transfer(df_polars, lib_name, dataset_name):
    """Simple SAS transfer without control table dependencies"""
    if len(df_polars) == 0:
    ...
    try:
        # Convert to pandas
        df_pandas = df_polars.to_pandas()
        print(f"  Transferring {dataset_name} with {len(df_pandas)} records...")
        
        # Use sas.dataframe2sasdata directly
        result = sas.dataframe2sasdata(df_pandas, table=dataset_name, libref=lib_name)
        
        # Check result
        if result is None:
            print(f"  ✓ {lib_name}.{dataset_name} created (no log returned)")
        elif isinstance(result, dict) and "LOG" in result:
            log_text = result["LOG"]
            if "ERROR" in log_text:
                print(f"  ✗ Error creating {lib_name}.{dataset_name}:")
                print(f"    {log_text[:500]}")
            else:
                print(f"  ✓ {lib_name}.{dataset_name} created successfully")
        else:
            print(f"  ? {lib_name}.{dataset_name} created (unexpected result type: {type(result)})")
        
        return result
    except Exception as e:
        print(f"  ✗ Exception transferring {dataset_name}: {e}")
        import traceback
        traceback.print_exc()
        return None
...
try:
    ...
    # Transfer data to SAS using simplified method
    print("\n2. Transferring data to SAS (simplified method)...")
    
    # Transfer each dataset
    if len(channel_df) > 0:
        result1 = simple_sas_transfer(channel_df, "crm", sum_data)
    else:
        print(f"  Skipping CHANNEL SUMMARY - no data")
    
    if len(update_df) > 0:
        result2 = simple_sas_transfer(update_df, "crm", update_data)
    else:
        print(f"  Skipping CHANNEL UPDATE - no data")
    
    if len(otc_detail) > 0:
        result3 = simple_sas_transfer(otc_detail, "crm", otc_data)
    else:
