...
try:
    con.execute("""
        ...
        SELECT
            ...
            -- Parse dates (take first 8 digits from packed decimal)
            CASE 
                WHEN BILL_DUE_DATE > 0 THEN 
                    TRY_CAST(
                        SUBSTR(CAST(BILL_DUE_DATE AS VARCHAR), 1, 8)
                        AS DATE
                    )
                ELSE NULL
            END AS BILL_DT,
            ...
            CASE 
                WHEN BILL_PAID_DATE > 0 THEN
                    TRY_CAST(
                        SUBSTR(CAST(BILL_PAID_DATE AS VARCHAR), 1, 8)
                        AS DATE
                    )
                ELSE NULL
            END AS BILL_PAID_DT
...
# assign_libname("ln" , "/sas/ln/dwh/enrichment")
