...
def accumulate_monthly_data(df_new, dataset_name, schema, month_field="MONTH", date_field="DATE"):
    ...
    if os.path.exists(curr_path):
        ...
        current_value = REPTDATE.strftime("%b%y").upper() if month_field == "MONTH" else REPTDATE.strftime("%d/%m/%Y")
        if current_value in df_current[month_field if month_field == "MONTH" else date_field].unique().to_list():
            df_current = df_current.filter(pl.col(month_field if month_field == "MONTH" else date_field) != current_value)
...
def process_channel_summary():
    ...
    return accumulate_monthly_data(df_new, "CHANNEL_SUM", schema)
...
def transfer_to_sas(df, lib_name, dataset_name):
    ...
    try:
        ...
        try:
            ...
            print(f"  ✓ {dataset_name} created with {sas_ds.shape[0] if hasattr(sas_ds, 'shape') else 'unknown'} rows")
            return True
        except:
