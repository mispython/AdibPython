...
def set_data(df, lib_name, ctrl_name, cur_data, prev_data):
    ...
    # Get column metadata from control table
    log = sas.submit(f"""
            proc sql noprint;
    ...
    if df_meta.empty or len(df_meta) == 0:
        print(f"WARNING: Control table {ctrl_name}.{prev_data} not found or has no columns.")
        print(f"Creating table {lib_name}.{cur_data} directly from work.{cur_data}")
        
        # Create table directly without template
        log = sas.submit(f"""
                proc sql noprint;
                     create table {lib_name}.{cur_data} as
                     select * from work.{cur_data};
                quit;
                """)
        print(log["LOG"])
        return log
    
    # Normal flow: control table exists
    cols = df_meta["name"].dropna().tolist()
    
    if not cols:
        print(f"WARNING: No columns found in control table {ctrl_name}.{prev_data}")
        print(f"Creating table {lib_name}.{cur_data} directly from work.{cur_data}")
