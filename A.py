...
def process_large_loan_bill_scd(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if input_ln_bill.exists() and input_iln_bill.exists():
            ...
            # Load only active records from LOAN_BILL - FIXED: Compare SAS numeric dates
            con.execute(f"""
                CREATE TABLE ln_bill_hist_active AS 
                SELECT 
                    ACCTNO, NOTENO, 
                    -- Convert SAS dates from numeric to DATE
                    CASE 
                    ...
                    END AS BILL_DT,
                    CASE 
                        WHEN BILL_PAID_DT IS NOT NULL AND BILL_PAID_DT > 0 THEN 
            ...
            # Load only active records from ILOAN_BILL - FIXED: Compare SAS numeric dates
            con.execute(f"""
...
def process_scd_chunk_optimized(con, chunk_file, REPTDATE, PREVDATE, temp_path, chunk_id, has_historical_data, active_hist_count):
    ...
    # Load chunk as new records - FIXED DATE CONVERSION
    con.execute(f"""
    ...
    if has_historical_data and active_hist_count > 0:
        ...
        con.execute(f"""
            ...
            new_or_changed AS (
                ...
                FROM new_records T2
                WHERE NOT EXISTS (
                    SELECT 1 FROM loan_bill_hist_active T1
                    WHERE T1.ACCTNO = T2.ACCTNO 
                    AND T1.NOTENO = T2.NOTENO
                    AND T1.BILL_DT = T2.BILL_DT
                    AND (T1.BILL_PAID_DT = T2.BILL_PAID_DT OR (T1.BILL_PAID_DT IS NULL AND T2.BILL_PAID_DT IS NULL))
