...
# Get final result as Pandas DataFrame for SAS
print("\nStep 3: Loading to ENRH_LN_BILL...")

result_pandas = con.execute("""
    SELECT 
        ACCTNO,
        NOTENO,
        BLDATE,
...
# Also get as Polars for writing to parquet
result_polars = con.execute("""
    SELECT 
        ACCTNO,
        NOTENO,
        BLDATE,
...
result_polars.write_parquet(output_path)
...
def set_data(df, lib_name, ctrl_name, cur_data, prev_data):
    ...
    print(f"Loading {len(df):,} records to SAS...")
    ...
    print("Column metadata retrieved")
    ...
    log = sas.submit(f"""
                proc sql noprint;
                    ...
                    quit;
                    """)
    print(f"Final table created in SAS")
    return log

try:
    # Assign libname and load data to SAS
    assign_libname("ln", "/stgsrcsys/host/uat")
    log1 = set_data(result_pandas, "ln", "ctrl", enrh_ln_bill, enrh_ln_bill_ctl)
    print("✓ Data successfully loaded to SAS")
    
except Exception as e:
    print(f"✗ ERROR loading to SAS: {e}")
    # Try alternative approach
    try:
        print("Trying alternative SAS loading method...")
        # Convert to SAS using direct method
        sas_df = sas.df2sd(result_pandas, table=enrh_ln_bill, libref='work')
        print("✓ Data loaded to SAS using alternative method")
