...
if sdbms_file.exists():
    try:
        sdbms_query = f"""
        SELECT SUBSTR(content, 1, 8) as SDBMS_DATE_STR
        ...
        if len(sdbms_df) > 0:
            ...
            if len(sdbms_date_full) == 8 and sdbms_date_full.isdigit():
                ...
                # Example: 09122025 -> 091225 (take DD, MM, last 2 of YYYY)
                dd = sdbms_date_full[0:2]   # 09
                mm = sdbms_date_full[2:4]   # 12
                yy = sdbms_date_full[6:8]   # 25 (last 2 digits of year)
                sdbms_date = dd + mm + yy   # 091225
