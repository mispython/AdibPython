...
if isinstance(tbd, (pd.Timestamp, dt.datetime, dt.date)):
    ...
    # Check if it's SAS date format (days since 1960-01-01)
    if tbd < 100000:  # Likely SAS date (days since epoch)
        sas_epoch = dt.datetime(1960, 1, 1)
        reptdate = sas_epoch + dt.timedelta(days=int(tbd))
    else:  # YYYYMMDD numeric format
        reptdate = dt.datetime.strptime(str(int(tbd)), "%Y%m%d")
else:
    # String format
    tbd_str = str(tbd).replace('-', '').replace('/', '').replace(' ', '')[:8]
    reptdate = dt.datetime.strptime(tbd_str, "%Y%m%d")
