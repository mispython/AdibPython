...
def set_data(df, lib_name, ctrl_name, cur_data, prev_data):
    ...
    # Instead of using input() which truncates, use substr() to trim to length
    for _, row in df_meta.iterrows():
        col = row["name"]
        length = row['length']
        if row['type'].strip().lower() == 'char' and pd.notnull(length) and length > 0:
            # Use substr() instead of input() to preserve data and avoid conversion errors
            casted_cols.append(f"substr({col}, 1, {int(length)}) as {col}")
        else:
