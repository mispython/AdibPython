...
def main():
    ...
    if 'nidstat' in df.columns:
        nidstat_counts = df.group_by('nidstat').agg(pl.len().alias('count'))
        print(f"  NIDSTAT distribution:")
        for row in nidstat_counts.rows():
            print(f"    {row[0]}: {row[1]:,}")
    
    if 'cdstat' in df.columns:
        cdstat_counts = df.group_by('cdstat').agg(pl.len().alias('count'))
        print(f"  CDSTAT distribution:")
        for row in cdstat_counts.rows():
            print(f"    {row[0]}: {row[1]:,}")
    ...
    print(f"  Records with positive balance: {len(df):,} (from {df_original_count:,})")
    ...
    if missing_cols:
        ...
        # Filter for Table 1 (Outstanding NID) - Use relaxed conditions
        tbl1_filtered = df.filter(
            (pl.col('matdt').is_not_null()) &
            (pl.col('startdt').is_not_null()) &
        ...
        if len(tbl1_filtered) > 0:
            ...
            print(f"  Sample matdt: {tbl1_filtered.select('matdt').head(3).to_series().to_list()}")
            print(f"  Sample startdt: {tbl1_filtered.select('startdt').head(3).to_series().to_list()}")
    ...
    if 'nidstat' in df.columns and 'early_wddt' in df.columns:
