...
def read_or_create_empty(path, schema_dict):
    ...
    if parquet_exists(path):
        ...
        # Ensure proper dtypes - cast object columns to string
...
def safe_print_unique(df, column_name):
    ...
    try:
        if column_name in df.columns and len(df) > 0:
            # Convert to list directly without using unique() if dtype is object
...
print(f"  Schema: {last_month_channel.schema}")
...
if parquet_exists(TODAY_CHANNEL_SUM):
    ...
    for col in today_channel.columns:
        if today_channel[col].dtype == pl.Object:
            today_channel = today_channel.with_columns(pl.col(col).cast(pl.Utf8))
    
    print(f"  Records: {len(today_channel)}")
    print(f"  Schema: {today_channel.schema}")
...
combined_channel_sum = combined_channel_sum.select([
...
print(f"  Schema: {last_month_update.schema}")
if len(last_month_update) > 0:
    try:
        if 'REPORT_DATE' in last_month_update.columns:
...
if parquet_exists(TODAY_CHANNEL_UPDATE):
    ...
    # Ensure proper dtypes
    for col in today_update.columns:
        if today_update[col].dtype == pl.Object:
            today_update = today_update.with_columns(pl.col(col).cast(pl.Utf8))
    
    print(f"  Records: {len(today_update)}")
    print(f"  Schema: {today_update.schema}")
    if len(today_update) > 0:
        try:
            if 'REPORT_DATE' in today_update.columns:
...
# Ensure all columns have proper types
combined_channel_update = combined_channel_update.select([
