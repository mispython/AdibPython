...
def calculate_remmth(row, reptdate_val):
    """Calculate remaining months - returns float for consistent typing"""
    openind = row.get("OPENIND", "")
    
    # Handle closed accounts - return -1.0 as float
    if openind == "D":
        return -1.0
...
# Check unique CUSTCODE values (first 20)
if "CUSTCODE" in fdmthly.columns:
    ...
    # Convert to string for proper filtering
    fdmthly = fdmthly.with_columns([
        pl.col("CUSTCODE").cast(pl.Utf8).alias("CUSTCODE_STR")
    ])
    
    # Check for numeric patterns - maybe CUSTCODE is stored as integer like 81, 82, etc.
    target_codes_1 = ["81", "82", "83", "84"]
    target_codes_2 = ["85", "86", "87", "88", "89", "90", "91", "92", "95", "96", "98", "99"]
    
    # Check as strings
    count_group1 = fdmthly.filter(pl.col("CUSTCODE_STR").is_in(target_codes_1)).height
    count_group2 = fdmthly.filter(pl.col("CUSTCODE_STR").is_in(target_codes_2)).height
    
    print(f"\nRecords with CUSTCODE in group 1 (81-84): {count_group1:,}")
    print(f"Records with CUSTCODE in group 2 (85-99): {count_group2:,}")
    
    # Also check as integers if they are stored as numbers
    if fdmthly["CUSTCODE"].dtype in [pl.Int32, pl.Int64, pl.Float32, pl.Float64]:
        print("\nCUSTCODE appears to be numeric. Checking numeric ranges...")
        # Filter for numeric values in range
        count_group1_num = fdmthly.filter(
            (pl.col("CUSTCODE") >= 81) & (pl.col("CUSTCODE") <= 84)
        ).height
        count_group2_num = fdmthly.filter(
            (pl.col("CUSTCODE") >= 85) & (pl.col("CUSTCODE") <= 99)
        ).height
        print(f"Records with CUSTCODE between 81-84 (numeric): {count_group1_num:,}")
        print(f"Records with CUSTCODE between 85-99 (numeric): {count_group2_num:,}")
        
        # If numeric values exist, use numeric comparison
        if count_group1_num > 0 or count_group2_num > 0:
            print("\nUsing numeric CUSTCODE values instead of string comparisons")
            # We'll handle this in the main logic
            use_numeric_custcode = True
        else:
            use_numeric_custcode = False
    else:
        use_numeric_custcode = False
...
if "OPENIND" in fdmthly.columns:
    ...
    print(f"\nRecords after filtering OPENIND in ['O','D']: {fdmthly.height:,}")
else:
...
# Determine if we need to use numeric or string CUSTCODE
if 'use_numeric_custcode' in locals() and use_numeric_custcode:
    print("Using numeric CUSTCODE values")
    # Convert CUSTCODE to string for display but use numeric for comparison
    alm_summary = alm_summary.with_columns([
        pl.col("CUSTCODE").cast(pl.Utf8).alias("CUSTCODE_STR")
    ])
    
    # For numeric CUSTCODE, we need to create conditions differently
    almdept = alm_summary.with_columns([
        ...
        pl.when(
            ...
            pl.concat_str([pl.col("BIC"), pl.col("CUSTCODE_STR"), pl.col("RM"), pl.lit("0000Y")], separator="")
        ).when(
            (pl.col("CUSTCODE") >= 85) & (pl.col("CUSTCODE") <= 99)
        ).then(
            pl.concat_str([pl.col("BIC"), pl.lit("85"), pl.col("RM"), pl.lit("0000Y")], separator="")
        )
        .otherwise(None)
        .alias("BNMCODE")
    ]).filter(pl.col("BNMCODE").is_not_null()).select(["BNMCODE", "AMOUNT", "CUSTCODE", "REMMTH"])
else:
    print("Using string CUSTCODE values")
...
if len(almdept) > 0:
    ...
    if "CUSTCODE" in alm.columns:
        sample_custcodes = alm.select(pl.col("CUSTCODE")).unique().head(10)
        print(sample_custcodes)
