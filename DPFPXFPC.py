...
TEST_ROWS = 10000  # Number of rows to read for testing
...
def read_sas_to_polars(filepath: Path, test_mode: bool = False, test_rows: int = 100000) -> pl.DataFrame:
    ...
    Args:
        ...
        test_mode: If True, limit rows after reading
        test_rows: Number of rows to keep in test mode
    ...
    try:
        ...
        # Read the full file (rows_limit not supported in this version)
        ...
        if test_mode and len(pl_df) > test_rows:
            logger.info(f"TEST MODE: Limiting to first {test_rows:,} rows (from {len(pl_df):,})")
            pl_df = pl_df.head(test_rows)
...
def main():
    ...
    try:
        ...
        if TEST_MODE:
            logger.info(f"⚠️  TEST MODE ENABLED: Will limit to {TEST_ROWS:,} rows after reading")
