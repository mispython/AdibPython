...
# CRA file format - ACTUAL production format
# Based on SAS code: INPUT @001 BRANCH $3. @007 CUSTICKETNO $60. @067 INVCURAC PD6. etc.
CRA_FORMAT = {
...
def load_fixed_width_file(file_path, widths, columns, dtypes=None, encoding='utf-8'):
    """Load a fixed-width file with proper error handling"""
    try:
        ...
        print(f"  Warning: UTF-8 decoding failed, trying Latin-1 encoding...")
        with open(file_path, 'r', encoding='latin-1') as f:
            lines = f.readlines()
    except Exception as e:
        print(f"  Warning: Reading as binary... {e}")
    ...
    print(f"  Parsed {len(data)} rows from {line_count} non-empty lines")
...
def convert_sas_date_to_display(sas_date):
    """Convert SAS date (days since 1960-01-01) to string format DD/MM/YY"""
...
def format_date_for_display(val):
    """Format date value for DCITXT display"""
...
def main():
    ...
    try:
        ...
        if not os.path.exists(cra_path):
            ...
            if os.path.exists(cra_path_txt):
                ...
                print(f"  ✗ CRA file not found at {cra_path} or {cra_path_txt}")
                return
        ...
        # Debug: Print first few rows of CRA
        if len(cra) > 0:
    ...
    eq = eqfl.with_columns([
        ...
        pl.col("PREMPAID").abs(),
        pl.col("PREMREC").abs(),
        pl.col("STARTDT").cast(pl.Utf8),
        pl.col("MATDT").cast(pl.Utf8),
    ])
    ...
    # -------------------------------------------------------------------
    # Customer Leg - CRITICAL: CRA data should be the main source
    # -------------------------------------------------------------------
    print("\nProcessing Customer Leg...")
    
    # First, process CRA data (this should be the main source of customer data)
    print(f"  Processing CRA data...")
    ...
    if len(dp_cra) == 0:
        ...
        if len(cra) > 0:
            print(f"  Available INV_STATUS values in CRA: {cra['INV_STATUS'].unique().to_list()}")
        dp_cra = pl.DataFrame()
    else:
    ...
    if len(dp_cra) > 0 and len(depo) > 0:
        ...
        print("  Warning: No CRA or DEPO data to join")

    # Now process EQDCI (DCI customer data)
    print(f"  Processing DCI customer data...")
    ...
    # Create MYR row with all necessary columns
    eqrt_cols = eqrt.columns
    myr_row_dict = {col: None for col in eqrt_cols}
    myr_row_dict['INVCURR'] = 'MYR'
    myr_row_dict['SPOTRT'] = 1.0000000
    myr_row = pl.DataFrame([myr_row_dict])
    eqrt_with_myr = pl.concat([eqrt, myr_row])
    eqrt_with_myr = eqrt_with_myr.unique(subset=['INVCURR'], keep='first')
    
    if len(eqdci) > 0:
        eqdci = eqdci.join(eqrt_with_myr.select(['INVCURR', 'SPOTRT']), on="INVCURR", how="left")
    ...
    if len(eqi) > 0:
        ...
        eqdci_ib = eqdci_ib.join(eqrt_with_myr.select(['INVCURR', 'SPOTRT']), on="INVCURR", how="left")
    ...
    with open(text_path, "w") as f:
        ...
        for row in cusmyr.iter_rows(named=True):
            ...
            accint = row.get('ACCINT', 0) / 1000000 if row.get('ACCINT', 0) > 1000000 else row.get('ACCINT', 0)
            accintrm = row.get('ACCINTRM', 0) / 1000000 if row.get('ACCINTRM', 0) > 1000000 else row.get('ACCINTRM', 0)
            prempaid = row.get('PREMPAID', 0) / 1000000 if row.get('PREMPAID', 0) > 1000000 else row.get('PREMPAID', 0)
            prempaidrm = row.get('PREMPAIDRM', 0) / 1000000 if row.get('PREMPAIDRM', 0) > 1000000 else row.get('PREMPAIDRM', 0)
        ...
        if len(cusmyr) > 0:
            total_accint = (cusmyr['ACCINT'].sum() / 1000000) if cusmyr['ACCINT'].sum() > 1000000 else cusmyr['ACCINT'].sum()
            total_accintrm = (cusmyr['ACCINTRM'].sum() / 1000000) if cusmyr['ACCINTRM'].sum() > 1000000 else cusmyr['ACCINTRM'].sum()
            total_prempaid = (cusmyr['PREMPAID'].sum() / 1000000) if cusmyr['PREMPAID'].sum() > 1000000 else cusmyr['PREMPAID'].sum()
            total_prempaidrm = (cusmyr['PREMPAIDRM'].sum() / 1000000) if cusmyr['PREMPAIDRM'].sum() > 1000000 else cusmyr['PREMPAIDRM'].sum()
