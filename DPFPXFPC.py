...
ISLAMIC_PROD_FORMATS = {
    'IR070': 'IR070',  # Islamic product code
...
def apply_format_mappings(df: pl.DataFrame) -> pl.DataFrame:
    ...
    Handles both numeric and alphanumeric product codes
    """
    if 'PRODCD' in df.columns:
        logger.info("Applying product format mappings from PBBDPFMT")
        
        # Apply format using map_elements - handle both numeric and string codes
        df = df.with_columns([
...
def _get_product_format(prodcd) -> str:
    ...
    # Check for Islamic product codes (alphanumeric)
