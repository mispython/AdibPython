...
def process_trust_data(pidmfin_path: Path) -> pl.DataFrame:
    ...
    if missing_cols:
        ...
        logger.info(f"Available columns: {cisdepxn_df.columns}")
        return pl.DataFrame()
...
def process_deposit_data(deposit1_path: Path) -> pl.DataFrame:
    ...
    if missing_cols:
        ...
        logger.info(f"Available columns: {cisdepd_df.columns}")
        return pl.DataFrame()
...
def apply_format_mappings(df: pl.DataFrame) -> pl.DataFrame:
    """
    Apply PRODBRH format mapping to PRODCD column using replace() method
    """
    if 'PRODCD' in df.columns:
        # Method 1: Using replace() - works in newer Polars versions
        try:
            df = df.with_columns([
            ...
            try:
                df = df.with_columns([
                    pl.col('PRODCD').map_elements(lambda x: PRODBRH.get(x, x), return_dtype=pl.String).alias('PRODCD_FORMATTED')
                ])
            except Exception as e:
                logger.error(f"Error applying format mappings: {e}")
                # Method 3: Using pl.Series.map_dict() - convert to series first
                df = df.with_columns([
                    pl.Series(df['PRODCD'].map_elements(lambda x: PRODBRH.get(x, x))).alias('PRODCD_FORMATTED')
                ])
    return df
