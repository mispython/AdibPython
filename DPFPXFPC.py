...
def main():
    ...
    print(f"   Branch data type: {branch_df.dtypes}")
    ...
    # 4. Fix data types for joining
    print("\n4. Fixing data types for merge...")
    
    # Convert BRANCH in categorized to Int64
    if categorized["BRANCH"].dtype != pl.Int64:
        try:
            categorized = categorized.with_columns(
                pl.col("BRANCH").cast(pl.Int64)
            )
            print(f"   Converted categorized BRANCH to Int64")
        except Exception as e:
            print(f"   Error converting categorized BRANCH: {e}")
            # Try to extract integer from float
            if categorized["BRANCH"].dtype == pl.Float64:
                categorized = categorized.with_columns(
                    pl.col("BRANCH").cast(pl.Int64)
                )
    
    # Convert BRANCH in branch_df to Int64 if needed
    if branch_df["BRANCH"].dtype != pl.Int64:
        try:
            branch_df = branch_df.with_columns(
                pl.col("BRANCH").cast(pl.Int64)
            )
            print(f"   Converted branch BRANCH to Int64")
        except Exception as e:
            print(f"   Error converting branch BRANCH: {e}")
            # Try to extract integer from string
            if branch_df["BRANCH"].dtype == pl.Utf8:
                branch_df = branch_df.with_columns(
    ...
    # 6. Always generate 17-bucket report
    ...
    # 7. Generate 15-bucket report only on day 15
    if variables['REPTDAY'] == '15':
        print("\n7. Day 15 detected - Generating 15-bucket report (EIMAR102-B)...")
        ...
        print("\n7. Not day 15 - Skipping 15-bucket report")
    
    # 8. Create summary statistics as text
