...
# Sector Reverse Format ($RVRSE) - Now with both string and numeric keys
...
# Customer Code Format ($LOCUSTCD) - With both string and numeric keys
CUSTCODE_FORMAT = {
    '1': '01', '2': '02', '3': '03', '4': '04', '5': '05',
    '6': '06', '7': '07', '8': '08', '9': '09', '10': '10',
    '11': '11', '12': '12', '13': '13', '14': '14', '15': '15',
...
def safe_to_string(series):
    """Safely convert a series to string, handling numeric types"""
    if series.dtype in [pl.Float64, pl.Float32]:
        # For float columns, convert to int first if no decimal places
        return series.cast(pl.Int64, strict=False).cast(pl.Utf8)
    else:
...
if data.get('imast') is not None:
    ...
    print(f"IMAST dtypes:")
    for col in imast_cols:
        if col in ['acctno', 'acctnox', 'custcode', 'custcodx', 'sector']:
            print(f"  {col}: {data['imast'][col].dtype}")
    ...
    mast = mast.with_columns([
        ...
        # Apply CUSTCODE format - handle numeric/string
        ...
        # Apply SECTOR format - convert to string safely
        pl.when(pl.col(sector_col).is_null())
          .then(pl.lit("9999"))
          .otherwise(
              pl.col(sector_col).map_elements(
...
if mast is not None and suba9 is not None:
    ...
    # Add calculated fields (only for columns that exist)
