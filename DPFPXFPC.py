...
# Convert the specific problem columns from string to SAS numeric dates
...
for col in date_columns:
    if col in summ1_df.columns and summ1_df[col].dtype == 'object':
        ...
        print(f"SUMM1 - AFTER CONVERSION {col}: dtype={summ1_df[col].dtype}, "
              f"sample head={summ1_df[col].head(3).tolist()}, "
              f"sample tail={summ1_df[col].tail(3).tolist()}")
    ...
    if col in summ2_df.columns and summ2_df[col].dtype == 'object':
        ...
        print(f"SUMM2 - AFTER CONVERSION {col}: dtype={summ2_df[col].dtype}, "
              f"sample head={summ2_df[col].head(3).tolist()}, "
              f"sample tail={summ2_df[col].tail(3).tolist()}")
...
def convert_dtcomplete_column(df, col_name):
    if col_name in df.columns and df[col_name].dtype == 'object':
        print(f"Before conversion {col_name}: dtype={df[col_name].dtype}, "
              f"sample={df[col_name].head(3).tolist()}")
        ...
        # Convert format 1
        if mask_format1.any():
            result[mask_format1] = pd.to_datetime(
        ...
        if mask_format2.any():
            ...
            result[mask_format2] = pd.to_datetime(
                standardized,
                format='%d%b%Y %H:%M:%S',
                errors='coerce'
            )
        ...
        if remaining_mask.any():
            result[remaining_mask] = pd.to_datetime(
                df.loc[remaining_mask, col_name], errors='coerce'
            )

        # Log values that failed to parse and were not empty
        bad = (
            result.isna()
            & df[col_name].notna()
        ...
        if bad.any():
            print(f"[WARN] {col_name}: {bad.sum()} values failed to parse. "
                  f"Examples: {df.loc[bad, col_name].head(5).tolist()}")

        # Convert to SAS datetime (seconds since 1960-01-01)
        sas_datetime = (result - pd.Timestamp('1960-01-01')).dt.total_seconds()

        print(f"After conversion {col_name}: dtype={sas_datetime.dtype}, "
              f"sample={sas_datetime.head(3).tolist()}")
...
def set_data(df, lib_name, ctrl_name, cur_data, prev_data):
    ...
    # Use single quotes inside the f-string to avoid nested double-quote issues
    meta_sql = f"""
        proc sql noprint;
...
print("SQL being sent for ALTER bnmsumm1_ctrl:", repr(alter_sql_1))
log = sas.submit(alter_sql_1)
print("ALTER bnmsumm1_ctrl:", log["LOG"])

# ---------------------------------------------------------------
# Add the new field to the SUMM1 pandas DataFrame ONLY
# (defaults to a proper Series so .astype works even when the
#  column does not exist in the parquet source)
# ---------------------------------------------------------------
if 'EVENT_TYPE' not in summ1_df.columns:
    summ1_df['EVENT_TYPE'] = ''
summ1_df['EVENT_TYPE'] = summ1_df['EVENT_TYPE'].astype(str).str.slice(0, 10)
