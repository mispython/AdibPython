...
def load_remit(d):
    """Load REMIT and UNCLAIM data following SAS logic"""
    try:
        # Load REMIT
...
def load_saca():
    """Load SA/CA/FD data following SAS logic"""
    dfs = []
...
def load_client():
    """Load CLIENT data following SAS logic"""
    try:
...
def main():
    ...
    # Debug: Print sample account numbers
    if not client_df.is_empty() and not saca_df.is_empty():
        print(f"\nDebug - Client sample ACCTNOs: {client_df['acctno'].head(5).to_list()}")
        print(f"Debug - SACA sample ACCTNOs: {saca_df['acctno'].head(5).to_list()}")
        print(f"Debug - Client ACCTNO type: {client_df['acctno'].dtype}")
        print(f"Debug - SACA ACCTNO type: {saca_df['acctno'].dtype}")
    ...
    if (not client_df.is_empty() and not saca_df.is_empty() and 
        ...
        # Debug: Check overlap between client and saca
        client_accts = set(client_df['acctno'].to_list())
        saca_accts = set(saca_df['acctno'].to_list())
        overlap = client_accts.intersection(saca_accts)
        print(f"Debug - Client accounts: {len(client_accts)}")
        print(f"Debug - SACA accounts: {len(saca_accts)}")
        print(f"Debug - Overlap: {len(overlap)}")
        
        # DATA DEPOSIT.CLIENT; MERGE DEPOSIT.CLIENT(IN=A) DEPOSIT(IN=B); BY ACCTNO; IF A & B; RUN;
        # In SAS, DEPOSIT here refers to the SA/CA/FD combined data
