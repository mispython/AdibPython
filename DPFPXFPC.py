...
def eibrp159():
    ...
    if len(npgs_df) > 0:
        ...
        try:
            ...
            # Create SAS dataset using a data step with datalines
            ...
            for _, row in pd_df.iterrows():
                ...
                for col in pd_df.columns:
                    ...
                    if pd.isna(val):
                        ...
                        if pd_df[col].dtype in ['int64', 'float64']:
                            data_line.append(str(val))
                        else:
                            data_line.append(str(val).strip())
                sas_code += "    " + " ".join(data_line) + "\n"
            
            sas_code += "    ;\n"
            sas_code += "run;\n"
            
            # Submit the data step to create the dataset
            print("Creating SAS dataset...")
            create_log = sas.submit(sas_code, results='TEXT')
            
            # Check if dataset was created successfully
            if 'ERROR' in str(create_log):
                print("Error creating SAS dataset:")
                print(create_log)
            else:
                print("SAS dataset created successfully")
                ...
                save_log = sas.submit(save_code, results='TEXT')
                
                # Check for errors in log
                if 'ERROR' in str(save_log):
