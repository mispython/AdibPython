# eibdcitx.py - FIXED CRA LOADING WITH PACKED DECIMAL
import polars as pl
...
def decode_packed_decimal(data, length, scale=0):
    """
...
def load_cra_file(file_path):
    """
    Load CRA binary file with packed decimal fields
    """
    records = []
    ...
    try:
        ...
        # Each record is 856 bytes (based on SAS INPUT statement)
        record_length = 856
        num_records = len(data) // record_length
        
        print(f"  Found {num_records} records of {record_length} bytes each")
        ...
        for i in range(num_records):
            ...
            # Parse fields based on SAS INPUT statement
            # @001 BRANCH $3.
            ...
            # @067 INVCURAC PD6.
            invcurac = decode_packed_decimal(record[66:72], 6, 0)
            
            # @073 CUSTNAME $140.
            custname = record[72:212].decode('ascii', errors='ignore').strip()
            
            # @443 INVAMT PD7.2
            invamt = decode_packed_decimal(record[442:449], 7, 2)
            ...
            # @477 DCIRT PD7.7
            dcirt = decode_packed_decimal(record[476:483], 7, 7)
            
            # @486 TENOR PD2.
            tenor = decode_packed_decimal(record[485:487], 2, 0)
            
            # @488 INV_STATUS $3.
            inv_status = record[487:490].decode('ascii', errors='ignore').strip()
            
            # @494 ACCINT PD8.6
            accint = decode_packed_decimal(record[493:501], 8, 6)
            
            # @839 CUSTCODE_DB2 2.
            custcode_db2 = int(record[838:840].decode('ascii', errors='ignore').strip()) if record[838:840].strip() else 0
...
def main():
    ...
    try:
        ...
        # Try to load as binary packed decimal
        try:
            cra = load_cra_file(cra_path)
            print(f"  ✓ Loaded CRA using packed decimal parser: {len(cra):,} rows")
        except Exception as e:
            print(f"  Packed decimal parser failed: {e}")
            print(f"  Falling back to fixed-width parser...")
            cra = load_fixed_width_file(
                cra_path,
                [3, 60, 6, 140, 7, 10, 10, 7, 2, 3, 8, 2],
            ...
            print(f"  ✓ Loaded CRA using fixed-width parser: {len(cra):,} rows")
