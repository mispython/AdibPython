...
if df_credsub is not None:
    ...
    print("  CCRIS file not found - creating empty DataFrame")
...
if df_loan.height > 0:
    ...
    # MATDATE formatting (simplified - just keep as is for now)
    if 'MATUREDT' not in df_loan.columns:
        df_loan = df_loan.with_columns([pl.lit('').alias('MATDATE')])
    else:
        df_loan = df_loan.with_columns([pl.lit('').alias('MATDATE')])  # Placeholder
    
    # LASTTRA1 formatting (simplified)
    if 'LASTTRAN' not in df_loan.columns:
        df_loan = df_loan.with_columns([pl.lit('').alias('LASTTRA1')])
    else:
        df_loan = df_loan.with_columns([pl.lit('').alias('LASTTRA1')])  # Placeholder
