...
def save_to_sas(df, dataset_name):
    ...
    try:
        ...
        # Create SAS dataset - fix the parameter issue
        # sasdata() expects the data as first argument and table= as keyword
        sas.sasdata(df_pandas, table=dataset_name.upper())
        
        # Save to permanent SAS dataset
        sas.submit(f"""
            libname out '{STORE_DIR}';
            data out.{dataset_name.upper()};
                set {dataset_name.upper()};
            run;
        """)
...
def main():
    ...
    if os.path.exists(STORE_DIR):
        files = [f for f in os.listdir(STORE_DIR) if f.endswith('.parquet') or f.endswith('.sas7bdat')]
