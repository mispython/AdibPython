...
# Print CA account numbers for debugging
print("CA account numbers (first 20):")
print(ca['ACCTNO'].head(20).tolist())
...
# Parse COLL with better understanding of structure
# From the sample, collateral numbers are 25 digits (like '0508000000000000000000000')
# and account numbers are 12 digits (like '940000159406')
coll_data = []
coll_lines = coll_raw.split('\n')

for line in coll_lines:
    # Find all digit sequences
    all_numbers = re.findall(r'\d{10,30}', line)
    
    if len(all_numbers) >= 2:
        # The first long number (20+ digits) is likely the collateral
        # The shorter number (10-13 digits) is likely the account
        coll_candidates = [n for n in all_numbers if len(n) >= 20]
        acct_candidates = [n for n in all_numbers if 10 <= len(n) <= 13]
        
        if coll_candidates and acct_candidates:
            # Extract the relevant parts
            coll_no_full = coll_candidates[0]
            acct_no = acct_candidates[0]
            
            # The collateral number likely has a specific format
            # Extract first 14 digits as the actual collateral number
            coll_no = coll_no_full[:14] if len(coll_no_full) >= 14 else coll_no_full
...
if not coll.empty:
    print("COLL sample data (first 20):")
    print(coll.head(20))
    
    # Try to match with CA accounts
    # CA accounts are like '3071098223' (10 digits)
    # COLL accounts are like '940000159406' (12 digits)
    # The last 10 digits might match
    ca['ACCTNO_STR'] = ca['ACCTNO'].astype(str).str.rstrip('.0')
    coll['ACCTNO_STR'] = coll['ACCTNO'].astype(str)
    
    # Try different matching strategies
    # 1. Exact match
    coll['EXACT_MATCH'] = coll['ACCTNO_STR'].isin(ca['ACCTNO_STR'])
    
    # 2. Last 10 digits match
    coll['LAST10_MATCH'] = coll['ACCTNO_STR'].str[-10:].isin(ca['ACCTNO_STR'].str[-10:])
    
    # 3. Remove leading zeros and match
    coll['CLEAN_ACCT'] = coll['ACCTNO_STR'].str.lstrip('0')
    coll['CLEAN_MATCH'] = coll['CLEAN_ACCT'].isin(ca['ACCTNO_STR'].str.lstrip('0'))
    
    print(f"COLL exact matches: {coll['EXACT_MATCH'].sum()}")
    print(f"COLL last-10 matches: {coll['LAST10_MATCH'].sum()}")
    print(f"COLL clean matches: {coll['CLEAN_MATCH'].sum()}")
    
    # Use the best matching strategy
    coll_matched = coll[coll['CLEAN_MATCH'] | coll['EXACT_MATCH'] | coll['LAST10_MATCH']]
    print(f"COLL matched with CA: {coll_matched.shape[0]} rows")
    
    if not coll_matched.empty:
        print("Matched COLL records:")
        print(coll_matched.head(10))

print("\nReading DESC file (EBCDIC)...")
desc_raw = read_ebcdic_file(DESC_FILE)

# Parse DESC with original fixed-width format
desc_lines = desc_raw.split('\n')
desc_data = []

for line in desc_lines:
    if len(line) >= 220:
        coll_no = line[0:11].strip()
        inst_cl = line[50:52].strip()
        nat_guar = line[54:56].strip()
        census = line[210:220].strip()
        
        if coll_no and inst_cl:
            desc_data.append({
                'CCOLLNO': coll_no,
                'CINSTCL': inst_cl,
                'NATGUAR': nat_guar,
...
if not desc.empty:
    # Filter for records with NATGUAR='RI' (Individual)
    desc_ri = desc[desc['NATGUAR'] == 'RI'].copy()
    print(f"DESC with NATGUAR='RI': {desc_ri.shape[0]} rows")
    
    if not desc_ri.empty:
        print("DESC RI sample:")
        print(desc_ri.head(10))
        
        # Clean collateral numbers for matching
        desc_ri['CCOLLNO_CLEAN'] = desc_ri['CCOLLNO'].str.lstrip('0')
        coll['CCOLLNO_CLEAN'] = coll['CCOLLNO'].str.lstrip('0')
        
        # Merge COLL with DESC
        coll_merged = coll.merge(
            desc_ri[['CCOLLNO_CLEAN', 'CINSTCL', 'NATGUAR']].drop_duplicates(),
            on='CCOLLNO_CLEAN',
            how='inner'
        )
        print(f"\nCOLL merged with DESC (RI): {coll_merged.shape[0]} rows")
        
        if not coll_merged.empty:
            print("Merged COLL-DESC sample:")
            print(coll_merged.head(10))
            ...
            print("WARNING: No matches between COLL and DESC RI records")
            dep = ca.copy()
            dep['CR'] = None
    else:
        print("WARNING: No RI records in DESC")
        dep = ca.copy()
        dep['CR'] = None
else:
