...
def from_sas_date(sas_date):
    """Convert SAS date to Python date"""
    return SAS_EPOCH + timedelta(days=sas_date)
...
print("=" * 80)
print(f"Report Date (REPTDATE): {reptdate}")
print(f"Previous Month (PREVMON): {params['PREVMON']}")
print(f"Next Month Start (SDATE): {sdate}")
...
def read_base_sas(prevmon):
    """Read BTBASE dataset for the specified month"""
    ...
    if filepath is None:
        print(f"\nERROR: BASE dataset not found for month {prevmon}")
        print("Tried the following paths:")
        for path in possible_paths:
            print(f"  - {path}")
        raise FileNotFoundError(f"BASE dataset not found for month {prevmon}")
...
# --------------------------------------------------------------------
# Step 5: Calculate OVERDUE and RECOVAMT (matching SAS DATA COMBT)
# --------------------------------------------------------------------
...
def write_fixed_width(df, filepath):
    ...
    # Get FACILITY mapping (from PBBBTFMT)
    facility_map = {
        'PBZ': 'PBZ',
        'PBA': 'PBA',
        'PBI': 'PBI',
...
test_cases = [
    ...
    {"ACCTNO": 2505707731, "TRANSREF": "Y080732", "EXP_OVERDUE": 1131, "EXP_RECOVAMT": -555.60},
    {"ACCTNO": 2505707731, "TRANSREF": "Y080733", "EXP_OVERDUE": 1131, "EXP_RECOVAMT": -555.60},
]
