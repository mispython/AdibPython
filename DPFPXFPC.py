...
def process_gl_data(df_gl, conditions, suffix):
    ...
    for condition, item_name, week_col, month_col, qtr_col, halfyr_col, year_col, last_col, total_col in conditions:
        ...
        if len(filtered) > 0:
            # Create a DataFrame with consistent schema
            result = filtered.select([
                pl.lit(item_name).alias('ITEM')
            ])
            
            # Add each column with proper null handling
            if week_col is not None:
                result = result.with_columns(week_col.alias('WEEK'))
            else:
                result = result.with_columns(pl.lit(None).cast(pl.Float64).alias('WEEK'))
                
            if month_col is not None:
                result = result.with_columns(month_col.alias('MONTH'))
            else:
                result = result.with_columns(pl.lit(None).cast(pl.Float64).alias('MONTH'))
                
            if qtr_col is not None:
                result = result.with_columns(qtr_col.alias('QTR'))
            else:
                result = result.with_columns(pl.lit(None).cast(pl.Float64).alias('QTR'))
                
            if halfyr_col is not None:
                result = result.with_columns(halfyr_col.alias('HALFYR'))
            else:
                result = result.with_columns(pl.lit(None).cast(pl.Float64).alias('HALFYR'))
                
            if year_col is not None:
                result = result.with_columns(year_col.alias('YEAR'))
            else:
                result = result.with_columns(pl.lit(None).cast(pl.Float64).alias('YEAR'))
                
            if last_col is not None:
                result = result.with_columns(last_col.alias('LAST'))
            else:
                result = result.with_columns(pl.lit(None).cast(pl.Float64).alias('LAST'))
                
            if total_col is not None:
                result = result.with_columns(total_col.alias('TOTAL'))
            else:
                result = result.with_columns(pl.lit(None).cast(pl.Float64).alias('TOTAL'))
