...
if datasets_to_combine:
    ...
    if 'progcd' not in deposit_combined.columns:
        ...
        # Convert product to float for proper comparisons
        ...
        if not deposit_filtered.is_empty():
            # Remove duplicates by ACCTNO (keep first occurrence)
            ...
            # DEBUG: Show sample DEPOSIT ACCTNOs
            print("\n  - DEBUG: Sample DEPOSIT ACCTNOs (first 10):")
            sample_acctnos = deposit_filtered.select('acctno').head(10).rows()
            for row in sample_acctnos:
                print(f"    {row[0]}")
        else:
...
if float_df is not None:
    ...
    if not float_df.is_empty() and 'float' in float_df.columns and 'acctno' in float_df.columns:
        ...
        # DEBUG: Show sample FLOAT ACCTNOs
        print("\n  - DEBUG: Sample FLOAT ACCTNOs (first 10):")
        sample_float_acctnos = float_summary.select('acctno').head(10).rows()
        for row in sample_float_acctnos:
            print(f"    {row[0]}")
    else:
...
if not deposit_filtered.is_empty() and not float_summary.is_empty():
    ...
    # Directly find FLOAT records not in DEPOSIT
    ...
    if not float_only.is_empty():
        ...
        # For B AND NOT A, CURBAL = 0, AVBAL = -FLOAT (since CURBAL - FLOAT = -FLOAT)
