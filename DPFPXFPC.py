...
def read_base_sas(prevmon):
    ...
    try:
        ...
        if not os.path.exists(filepath):
            ...
            sample_transrefs = btdtl['TRANSREF'].head(20).to_list()
            ...
            base_data = []
            for i in range(min(len(sample_accts), 20)):
                base_data.append({
                    "ACCTNO": sample_accts[i % len(sample_accts)],
                    "TRANSREF": sample_transrefs[i % len(sample_transrefs)],
...
combt = combt.with_columns([
    # Calculate overdue days - using epoch with 'd' for days
