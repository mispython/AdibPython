...
# Check FLAG1, BORSTAT, DELQCD which might be filters
for col in ['FLAG1', 'BORSTAT', 'DELQCD', 'EXCL', 'REINPROD', 'NEW']:
    if col in loan_pl.columns:
        print(f"\n{col} value counts (top 10):")
        value_counts = loan_pl.group_by(col).agg(pl.len().alias('count')).sort('count', descending=True).head(10)
        for row in value_counts.iter_rows(named=True):
            print(f"  {row[col]}: {row['count']}")

# Check if there's a filter on specific columns
# Based on production having ~13,376 records vs our 54,276, 
# there might be a filter reducing records by ~75%

# Check NEWIC for missing values (since it's used for encryption)
if 'NEWIC' in loan_pl.columns:
    newic_null = loan_pl.filter(pl.col('NEWIC').is_null()).height
    newic_empty = loan_pl.filter(pl.col('NEWIC').cast(pl.Utf8).str.strip_chars() == '').height
    print(f"\nNEWIC statistics:")
    print(f"  Null: {newic_null}")
    print(f"  Empty: {newic_empty}")
    print(f"  Non-empty: {len(loan_pl) - newic_null - newic_empty}")

# Filter records where REPAID > 100000
rlslist = loan_pl.filter(pl.col('REPAID') > 100000)
print(f"\nRecords after REPAID > 100000: {len(rlslist)}")

# Additional potential filters based on business logic
# Check if we should filter out empty GUAREND
...
# Check if NEWIC is required
if 'NEWIC' in rlslist.columns:
    rlslist = rlslist.filter(pl.col('NEWIC').is_not_null())
    rlslist = rlslist.filter(pl.col('NEWIC').cast(pl.Utf8).str.strip_chars() != '')
    print(f"Records after requiring NEWIC: {len(rlslist)}")
