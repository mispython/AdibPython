...
try:
    ...
    # ---- Overlap diagnostics: is a small/zero downstream result data, or a bug? ----
    coll_unique_ccollno = coll["ccollno"].n_unique()
    desc_unique_ccollno = desc["ccollno"].n_unique()
    overlap_ccollno = (
        coll.select("ccollno").unique()
        .join(desc.select("ccollno").unique(), on="ccollno", how="inner")
        .height
    )
    print(f"  Distinct CCOLLNO -- COLL: {coll_unique_ccollno}, DESC: {desc_unique_ccollno}, "
          f"overlap: {overlap_ccollno}")
    if coll_unique_ccollno > 0 and desc_unique_ccollno > 0:
        print(f"  Overlap as % of DESC's distinct CCOLLNO: "
              f"{overlap_ccollno / desc_unique_ccollno:.1%}")
    print(f"  DESC rows matching filter criteria independently: "
          f"CINSTCL='18' -> {desc.filter(pl.col('cinstcl') == '18').height}, "
          f"NATGUAR='06' -> {desc.filter(pl.col('natguar') == '06').height}, "
          f"BOTH -> {desc.filter((pl.col('cinstcl') == '18') & (pl.col('natguar') == '06')).height}")
...
if loan.height > 0:
    print(f"  (of {loan.height} LOAN rows, this used a COLL universe already filtered "
          f"to CINSTCL='18' AND NATGUAR='06' -- a small or zero result here can be a "
          f"genuine data outcome on a tiny LOAN sample like this one, not necessarily a "
          f"decode bug. Check the LOAN acctno/noteno pairs against COLL's full "
          f"(unfiltered) acctno/noteno set if you need to confirm.)")
