...
def yyyymmdd_to_date(s: str) -> Optional[date]:
    ...
    try:
        ...
        logger.warning(f"Invalid date format: {s}")
        return None
...
# =========================
# DATA PARSING - BASED ON ORIGINAL WORKING CODE
# =========================
def parse_rpvdata() -> pl.DataFrame:
    """
    Parse RPVBDATA.txt using the original working logic
    This matches the output from your original script that gave 1207 records
    """
    records = []
        ...
        for line in f:
            ...
            if line.startswith('1'):
                ...
                if len(parts) >= 15:  # Minimum required fields
                    record = {
                        # Core fields from original working code
                        ...
                        # Date components - using the same logic as original
...
def process_rpvb_data(df: pl.DataFrame) -> pl.DataFrame:
    ...
    # Create date field from components
    if all(col in df.columns for col in ['YY1', 'MM1', 'DD1']):
        df = df.with_columns([
            ...
            .otherwise(
                pl.datetime(
                    pl.col("YY1").cast(pl.Int32),
                    pl.col("MM1").cast(pl.Int32), 
                    pl.col("DD1").cast(pl.Int32)
                ).cast(pl.Date)
            )
            .alias("DATEWOFF")
        ]).drop(["YY1", "MM1", "DD1"])
