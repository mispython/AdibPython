...
# ============================================================================
# ROUNDING HELPER - Match SAS rounding behavior
# ============================================================================

def sas_round(value, decimals=2):
    """
    Simulate SAS rounding behavior.
    SAS uses round-to-even (banker's rounding) for .5 cases.
    """
    if value is None:
        return None
...
def format_amount(amount):
    ...
    SAS uses fixed decimal with proper rounding.
    """
    if amount is None or amount == 0:
        return "0"
    
    # Round to 2 decimal places using SAS rounding
    rounded = sas_round(amount, 2)
...
def main():
    ...
    try:
        ...
        for row in df_note.iter_rows(named=True):
            ...
            # ITEM is ALWAYS '219' for EIBMABTL
            # SAS: PROD = 'BT' (hardcoded) -> OTHERWISE -> ITEM = '219'
            item = '219'
        ...
        with open(NLFBT, 'w') as f:
            ...
            for row in df_summary.iter_rows(named=True):
                # Format amount to match SAS rounding
