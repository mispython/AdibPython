...
def eibrp159():
    ...
    if ln_file.exists():
        try:
            ...
            print(f"LN columns: {list(ln_df.columns)}")
            ln_df = pl.from_pandas(ln_df)
            # Convert column names to lowercase
            ln_df.columns = [col.lower() for col in ln_df.columns]
            print(f"LN columns (lowercase): {ln_df.columns}")
        except Exception as e:
    ...
    if len(dp_df) > 0 and len(ln_df) > 0:
        ...
        for col in all_columns:
            if col not in dp_df.columns:
                ...
                print(f"Added missing column '{col}' to DP dataframe")
            if col not in ln_df.columns:
                ln_df = ln_df.with_columns(pl.lit(None).alias(col))
                print(f"Added missing column '{col}' to LN dataframe")
    ...
    if len(npgs_df) > 0 and 'cvarxx' not in npgs_df.columns:
        ...
        print(f"Added cvarxx column. Total rows: {len(npgs_df)}")
