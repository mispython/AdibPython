...
"""
Program Name : CGCRPT
Purpose      : Public Bank Berhad - CGC Report Template
               Reusable PROC REPORT equivalent called via %INC PGM(CGCRPT) from its
               calling program (e.g. EIBRNPGS-style driver) for each schedule code.
               Generates ASA carriage-control detail listing report for
               CGC (Credit Guarantee Corporation) submissions, sourced from NPGS3.

Original SAS:
  PROC   REPORT DATA=NPGS3 NOWD HEADSKIP HEADLINE SPLIT='*';
  COLUMN CVAR01 CVAR02 CVAR03 CVAR04 CVAR05 CVAR06 CVAR07 CVAR08
         CVAR09 CVAR10 CVAR11 CVAR12A CVAR13 CVAR14 CVAR15;
...
#
# NOTE vs NPGSRPT:
#   - No CVARXX filler column
#   - No trailing BRANCH column
#   - CVAR12A (not CVAR12) supplies the 'ST ' status column
# =============================================================================
...
def cgc_report(
    ...
    """
    Generate an ASA carriage-control CGC detail listing report.
    ...
    df          : Polars DataFrame — already filtered / derived as NPGS3,
                  and sorted BY CVAR01 CVAR06 (or as required) by the caller.
    report_path : Destination file path for the ASA report.
    title1      : TITLE1 text  (e.g. 'PUBLIC BANK BERHAD')
    title2      : TITLE2 text  (e.g. 'CGC DETAIL OF ACCTS (SCH=70) ...')
    """
