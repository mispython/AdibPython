...
df_hploan = df_hploan.with_columns([
    ...
    # NOTE: made non-strict - some source files (e.g. Islamic/Aitab extracts)
    # encode ISSUEDT differently than the conventional MMDDYYYY assumption
    # below, which previously caused a hard crash. Rows that don't match
    # become null (ISSUE DATE blank in the detail report) instead of failing
    # the whole run. See printed diagnostic below for how many rows this hits.
    pl.col('ISSUEDT').cast(pl.Utf8).str.slice(0, 8).str.to_datetime('%m%d%Y', strict=False).alias('ISSDTE'),
