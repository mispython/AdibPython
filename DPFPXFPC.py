...
# =========================
print("Reading LOAN/LNNOTE datasets...")
loani_ln = read_sas7bdat(LOANI_LNNOTE)  # Islamic - ENTITY_CD = 'PIBB'
loan_ln  = read_sas7bdat(LOAN_LNNOTE)   # Conventional - ENTITY_CD != 'PIBB'

# Apply entity filters
if 'entity_cd' in loani_ln.columns:
    loani_ln = loani_ln.filter(pl.col('entity_cd') == 'PIBB')
if 'entity_cd' in loan_ln.columns:
    loan_ln = loan_ln.filter(pl.col('entity_cd') != 'PIBB')
...
# =========================
print("Processing COMM datasets...")
loani_comm = read_sas7bdat(LOANI_LNCOMM)  # Islamic
loan_comm  = read_sas7bdat(LOAN_LNCOMM)   # Conventional

# Apply entity filters
if 'entity_cd' in loani_comm.columns:
    loani_comm = loani_comm.filter(pl.col('entity_cd') == 'PIBB')
if 'entity_cd' in loan_comm.columns:
    loan_comm = loan_comm.filter(pl.col('entity_cd') != 'PIBB')
