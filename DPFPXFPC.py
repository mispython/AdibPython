...
def parse_desc_record(record_bytes):
    ...
    try:
        ...
        # Convert CCOLLNO to integer (not float)
...
# =========================
# Merge COLL and DESC - FIXED DATA TYPES
# =========================
print("\n" + "="*60)
...
if coll.height > 0 and desc.height > 0:
    # Ensure CCOLLNO is same type (Int64) in both dataframes
    coll = coll.with_columns(pl.col("CCOLLNO").cast(pl.Int64))
    desc = desc.with_columns(pl.col("CCOLLNO").cast(pl.Int64))
    
    print(f"COLL CCOLLNO dtype: {coll.schema['CCOLLNO']}")
    print(f"DESC CCOLLNO dtype: {desc.schema['CCOLLNO']}")
    
    # Perform the join
    coll = coll.join(desc, on="CCOLLNO", how="inner")
    
    # Filter for CINSTCL='18' AND NATGUAR='06'
