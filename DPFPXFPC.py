...
def generate_text_report(deposit_merged, float_only, output_path):
    ...
    print(f"  - deposit_merged records: {deposit_merged.height}")
    print(f"  - float_only records: {float_only.height}")
    
    if not deposit_merged.is_empty():
        print(f"  - Columns in deposit_merged: {deposit_merged.columns}")
        print(f"  - Sample of float column: {deposit_merged['float'].head(5)}")
        print(f"  - Sample of curbal column: {deposit_merged['curbal'].head(5)}")
        print(f"  - Sample of product column: {deposit_merged['product'].head(5)}")
    ...
    with open(report_path, 'w') as f:
        ...
        if not float_only.is_empty():
            ...
            for row in float_only_sorted.rows():
                ...
                if acctno is None or (isinstance(acctno, float) and pd.isna(acctno)):
                    acctno_str = ''
                else:
                    acctno_str = str(int(acctno)) if isinstance(acctno, (int, float)) else str(acctno)
                
                prodcd_str = str(prodcd) if prodcd is not None else ''
                amtind_str = str(amtind) if amtind is not None else ''
                branch_str = str(int(branch)) if branch is not None and not pd.isna(branch) else ''
                ledgbal_str = f"{ledgbal:.2f}" if ledgbal is not None and not pd.isna(ledgbal) else ''
                product_str = str(int(product)) if product is not None and not pd.isna(product) else ''
...
if datasets_to_combine:
    ...
    print(f"  - Combined columns: {deposit_combined.columns}")
    if 'progcd' in deposit_combined.columns:
        print(f"  - Unique PROGCD values: {deposit_combined['progcd'].unique().head(10)}")
...
if float_df is not None:
    ...
    if not float_df.is_empty() and 'float' in float_df.columns and 'acctno' in float_df.columns:
        ...
        # Debug: Check float_summary
        print(f"  - FLOAT_SUMMARY sample: {float_summary.head(5)}")
    else:
...
if not deposit_filtered.is_empty():
    ...
    if not float_summary.is_empty():
        ...
        print(f"  - Float column null count: {deposit_merged['float'].null_count()}")
        print(f"  - Curbal column null count: {deposit_merged['curbal'].null_count()}")
        print(f"  - Product column null count: {deposit_merged['product'].null_count()}")
        ...
        # Condition: float is not null AND (curbal is null OR product is null)
        ...
        if not float_only.is_empty():
            print(float_only.select(['acctno', 'float', 'curbal', 'product']).head(10))
