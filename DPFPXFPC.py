...
def save_sas_dataset(df, dataset_name, output_dir):
    ...
    try:
        ...
        # Create SAS dataset - pass DataFrame and dataset name separately
        sas_df = sas.sasdata(df_sas, dataset_name)
        
        # Save to permanent SAS dataset using the correct method
        # The sasdata object has a to_file method
        ...
        try:
            # Alternative: Use submit to create SAS dataset
            sas.submit(f"""
                data {output_dir}/{dataset_name};
                    set {dataset_name};
                run;
            """)
            print(f"  ✓ Saved SAS dataset (alt method): {dataset_name}.sas7bdat")
            return True
