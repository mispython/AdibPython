...
INPUT_BASE = "input/prod/btbase"                 # BASE DD - Previous month snapshot
    ...
    with open(filepath, 'r') as f:
        for line_num, line in enumerate(f, 1):
            ...
            try:
                ...
                branch = int(line[1:5].strip()) if line[1:5].strip().isdigit() else 0
                acctno = int(line[5:15].strip())
                transref = line[15:22].strip()
                outstanding = float(line[22:37].strip()) if line[22:37].strip() else 0.0
                matdt = line[37:43].strip()
                liabcode = line[42:45].strip()
                
                if not acctno or not matdt.isdigit() or len(matdt) != 6:
                    continue
...
def read_base(prevmon):
    ...
    base_file = f"{INPUT_BASE}{prevmon}.sas7bdat"
    if not os.path.exists(base_file):
        base_file = f"{INPUT_BASE}_{prevmon}.sas7bdat"
    if not os.path.exists(base_file):
        base_file = f"input/prod/BTBASE{prevmon}.sas7bdat"
    if not os.path.exists(base_file):
        base_file = f"input/prod/btbase{prevmon}.sas7bdat"
    
    if not os.path.exists(base_file):
        print(f"ERROR: BASE file not found for month {prevmon}")
        print(f"Tried: {base_file}")
