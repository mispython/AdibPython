...
def write_output_saspy(df, sas_path):
    ...
    try:
        ...
        # Upload DataFrame to SAS - use the correct method
        # Convert pandas DataFrame to SAS using saspy's DataFrame upload
        sas.dataframe(pd_df, temp_ds)
        
        # Save to permanent location with proper path handling
        ...
        try:
            ...
            try:
                ...
                if hasattr(pyreadstat, 'write_sas7bdat'):
                    ...
                    # Use pyreadstat's write function if available
                    pyreadstat.write(pd_df, str(sas_path))
                    print(f"✓ Saved SAS: {sas_path}")
                    return True
    except Exception as e:
