    ...
    with open(report_path, 'w') as f:
        ...
        if not float_only.is_empty():
            ...
            for row in float_only_sorted.rows():
                ...
                curbal_str = '0' if curbal is None or (isinstance(curbal, float) and pd.isna(curbal)) else f"{curbal:.2f}"
                ...
                curbaltt_str = '0' if curbaltt is None or (isinstance(curbaltt, float) and pd.isna(curbaltt)) else f"{curbaltt:.2f}"
