...
MICR_FILE = Path("/sas/python/virt_edw/Data_Warehouse/MIS/XMIS/input/prod/EIBTNPGS/bopess.txt")  # Changed to .txt
...
# Read text file - it appears to be fixed-width format without header
# The file likely has specific column positions
...
# Parse the fixed-width data - adjust column positions based on actual file format
# Common SAS export format - adjust widths as needed
crft_data = crft_data.with_columns([
    pl.col('data').str.slice(0, 1).alias('RECTYP1'),
    pl.col('data').str.slice(1, 8).alias('TFID'),
    pl.col('data').str.slice(8, 11).alias('SUBACCT'),
    pl.col('data').str.slice(11, 12).alias('PREIND'),
    pl.col('data').str.slice(12, 13).cast(pl.Int64).alias('CENSUST'),
    pl.col('data').str.slice(13, 25).cast(pl.Int64).alias('ACCTNO')
...
# Read text file for MICR data
micr_data = pl.read_csv(
    MICR_FILE, 
    has_header=False, 
    separator='\t',
    new_columns=['data']
