...
def format_dcitxt_row(row, is_interbank=False):
    """Format a single row for DCITXT output matching SAS format"""
    if is_interbank:
        # Interbank format
        return (f"{str(row.get('CUSTICKETNO', '')):>20} "
                ...
                f"{row.get('STATUSIND', ''):>12} "
                f"{row.get('STARTDT', ''):>12} "
                f"{row.get('MATDT', ''):>12} "
                f"{row.get('PREMREC', 0):>12,.2f} "
                f"{row.get('PREMRECRM', 0):>12,.2f}")
    else:
        # Customer format
        return (f"{str(row.get('CUSTICKETNO', '')):>20} "
                f"{str(row.get('TICKETNO', '')):>10} "
                ...
                f"{row.get('INVCURAC', 0):>12} "
                f"{row.get('ALTCURAC', 0):>12} "
                f"{str(row.get('INVCURR', '')):>8} "
                f"{str(row.get('ALTCURR', '')):>8} "
                f"{row.get('INVAMT', 0):>12,.2f} "
                f"{row.get('ALTAMT', 0):>12,.2f} "
...
def main():
    ...
    # -------------------------------------------------------------------
    # Step 3-6: Processing (same as before)
    # -------------------------------------------------------------------
    print("\nProcessing DPST...")
    ...
    # -------------------------------------------------------------------
    # Write DCITXT with exact SAS format
    # -------------------------------------------------------------------
    ...
    with open(text_path, "w") as f:
        ...
        total_accint = cusmyr['ACCINT'].sum() if len(cusmyr) > 0 else 0
        total_accintrm = cusmyr['ACCINTRM'].sum() if len(cusmyr) > 0 else 0
        total_prempaid = cusmyr['PREMPAID'].sum() if len(cusmyr) > 0 else 0
        total_prempaidrm = cusmyr['PREMPAIDRM'].sum() if len(cusmyr) > 0 else 0
        ...
        if len(ibnmyr) > 0:
            # ... (similar format for interbank)
            pass

    print(f"  ✓ DCITXT written to {text_path}")

    # -------------------------------------------------------------------
    # Step 7-8: Build DCI and write outputs
    # -------------------------------------------------------------------
