...
print(f"COLL_FILE exists: {COLL_FILE.exists()}")
print(f"DESC_FILE exists: {DESC_FILE.exists()}")
...
crft_merged = crft_merged.with_columns([
    pl.col('FICODE').cast(pl.Utf8).alias('BRANCH')  # Cast to string
])
...
subalmt_data = suba1_summary.join(suba2_summary, on='ACCTNO', how='full')  # Changed from 'outer' to 'full'
...
try:
    ...
    micr_data = micr_data.with_columns([
        pl.col('data').str.slice(0, 5).cast(pl.Utf8).alias('BRANCH'),  # Cast to string
        pl.col('data').str.slice(5, 11).alias('MICRCD')
    ]).select(['BRANCH', 'MICRCD'])
    
    # Ensure BRANCH is string type in mast_final
...
if 'MATURED1' not in npgs_data.columns:
    npgs_data = npgs_data.with_columns([pl.lit(None).cast(pl.Float64).alias('MATURED1')])
if 'MATUREDS' not in npgs_data.columns:
    npgs_data = npgs_data.with_columns([pl.lit(None).cast(pl.Float64).alias('MATUREDS')])
