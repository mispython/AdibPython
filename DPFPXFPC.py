...
# ==================================================
# Helper to write SAS7BDAT files using saspy or xport
# ==================================================
def write_sas7bdat(df: pl.DataFrame, filepath: Path):
    """Write DataFrame to SAS7BDAT format.
    Tries saspy first, falls back to XPORT format, then CSV as last resort.
    """
    sas_path = filepath.with_suffix('.sas7bdat')
    pd_df = df.to_pandas()
    
    # Try using saspy if available
    try:
        import saspy
        sas = saspy.SASsession()
        # Upload DataFrame to SAS
        sas.df2sd(pd_df, table='_temp_output', libref='WORK')
        # Export to sas7bdat
        sas.submit(f"""
            PROC EXPORT DATA=WORK._temp_output
                OUTFILE="{sas_path}"
                DBMS=SAS7BDAT REPLACE;
            RUN;
        """)
        sas.endsas()
        print(f"  Written: {sas_path} (using saspy)")
        return
    except ImportError:
        pass
    ...
    # Fall back to XPORT format (SAS transport file)
    try:
        xpt_path = filepath.with_suffix('.xpt')
        pyreadstat.write_xport(pd_df, str(xpt_path), table_name=filepath.stem)
        print(f"  Written: {xpt_path} (XPORT format, SAS compatible)")
        print(f"  Note: sas7bdat not available, wrote XPORT instead")
        return
    except Exception as e:
        print(f"  XPORT write failed: {e}")
    
    # Last resort: write CSV with SAS column names
    try:
        csv_path = filepath.with_suffix('.csv')
        pd_df.to_csv(csv_path, index=False)
        print(f"  Written: {csv_path} (CSV format)")
        print(f"  Note: sas7bdat not available, wrote CSV instead")
    except Exception as e:
