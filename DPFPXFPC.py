...
print(f"DPST TICKETNO dtype: {dpst['TICKETNO'].dtype}")
...
print(f"EQTN TICKETNO dtype: {eqtn['TICKETNO'].dtype}")

# -------------------------
# STEP 5: Merge DPST & EQTN by TICKETNO
# -------------------------
# Ensure TICKETNO columns have the same format (strip whitespace and pad if needed)
dpst = dpst.with_columns(pl.col("TICKETNO").str.strip().str.zfill(7))
eqtn = eqtn.with_columns(pl.col("TICKETNO").str.strip().str.zfill(7))
...
if len(common_tickets) == 0:
    ...
    if "CUSTICKETNO" in eqtn.columns and "TICKETNO" in dpst.columns:
        print("Trying join on CUSTICKETNO...")
        dcid = dpst.join(
            eqtn.select(["CUSTICKETNO", "BRANCH", "PRODUCT", "INVCURR", "ALTCURR", 
                        "INVAMT", "ALTAMT", "TRADEDT", "STARTDT", "FIXINGDT", "MATDT", 
                        "TENOR", "STRIKERT", "SPOTRT", "DCIRT", "MMRT", "PREMREC", 
                        "PREMPAID", "UNWINDCOST", "NEWDEAL", "STATUSIND", "STOPDT"])
            .rename({"CUSTICKETNO": "TICKETNO"}),
            on="TICKETNO", how="inner"
        )
else:
...
# Combine all reference data (now all CUSTCODE2 are Int64)
