...
if data.get('imast') is not None:
    mast = data['imast'].filter(
        ...
        pl.when(pl.col("retailid") == 'C').then(999).otherwise(0).cast(pl.Int64).alias("apcode"),
        pl.lit(0).cast(pl.Int64).alias("oldbrh"),
        pl.lit("     ").alias("ficody")
    ])
    
    # Apply $LOCUSTCD format from PBBLNFMT
    ...
    # Convert custcode_clean to numeric for LOCUSTCD format
    mast = mast.with_columns([
    ...
    # Apply $RVRSE format from PBBLNFMT for SECTFISS
    mast = mast.with_columns([
        pl.when(
    ...
    # Apply $INDSECT format if industrial_sector_cd exists
...
if data.get('icred') is not None:
    ...
    # Calculate MATUREDS (YYMMDD format)
    ...
    # Calculate ARREARS (using DAYR format from PBBBTFMT)
    cred = cred.with_columns([
        pl.when(
            (pl.col("matureds") > 0) & 
...
if mast is not None and suba9 is not None:
    ...
    # Apply date calculations for CREATDS
