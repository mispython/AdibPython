...
if datasets_to_combine:
    ...
    if 'progcd' in deposit_combined.columns:
        sample_progcd = deposit_combined.select('progcd').head(20).rows()
        ...
        # Show unique values
        unique_vals = deposit_combined['progcd'].unique().to_list()[:20]
        print(f"    Sample unique prodcd values: {unique_vals}")
    ...
    if 'progcd' not in deposit_combined.columns:
        ...
        if not deposit_filtered.is_empty():
            print(f"  - DEBUG: Filtered prodcd unique values: {deposit_filtered['progcd'].unique().to_list()[:10]}")
