...
def process_gl_data(df_gl, conditions, suffix):
    ...
    for condition, item_name, week_col, month_col, qtr_col, halfyr_col, year_col, last_col, total_col in conditions:
        ...
        if len(filtered) > 0:
            ...
            select_exprs = [pl.lit(item_name).alias('ITEM')]
            
            if week_col is not None:
                select_exprs.append(week_col.alias('WEEK'))
            else:
                select_exprs.append(pl.lit(None).alias('WEEK'))
                
            if month_col is not None:
                select_exprs.append(month_col.alias('MONTH'))
            else:
                select_exprs.append(pl.lit(None).alias('MONTH'))
                
            if qtr_col is not None:
                select_exprs.append(qtr_col.alias('QTR'))
            else:
                select_exprs.append(pl.lit(None).alias('QTR'))
                
            if halfyr_col is not None:
                select_exprs.append(halfyr_col.alias('HALFYR'))
            else:
                select_exprs.append(pl.lit(None).alias('HALFYR'))
                
            if year_col is not None:
                select_exprs.append(year_col.alias('YEAR'))
            else:
                select_exprs.append(pl.lit(None).alias('YEAR'))
                
            if last_col is not None:
                select_exprs.append(last_col.alias('LAST'))
            else:
                select_exprs.append(pl.lit(None).alias('LAST'))
                
            if total_col is not None:
                select_exprs.append(total_col.alias('TOTAL'))
            else:
                select_exprs.append(pl.lit(None).alias('TOTAL'))
            
            rows.append(filtered.select(select_exprs))
...
# Format: (condition, item_name, week_expr, month_expr, qtr_expr, halfyr_expr, year_expr, last_expr, total_expr)
conditions_p1 = [
