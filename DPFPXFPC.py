...
def main():
    ...
    # Step 1: Get date parameters
    # OPTION 1: Use a specific date (uncomment and set your desired date)
    # reptdate = datetime(2025, 7, 1)  # Example: July 1, 2025
    
    # OPTION 2: Use the latest available date from the data
    # reptdate = get_latest_date_from_sas(final_base_path)
    
    # OPTION 3: Use today's date (current behavior)
    ...
    # Step 2: Check if date exists in the data
    date_exists, available_dates, latest_date = check_date_availability(final_base_path, sas_date_num)
    
    if not date_exists:
        print(f"\n⚠️  WARNING: Date {reptdate} (SAS: {sas_date_num}) not found in the data")
        if latest_date:
            print(f"   Latest available date: {latest_date}")
            print(f"   (SAS date: {(latest_date - datetime(1960,1,1)).days})")
            print(f"\n   Available date range: {available_dates[0]} to {available_dates[-1]}")
            
            # Ask user if they want to use the latest available date
            print(f"\n   Would you like to process the latest available date instead?")
            # For automatic processing, uncomment the next line:
            # reptdate = latest_date
            # sas_date_num = (reptdate - datetime(1960,1,1)).days
            # print(f"   Using latest available date: {reptdate}")
        return
    
    # Step 3: Process NLF data from multiple SAS7BDAT sources
    conn = duckdb.connect()
    ...
    # Step 4: Build dynamic UNION ALL query based on loaded tables
    union_queries = []
    ...
    # Step 5: Summarize data (equivalent to PROC SUMMARY)
    summary_query = """
    SELECT 
        REPTDATE,
    ...
    # Step 6: Append logic (equivalent to MACRO APPEND)
...
def check_date_availability(final_base_path, sas_date_num):
    """Check if the date exists in any of the SAS files"""
    sas_files = [
        ...
        if os.path.exists(filepath):
            try:
                ...
                if 'DATE' in df.columns:
                    ...
                    if sas_date_num in dates:
                        date_exists = True
                        # Convert SAS date to datetime
                        latest_sas = max(dates)
                        latest_date = datetime(1960, 1, 1) + timedelta(days=latest_sas)
            except:
                pass
    
    # Convert all dates to datetime for display
    readable_dates = sorted([datetime(1960, 1, 1) + timedelta(days=d) for d in all_dates])
    
    return date_exists, readable_dates, latest_date
