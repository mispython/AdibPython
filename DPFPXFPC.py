...
if df_credsub is not None:
    ...
    # If CCRIS file doesn't exist, create empty DataFrame
...
try:
    with open(OUTPUT_FILE1, 'r') as f:
        for line in f:
            if len(line) >= 372:  # Ensure line has enough characters
        ...
        for idx, row in enumerate(df_text.iter_rows(named=True)):
            ...
            if len(line) >= 116:
                ...
                if len(line) > 373:
                    ...
                    f.write(line[407:679] if len(line) > 679 else line[407:])  # GUAREND through DELQCD
                    f.write(f"{delqdes:30}")  # DELQDES
                    f.write(f"{biztype:1}")  # BIZTYPE
                    f.write(line[712:715] if len(line) > 715 else '   ')  # OCCUPAT
                    f.write(f"{occupdes:25}")  # OCCUPDES
                    f.write(line[742:744] if len(line) > 744 else '  ')  # BGC
                    f.write(f"{bgcdes:20}")  # BGCDES
                    f.write(line[766:] if len(line) > 766 else '')  # PAY75PCT through end
                    f.write('\n')
    ...
    print(f"Warning: {OUTPUT_FILE1} not found for processing")
