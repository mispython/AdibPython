...
# Import PBBMRDLF reference data
try:
    from PBBMRDLF import PBBMRDLF
except ImportError:
...
def load_pbbrdal_data():
    """
    Load PBBRDAL reference data from PBBMRDLF.py
    """
    if PBBMRDLF is not None:
        try:
            logger.info("Loading PBBRDAL reference data from PBBMRDLF.py")
            df_ref = PBBMRDLF
...
def load_loan_data_chunked(limit_rows=10000):
    """
    Load loan note data in chunks for memory efficiency
    Only loads first limit_rows records
    """
    loan_paths = [
    ...
    for loan_path in loan_paths:
        if loan_path.exists():
            ...
            try:
                # Use pyreadstat to read only first N rows
                # This is much more memory efficient
                df_loan, meta_loan = pyreadstat.read_sas7bdat(
                    loan_path, 
                    rows_limit=limit_rows
                )
                ...
                # Try alternative: read with pandas first then convert
                try:
                    import pandas as pd
                    logger.info("Trying to read with pandas first...")
                    df_pd = pd.read_sas(loan_path, nrows=limit_rows)
                    df_loan = pl.from_pandas(df_pd)
                    logger.info(f"Loan data loaded via pandas: {len(df_loan)} rows")
                    return df_loan
                except Exception as e2:
                    logger.error(f"Error loading with pandas: {e2}")
...
def load_loan_data_filtered(limit_rows=10000):
    """
    ...
    for loan_path in loan_paths:
        if loan_path.exists():
            logger.info(f"Loading filtered loan data from: {loan_path}")
            try:
                # Read the SAS file with only needed columns
                # and limit rows
                df_loan, meta_loan = pyreadstat.read_sas7bdat(
                    loan_path,
                    rows_limit=limit_rows
                ...
                if zipcode_col:
                    ...
                    logger.warning(f"PZIPCODE column not found. Available: {df_loan.columns}")
...
if len(pbbrdal) > 0:
    if 'ITCODE' in pbbrdal.columns and 'AMTIND' in pbbrdal.columns:
        ...
        logger.error("PBBRDAL missing required columns: ITCODE and/or AMTIND")
...
loan_data = load_loan_data_filtered(limit_rows=50000)  # Limit to 50k rows
