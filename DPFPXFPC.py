...
EGOLD = EGOLD.with_columns([
    pl.lit(".").alias("TRANCODE"),  # Use "." instead of null to match production
    pl.lit(None).cast(pl.Utf8).alias("CHANNEL")
])

# Create TRXNDATE as SAS date (days since 1960-01-01)
# Use map_elements for date conversion
EGOLD = EGOLD.with_columns([
...
# Round numeric columns to match production format (only if they exist and are numeric)
