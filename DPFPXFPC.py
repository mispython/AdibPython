...
if df_sample is not None:
    ...
    for row in unique_vals.iter_rows(named=True):
        print(f"    '{row['BORSTAT']}' : {row['count']} rows")
    ...
    if 'A' in df_sample['BORSTAT'].to_list():
        ...
        unique_vals_stripped = df_sample.group_by('BORSTAT_STRIPPED').agg(pl.count())
        print(f"  Unique values after stripping:")
        for row in unique_vals_stripped.iter_rows(named=True):
            print(f"    '{row['BORSTAT_STRIPPED']}' : {row['count']} rows")
else:
...
if df_npla_raw is not None:
    ...
    borstat_counts = df_npla_raw.group_by('BORSTAT').agg(pl.count())
    ...
    df_npla_raw = df_npla_raw.with_columns([
        pl.col('BORSTAT').cast(pl.Utf8).str.strip().alias('BORSTAT_CLEAN')
    ])
    df_active_clean = df_npla_raw.filter(pl.col('BORSTAT_CLEAN') == 'A')
    print(f"  After strip 'A': {df_active_clean.height} rows")
    
    # 3. Case insensitive
    df_active_case = df_npla_raw.filter(pl.col('BORSTAT_CLEAN').str.to_uppercase() == 'A')
    print(f"  Case insensitive 'A': {df_active_case.height} rows")
    
    # 4. Try numeric values (SAS sometimes stores codes as numbers)
    if 'BORSTAT_NUM' in df_npla_raw.columns or df_npla_raw['BORSTAT'].dtype in [pl.Float64, pl.Int64]:
        df_active_num = df_npla_raw.filter(pl.col('BORSTAT') == 1)  # Try numeric 1
        print(f"  Numeric 1: {df_active_num.height} rows")
    ...
    elif df_active_case.height > 0:
        print("\n  Using case-insensitive filtering...")
        df_npla = df_active_case.with_columns([
            pl.lit(0.0).alias('IIS'),
            (pl.col('FEEDUE').fill_null(0) - pl.col('FEEDUEMS').fill_null(0)).alias('OI'),
            (pl.lit(0.0) + (pl.col('FEEDUE').fill_null(0) - pl.col('FEEDUEMS').fill_null(0))).alias('TOTIIS'),
            (pl.col('FEEDUEMS').fill_null(0) + pl.col('FEEAMT16').fill_null(0)).alias('SP')
