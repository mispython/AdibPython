...
# -------------------------
# Check if PRODTYPE exists in BTDTL, if not use PRODGRP
try:
    ...
    # If PRODTYPE doesn't exist, use PRODGRP
    print("PRODTYPE not found in BTDTL, using PRODGRP instead")
    btdtl_trans = con.execute("""
        SELECT BRANCH, ACCTNO, TRANSREF,
               CASE 
                 WHEN RETAILID = 'R' THEN 0
                 WHEN RETAILID = 'C' THEN 999
                 ELSE CAST(PRODGRP AS INT)
               END AS PRODTYPE_CALC,
               OUTSTAND, MATDATE, FACILITY
        FROM btdtl
        GROUP BY BRANCH, ACCTNO, TRANSREF, 
