...
if fdmthly_df is not None:
    ...
    # Select available columns that match our needs
    available_cols = ['acctno', 'branch', 'curbal', 'amtind']
    existing_cols = [col for col in available_cols if col in fdmthly_df.columns]
    if existing_cols:
        fdmthly_df = fdmthly_df.select(existing_cols)
...
if ifdmthly_df is not None:
    ...
    print(f"  - IMNI FDMTHLY columns: {ifdmthly_df.columns}")
    available_cols = ['acctno', 'branch', 'curbal', 'amtind']
    existing_cols = [col for col in available_cols if col in ifdmthly_df.columns]
    if existing_cols:
        ifdmthly_df = ifdmthly_df.select(existing_cols)
...
if not fdmthly_combined.is_empty():
    # Add LEDGBAL as CURBAL (since CURBAL is the same as LEDGBAL in this context)
...
if curn1_df is not None:
    ...
    print(f"  - MNI CURN124 columns: {curn1_df.columns[:10]}")  # Show first 10 columns
...
if not curn_combined.is_empty():
    if 'product' in curn_combined.columns:
        ...
        print(f"  - Available columns: {curn_combined.columns}")
...
if savg1_df is not None:
    ...
    # Select available columns (all lowercase now)
    available_cols = ['acctno', 'product', 'curbal', 'ledgbal', 'progcd', 'amtind', 'intpaybl', 'branch']
...
if not curn_filtered.is_empty():
    ...
    if existing_cols:
        ...
        print(f"  - No matching columns found in CURN")
        print(f"  - Available columns: {curn_filtered.columns}")
else:
...
if not fdmthly_processed.is_empty():
    # FDMTHLY already has lowercase columns: acctno, branch, curbal, ledgbal, amtind
    # Need to add product, prodcd, intpaybl (set to 0 or null)
    ...
    print(f"    - Columns: {fdmthly_renamed.columns}")
else:
