...
# COMPLETE FIXED VERSION - Islamic Bank Trade Report
...
print("\n" + "=" * 80)
print("DEBUG: Checking specific records")
print("=" * 80)
...
for test in debug_accts:
    ...
    if len(match) > 0:
        ...
        print(f"  PREOUTSTD (from BASE): {row['PREOUTSTD']}")
        print(f"  OUTSTAND (from BTDTL): {row['OUTSTAND']}")
        print(f"  DAYS (from BASE): {row['DAYS']}")
        print(f"  OVERDUE: {row['OVERDUE']}")
        print(f"  RECOVAMT: {row['RECOVAMT']}")
...
test_cases = [
    {"ACCTNO": 2850228011, "TRANSREF": "Y090755", "EXP_PREOUTSTD": 70184.05, "EXP_OUTSTAND": 45067.65, "EXP_OVERDUE": 67, "EXP_RECOVAMT": 25116.40},
    {"ACCTNO": 2850331901, "TRANSREF": "Y090696", "EXP_PREOUTSTD": 50142.80, "EXP_OUTSTAND": 50255.60, "EXP_OVERDUE": 70, "EXP_RECOVAMT": -112.80},
    {"ACCTNO": 2850383833, "TRANSREF": "Y089246", "EXP_PREOUTSTD": 49216.32, "EXP_OUTSTAND": 49322.37, "EXP_OVERDUE": 658, "EXP_RECOVAMT": -106.05},
]
...
for test in test_cases:
    ...
    if len(match) > 0:
        ...
        preoutstd_match = abs(row['PREOUTSTD'] - test["EXP_PREOUTSTD"]) < 0.01
        outstanding_match = abs(row['OUTSTAND'] - test["EXP_OUTSTAND"]) < 0.01
        overdue_match = row['OVERDUE'] == test["EXP_OVERDUE"]
        recovamt_match = abs(row['RECOVAMT'] - test["EXP_RECOVAMT"]) < 0.01
        
        all_match = preoutstd_match and outstanding_match and overdue_match and recovamt_match
        ...
        print(f"  PREOUTSTD:  Expected={test['EXP_PREOUTSTD']:>12.2f}, Got={row['PREOUTSTD']:>12.2f} {'✓' if preoutstd_match else '✗'}")
        print(f"  OUTSTAND:   Expected={test['EXP_OUTSTAND']:>12.2f}, Got={row['OUTSTAND']:>12.2f} {'✓' if outstanding_match else '✗'}")
