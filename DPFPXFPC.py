...
if almdept.height > 0:
    # Show sample BNMCODEs
    print("\nSample BNMCODEs created:")
    print(almdept.select(["BNMCODE", "AMOUNT"]).head(10))
else:
    print("\nNo BNMCODEs created. Checking REMMTH values...")
    # Show REMMTH distribution for records with target CUSTCODEs
    target_records = alm_summary.filter(
        pl.col("CUSTCODE_STR").is_in(["81", "82", "83", "84", "85", "86", "87", "88", "89", "90", "91", "92", "95", "96", "98", "99"])
    )
    if target_records.height > 0:
        print(f"Found {target_records.height:,} records with target CUSTCODEs")
        print("\nREMMTH values for target records:")
        print(target_records.select(["CUSTCODE", "REMMTH"]).head(10))
    else:
        print("No records with target CUSTCODEs in summary")
...
if len(almdept) > 0:
    ...
    print("1. How many records had valid REMMTH")
    print("2. How many records have target CUSTCODEs")
    print("3. What REMMTH values exist for target CUSTCODEs")
