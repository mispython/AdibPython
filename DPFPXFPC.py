...
def load_branch_data() -> pl.DataFrame:
    ...
    if branch_path.exists():
        ...
        try:
            ...
            # Parse the file manually - skip header lines
            data = []
            for line in lines:
                line = line.strip()
                if line and not line.startswith('---') and not line.startswith('B00'):
                    ...
                    parts = line.split()
                    if len(parts) >= 2:
                        # The first part might be branch code (like PCS, JSS)
                        # The second part might be branch name
                        # We need to find the numeric branch code somewhere
                        data.append(parts)
            
            if data:
                # Create dataframe with branch code and name
                df = pl.DataFrame({
                    "BRHCODE": [row[0] for row in data],
                    "BRANCH_NAME": [" ".join(row[1:]) for row in data if len(row) > 1]
                })
                # Add a sequential BRANCH number for joining
                df = df.with_columns(
                    pl.Series("BRANCH", range(1, len(df) + 1)).cast(pl.Int64)
                )
                print(f"   Branch file parsed: {len(df)} branches")
                print(f"   Sample branch codes: {df['BRHCODE'].head(5).to_list()}")
                return df
            else:
...
def calculate_17_bucket_summaries(loan_df: pl.DataFrame) -> Dict:
    ...
    branch_summary = loan_df.group_by(["CAT", "BRANCH", "ARREAR"]).agg([
        ...
        pl.len().alias("NOACC")  # Changed from pl.count() to pl.len()
    ])
...
def generate_17_bucket_report_text(results: Dict, variables: Dict):
    ...
    with open(output_file, 'w') as f:
        ...
        for cat, cat_data in sorted(results.items()):
            # Get the type from the first branch or use Unknown
            branch_type = cat_data['branches'][0].get('TYPE', 'Unknown') if cat_data['branches'] else 'Unknown'
            
            f.write(f"\n{'='*50}\n")
            f.write(f"CATEGORY {cat}: {branch_type}\n")
            f.write(f"{'='*50}\n\n")
...
def main():
    ...
    print(f"   Branch data types: {branch_df.dtypes}")
    
    # 3. Categorize loans
    print("\n3. Categorizing loans...")
    categorized = categorize_loans(loan_df, HPD_LIST)
    print(f"   Categorized records: {len(categorized)}")
    print(f"   Categorized BRANCH type: {categorized['BRANCH'].dtype}")
    
    # 4. Merge with branch data
    print("\n4. Merging with branch data...")
    
    # Since the branch file contains text codes like "PCS", "JSS", "ASR"
    # and the loan data has numeric branch codes, we need to handle this differently
    # We'll create a mapping from branch code to branch ID
    
    # For now, we'll just keep the BRANCH from the loan data and add branch info if available
    # But since we can't match them, we'll just use the loan data as-is
    
    # Check if we can find any matching branch codes
    # The loan data has numeric BRANCH values, the branch file has text codes
    # We'll create a mapping based on position or use a lookup table
    
    # For this case, since we can't match the branch codes directly,
    # we'll keep the loan data as-is and add BRHCODE from branch file if available
    # but we won't do an inner join that might lose data
    
    # Instead, let's just use the categorized data and add a dummy BRHCODE
    merged_data = categorized.with_columns(
        pl.lit("").alias("BRHCODE")
    )
    
    # If we have branch data, we can try to map it
    if len(branch_df) > 0:
        # Try to match based on position or create a mapping
        # Since we can't match directly, we'll just add the branch codes as a separate column
        branch_codes = branch_df["BRHCODE"].to_list()
        # Create a mapping from branch index to branch code
        branch_map = {i+1: code for i, code in enumerate(branch_codes)}
        
        # Map branch codes to the merged data
