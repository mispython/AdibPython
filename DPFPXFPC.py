...
# STEP 1: Read LNNOTE with LOANTYPE=575 (original requirement)
print("\n" + "="*60)
print("STEP 1: Reading LNNOTE (LOANTYPE=575, CENSUS=575.09)")
print("="*60)

def filter_lnnote_575(chunk):
    if chunk.height == 0:
        return chunk
    ...
    if "LOANTYPE" in chunk.columns and "CENSUS" in chunk.columns:
        chunk = chunk.filter(
            ...
            (pl.col("CENSUS").cast(pl.Float64) == 575.09)
        )
...
loan_base = read_sas7bdat_in_chunks(
    ...
    filter_func=filter_lnnote_575,
    keep_columns=essential_cols
)

print(f"Filtered LNNOTE rows (LOANTYPE=575): {loan_base.height}")

if loan_base.height == 0:
    print("\n" + "="*60)
    print("NO DATA FOUND FOR LOANTYPE=575, CENSUS=575.09")
    print("The August 2026 file does not contain the required loan type.")
    print("This is expected if LOANTYPE=575 is a specific product")
...
if loan1.height > 0:
    ...
    # Process COLL and DESC files
    print("\nReading COLL file...")
    coll = read_fixed_length_ebcdic(COLL_FILE, COLL_RECORD_LENGTH, parse_coll_record, chunk_size=10000)
    
    print("\nReading DESC file...")
    desc = read_fixed_length_ebcdic(DESC_FILE, DESC_RECORD_LENGTH, parse_desc_record, chunk_size=10000)
    ...
    if coll.height > 0 and desc.height > 0:
        ...
        print("Insufficient COLL/DESC data")
