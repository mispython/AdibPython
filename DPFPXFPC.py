...
print(f"  Available columns in LOAN: {loan_pl.columns}")
...
print(f"  RLSLIST columns: {rlslist.columns}")
...
print(f"  LNNAME columns: {lnname_pl.columns}")

# Merge with RLSLIST - include GUAREND from rlslist for encryption
pbbname = lnname_pl.join(
    rlslist.select(['ACCTNO', 'GUAREND']), 
    on='ACCTNO', 
    how='inner'
)
...
# For email output, we need the original ID (validated) for the index file
...
print(f"  LNINAME columns: {lni_pl.columns}")
