...
def write_report_matching_sas():
    """Generate report matching SAS PROC REPORT output with exact column alignment"""
    ...
    with open(REPORT_PATH, 'w') as f:
        ...
        for row in rlslist_report.iter_rows(named=True):
            ...
            if current_branch is None:
                # First occurrence - show branch code (right-aligned in 7 chars)
                branch_display = f"{branch:>7d}"
                current_branch = branch
            else:
                # Subsequent occurrences - blank (7 spaces)
                branch_display = " " * 7
