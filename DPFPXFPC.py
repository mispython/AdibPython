...
def read_desc_fixed_width_binary(file_path: Path, max_rows: int = None) -> pl.DataFrame:
    """
    Read DESC file as fixed-width binary/text without relying on newlines.
    ...
    The DESC file appears to be fixed-width records without newline delimiters.
    Need to determine the record length.
    """
    if not file_path.exists():
        raise FileNotFoundError(f"File not found: {file_path}")
    ...
    # Based on SAS code, CENSUS ends at position 220 (211 + 10 - 1)
    # But there might be additional fields after position 220
    # Let's try common record lengths for fixed-width files
    record_length = None
    
    # First, let's read a sample to understand the structure
    with open(file_path, 'rb') as f:
        sample = f.read(1000)
    
    # Try to find a pattern or determine record length
    # Check if file size is divisible by common record lengths
    possible_lengths = [220, 221, 240, 250, 256, 300, 320, 400, 500, 512, 1000]
    ...
    if record_length is None:
        # Try to detect record length by looking for patterns
        # For now, assume 220 bytes as minimum (based on CENSUS position)
        record_length = 220
        print(f"Using minimum record length: {record_length}")
    else:
        print(f"Detected record length: {record_length}")
    ...
    with open(file_path, 'rb') as f:
        for chunk_start in range(0, total_records, chunk_size):
            ...
            for i in range(records_to_read):
                ...
                # Extract fields
                ccollno_bytes = record[0:11]  # @001, 11 chars
                cinstcl_bytes = record[50:52]  # @051, 2 chars
                natguar_bytes = record[54:56]  # @055, 2 chars
                census_bytes = record[210:220]  # @211, 10 chars
                
                # Decode with error handling
                try:
                    ccollno_str = ccollno_bytes.decode('ascii', errors='ignore').strip()
                    cinstcl = cinstcl_bytes.decode('ascii', errors='ignore').strip()
                    natguar = natguar_bytes.decode('ascii', errors='ignore').strip()
                    census_str = census_bytes.decode('ascii', errors='ignore').strip()
                except:
                    ccollno_str = ""
                    cinstcl = ""
                    natguar = ""
