...
for pattern in ccris_file_patterns:
    ...
    if df_credsub is not None:
        ...
        print(f"  Columns in CCRIS file: {df_credsub.columns}")
        break

if df_credsub is not None and df_credsub.height > 0:
    # Check for the correct column names
    cred_cols = df_credsub.columns
    
    # Find the account number column - try different possible names
    acct_col = None
    for col in ['ACCTNUM', 'ACCTNO', 'ACCT_NUM', 'ACCOUNT', 'ACCOUNTNO']:
        if col in cred_cols:
            acct_col = col
            break
    
    # Find the days arrears column
    days_col = None
    for col in ['DAYSARR', 'DAYS', 'DAYS_ARR', 'DAYSARREARS']:
        if col in cred_cols:
            days_col = col
            break
    
    # Find the facility column
    facility_col = None
    for col in ['FACILITY', 'FACILITY_CD', 'FACILITYCD', 'FAC']:
        if col in cred_cols:
            facility_col = col
            break
    ...
    if acct_col is None:
        ...
        print(f"  Available columns: {cred_cols}")
...
if df_loan.height > 0:
    ...
    try:
        df_loan = df_loan.with_columns([
            pl.when(pl.col('ECSRRSRV') > 0).then('Y').otherwise('N').alias('ECSRIND')
        ])
    except Exception as e:
    ...
    try:
        df_loan = df_loan.with_columns([
            ((pl.col('ORGBAL').fill_null(0) - pl.col('CURBAL').fill_null(0)) / 
             pl.when(pl.col('PAYAMT').fill_null(0) > 0).then(pl.col('PAYAMT')).otherwise(1))
            .cast(pl.Int64).alias('BILPAID')
        ])
    except Exception as e:
