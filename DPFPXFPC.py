...
# EBCDIC to ASCII translation table for common characters
EBCDIC_TO_ASCII = {
...
def read_desc_ebcdic_efficient(file_path: Path, max_rows: int = None) -> pl.DataFrame:
    ...
    The DESC file is in EBCDIC encoding, not ASCII.
    SAS INPUT:
    @001 CCOLLNO   11.  - 11 characters (EBCDIC digits)
    @051 CINSTCL   $2.  - 2 characters (EBCDIC)
    @055 NATGUAR   $2.  - 2 characters (EBCDIC)
    @211 CENSUS    10.  - 10 characters (EBCDIC digits)
    
    Record length appears to be 220 bytes based on the data.
    """
    if not file_path.exists():
        raise FileNotFoundError(f"File not found: {file_path}")
    ...
    # Record length - based on the debug output, 220 bytes seems correct
    # File size 4962984400 / 220 = 22,559,020 records
    record_length = 220
    ...
    with open(file_path, 'rb') as f:
        for chunk_start in range(0, total_records, chunk_size):
            ...
            for i in range(records_to_read):
                ...
                # Extract EBCDIC fields
                ccollno_bytes = record[0:11]  # @001, 11 bytes
                cinstcl_bytes = record[50:52]  # @051, 2 bytes
                natguar_bytes = record[54:56]  # @055, 2 bytes
                census_bytes = record[210:220]  # @211, 10 bytes
                ...
                if processed <= 3:
                    print(f"Record {processed}: CCOLLNO={ccollno}, CINSTCL={cinstcl}, NATGUAR={natguar}, CENSUS={census}")
