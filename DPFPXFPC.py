...
from pbblnfmt import (
    get_remfmt, get_days_in_month, get_liqpfmt, format_btcustcd,
)

# Import from PBBELF (macro functions only)
from pbbelf import (
...
def main(reptdate=None):
    ...
    try:
        ...
        for row in ba_data.iter_rows(named=True):
            ...
            # Product type using get_liqpfmt from PBBLNFMT
            product = row.get('PRODUCT', 0) or 0
            prod_type = get_liqpfmt(product)
