...
def main():
    ...
    if rept_day == "01":
        ...
        # Write SAS file using saspy or pyreadstat
        write_sas_dataset(nlf_summary, output_sas_path, mode='create')
        ...
        if os.path.exists(output_parquet_path):
            try:
                ...
                # Write SAS file using saspy or pyreadstat
                write_sas_dataset(combined_df, output_sas_path, mode='update')
...
def write_sas_dataset(df, output_path, mode='create'):
    """
    Write a pandas DataFrame to a SAS dataset using saspy or pyreadstat.
    
    Parameters:
    - df: pandas DataFrame to write
    - output_path: path to the output SAS file
    - mode: 'create' or 'update'
    """
    sas = None
    
    try:
        # Try using saspy first
        if SASPY_AVAILABLE:
            try:
                # Connect to SAS session
                sas = saspy.SASsession()
                
                # Create a temporary SAS dataset from DataFrame
                temp_sas_name = 'NLF_TEMP'
                sas.dataframe2sasdata(df, table=temp_sas_name)
                
                # Use SAS code to export the dataset
                # The output path needs to be properly quoted for SAS
                sas_code = f'''
                    libname out "{os.path.dirname(output_path)}";
                    
                    data out.{os.path.basename(output_path).replace('.sas7bdat', '')};
                        set work.{temp_sas_name};
                    run;
                    
                    proc datasets lib=work;
                        delete {temp_sas_name};
                    run;
                '''
                ...
                if 'ERROR' in str(result):
                    print(f"   SAS error detected in log")
                    raise Exception("SAS code execution failed")
                
                sas.endsas()
                print("   ✅ SAS dataset written successfully using saspy")
                return True
                ...
                if sas:
                    try:
                        ...
                        pass
                # Fall through to pyreadstat
        
        # Fallback to pyreadstat - try to use it directly
        try:
            # Check if we can create a SAS file using pyreadstat
            # Some versions have write_sas7bdat
            if hasattr(pyreadstat, 'write_sas7bdat'):
                pyreadstat.write_sas7bdat(output_path, df)
                print("   ✅ SAS dataset written successfully using pyreadstat")
                return True
            else:
                # Try alternative import
                try:
                    from pyreadstat import write_sas7bdat
                    write_sas7bdat(output_path, df)
                    print("   ✅ SAS dataset written successfully using pyreadstat")
                    return True
                except ImportError:
                    raise AttributeError("pyreadstat.write_sas7bdat not available")
                
        except Exception as e:
            print(f"   pyreadstat method failed: {e}")
            raise
        
    except Exception as e:
        print(f"⚠️  Warning: Could not write SAS file: {e}")
        # Note: The Parquet and CSV files were still created successfully
        return False
