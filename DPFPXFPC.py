...
def write_sas_dataset(df, output_path, mode='create'):
    ...
    try:
        # Try using saspy first (more reliable for SAS dataset writing)
        if SASPY_AVAILABLE:
            try:
                ...
                temp_sas_name = 'WORK.NLF_TEMP'
                sas.dataframe2sasdata(df, table=temp_sas_name)
                
                # Export to file using the correct method
                # Use proc copy to export the dataset
                sas.submit(f'''
                    proc copy inlib=WORK outlib=WORK;
                        select NLF_TEMP;
                    run;
                    
                    proc export data=WORK.NLF_TEMP
                        outfile="{output_path}"
                        dbms=sas7bdat
                        replace;
                    run;
                ...
                # Clean up
                sas.submit(f'proc datasets lib=WORK; delete NLF_TEMP; run;')
                sas.quit()
        ...
        try:
            # Check if pyreadstat has write_sas7bdat
