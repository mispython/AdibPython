...
PRODBRH_ISLAMIC = {
    ...
    '46795': 'DEBIT CARD (E)',
    'TRUST': 'TRUST ACCT',
    # Islamic specific products might include:
    'ISLAMIC_CA': 'ISLAMIC CA',
    'ISLAMIC_SA': 'ISLAMIC SA',
    'ISLAMIC_FD': 'ISLAMIC FD',
}

# Format for Islamic product descriptions
PRODFMD_ISLAMIC = {
    '42110': 'CA (A)',
    '42310': 'CA (A)',
    '34180': 'CA (A)',
...
def process_islamic_trust_data(pidmfin_path: Path) -> pl.DataFrame:
    """
    Process Islamic TRUST data from CISDEPXN
    Similar to conventional but with Islamic banking filters
    """
    logger.info("Processing Islamic TRUST data from cisdepxn.sas7bdat")
    
    # Read SAS file
    cisdepxn_df = read_sas_to_polars(pidmfin_path / "cisdepxn.sas7bdat")
    
    if cisdepxn_df.is_empty():
        logger.warning("PIDMFIN.cisdepxn is empty or not found - skipping Islamic TRUST data")
        return pl.DataFrame()
    ...
    # Filter for Islamic accounts (ACCTYPE2 values might differ for Islamic)
    # Based on your data structure, you may need to adjust the filter conditions
    ...
    logger.info(f"Islamic TRUST records: {trust_df.height:,}")
    return trust_df
...
def apply_islamic_format_mappings(df: pl.DataFrame) -> pl.DataFrame:
    """
    Apply PRODBRH format mapping to PRODCD column for Islamic products
    """
    if 'PRODCD' in df.columns:
        try:
            logger.info("Applying Islamic format mappings using replace() method")
            df = df.with_columns([
                pl.col('PRODCD').replace(PRODBRH_ISLAMIC).alias('PRODCD_FORMATTED')
            ])
        except AttributeError:
            logger.info("Falling back to map_elements() method")
            df = df.with_columns([
                pl.col('PRODCD').map_elements(
                    lambda x: PRODBRH_ISLAMIC.get(x, str(x)), 
...
def main():
    ...
    try:
        ...
        trust_df = process_islamic_trust_data(pidmfin_path)
        ...
        # Apply format mappings for Islamic products
        rpt_base = apply_islamic_format_mappings(rpt_base)
