...
def eibrsmee():
    ...
    # Map actual column names to expected CVAR names based on the error message
    # The actual columns are: curbal, costctr, accrual, balance, product, censust, sch, cinstcl, natguar, tranche
    # Plus already mapped: cvar02, cvar01, cvar06, cvar03, cvar04, cvar14, cvar13, cvar08, cvar09, cvar10, cvar11, cvar05, cvar07, cvar12, cvar15, branch
    
    # Based on the error, some cvar columns already exist, but we need to map the others
    # Let's check if we need to create mappings for missing columns
    
    # The key mappings from the error suggest:
    # - cvar02 = sch (Schedule)
    # - cvar01 = ? (Reference Number - probably from product or another field)
    # - cvar06 = ? (Account Number - probably from product)
    # - cvar12 = censust (Status - NPL/AP)
    # - cvar13 = ? (NPL Date)
    
    # Create a mapping dictionary
    column_mapping = {}
    
    # Check if columns need to be renamed/mapped
    if "sch" in npgs_df.columns and "cvar02" not in npgs_df.columns:
        column_mapping["sch"] = "cvar02"
    
    if "censust" in npgs_df.columns and "cvar12" not in npgs_df.columns:
        column_mapping["censust"] = "cvar12"
    ...
    # Apply mappings
    if column_mapping:
        npgs_df = npgs_df.rename(column_mapping)
    
    # Now process based on what columns are actually available
    # Create SMEE dataset
    smee_columns = []
    ...
    # Filter for SCH=E5
    # Use actual column names for filtering
    filter_conditions = []
    
    # Check if cvar02 (schedule) exists and filter
    if "cvar02" in npgs_df.columns:
        filter_conditions.append(pl.col("cvar02") == "E5")
    
    # Check if natguar exists and filter
    if "natguar" in npgs_df.columns:
        filter_conditions.append(pl.col("natguar") == "06")
    
    # Check if cinstcl exists and filter
    if "cinstcl" in npgs_df.columns:
        filter_conditions.append(pl.col("cinstcl") == "18")
    
    if filter_conditions:
        npgs_df = npgs_df.filter(pl.all_horizontal(filter_conditions))
    
    # Ensure cvarxx exists
    if "cvarxx" not in npgs_df.columns:
