...
def eibrtrrf():
    ...
    # Process NPGS data - Fix the issue here
...
def process_scheme(npgs_df, scheme, rdate, base):
    ...
    npgs5_df = npgs5_df.with_columns(
        ...
        pl.when(pl.col("cvar17").is_null()).then(0.0).otherwise(pl.col("cvar17")).alias("cvar17_clean"),
        pl.when(pl.col("accrual").is_null()).then(0.0).otherwise(pl.col("accrual")).alias("accrualx")
    )
    
    # Rename cvar17_clean back to cvar17
...
def apply_npl_logic(df):
    ...
    return df.with_columns(
        ...
        pl.when(pl.col("cvar17").is_null()).then(0.0).otherwise(pl.col("cvar17")).alias("cvar17_clean"),
        pl.when(pl.col("accrual").is_null()).then(0.0).otherwise(pl.col("accrual")).alias("accrualx")
    ).rename({"cvar17_clean": "cvar17"}).sort(["cvar01", "cvar06"])
