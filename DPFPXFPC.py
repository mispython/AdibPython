    ...
    with open(OUTPUT_FILE, 'w') as f:
        ...
        for cat, cat_group in loantemp_final_df.groupby('CAT'):
            ...
            for branch, branch_group in cat_group.groupby('BRANCH_NUM'):
                ...
                # Line 1: Branch number and first 5 columns
                # SAS format: @1 BRANCH Z3. @5 NOACC1 COMMA7.0 @13 BRHAMT1 COMMA16.2 etc.
                f.write(f" {branch:3d}    {noacc[0]:>7,.0f}       {brhamt[0]:>15,.2f}       "
                       f"{noacc[1]:>7,.0f}       {brhamt[1]:>15,.2f}       "
                       f"{noacc[2]:>7,.0f}       {brhamt[2]:>15,.2f}        "
                       f"{noacc[3]:>8,.0f}        {brhamt[3]:>17,.2f}        "
                       f"{noacc[4]:>8,.0f}        {brhamt[4]:>17,.2f}\n")
                
                # Line 2: BRHCODE and columns 6-10
                f.write(f" {brhcode:<3}    {noacc[5]:>7,.0f}       {brhamt[5]:>15,.2f}       "
                ...
                # Line 4: Columns 16-17 and subtotals
