...
def load_remit():
    ...
    try:
        ...
        print(f"    REMIT columns: {remit.columns}")
        
        # Ensure consistent columns
        if 'paymode' not in remit.columns:
            if 'acctno' in remit.columns:
                remit = remit.rename({'acctno': 'paymode'})
            else:
        ...
        # Print columns for debugging
        print(f"    UNCLAIM columns: {unclaim.columns}")
        
        # Ensure consistent columns for unclaim
        if 'paymode' not in unclaim.columns:
            if 'acctno' in unclaim.columns:
                unclaim = unclaim.rename({'acctno': 'paymode'})
            else:
        ...
        if 'unclaimx' not in unclaim.columns:
            if 'ledgbal' in unclaim.columns:
                unclaim = unclaim.with_columns(pl.col('ledgbal').alias('unclaimx'))
            else:
        ...
        summary = combined.group_by('paymode').agg([
            pl.col('ledgbal').cast(pl.Float64, strict=False).fill_null(0).sum().alias('plusbal'),
            pl.col('unclaimx').cast(pl.Float64, strict=False).fill_null(0).sum().alias('unclaim')
        ])
        
        # Create result with acctno
...
def load_saving():
    ...
    try:
        ...
        # Select only needed columns with consistent order
        ...
        # Filter and select with consistent column order
...
def load_current():
    ...
    try:
        ...
        # Apply FX conversion if forate exists
        if 'forate' in df.columns and 'curcode' in df.columns:
            df = df.with_columns([
...
def load_dep(d):
    ...
    try:
        ...
        if Path(savg_file).exists():
            ...
            needed_cols = ['acctno', 'amtind', 'prodcd', 'product']
            for col in needed_cols:
                if col not in df.columns:
                    df = df.with_columns(pl.lit(None).alias(col))
            
            dfs.append(df.select(needed_cols))
...
def main():
    ...
    # Load REMIT/UNCLAIM
    remit_df = load_remit()
    print(f"  REMIT/UNCLAIM: {len(remit_df)} records loaded")
    ...
    print(f"  SA columns: {sa.columns if not sa.is_empty() else 'empty'}")
    print(f"  CA columns: {ca.columns if not ca.is_empty() else 'empty'}")
    print(f"  FD columns: {fd.columns if not fd.is_empty() else 'empty'}")
    
    # Concatenate non-empty DataFrames with consistent columns
    dfs_to_concat = []
    for df, name in [(sa, 'SA'), (ca, 'CA'), (fd, 'FD')]:
        if not df.is_empty():
    ...
    if not saca.is_empty():
        ...
        if not float_df.is_empty():
            ...
            trustee = trustee.with_columns([
                ...
                (pl.col('curbal').cast(pl.Float64, strict=False).fill_null(0) - 
                 pl.col('float').fill_null(0)).alias('avbal')
            ])
        else:
            trustee = saca.with_columns([
                pl.lit(0).alias('float'),
                pl.col('curbal').cast(pl.Float64, strict=False).fill_null(0).alias('avbal')
            ])
        ...
        if not trustee.is_empty():
            ...
            if not remit_df.is_empty():
                trustee = trustee.join(remit_df.select(['acctno', 'plusbal', 'unclaim']), 
                                     on='acctno', how='left')
                trustee = trustee.with_columns([
            ...
            trustee = trustee.with_columns([
                (pl.col('avbal') + 
                 pl.col('intpaybl').cast(pl.Float64, strict=False).fill_null(0) + 
                 pl.col('plusbal').fill_null(0) + 
                 pl.col('unclaim').fill_null(0)).alias('avbaltt')
            ])
    ...
    if (not client_df.is_empty() and not saca.is_empty() and 
        ...
        if not client.is_empty():
            ...
            # Merge with REMIT
            if not remit_df.is_empty():
                client = client.join(remit_df.select(['acctno', 'plusbal', 'unclaim']), 
                                   on='acctno', how='left')
                client = client.with_columns([
                    pl.col('plusbal').fill_null(0),
                    pl.col('unclaim').fill_null(0)
