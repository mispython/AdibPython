...
output_data = {
    'CVAR01': dep_filtered['ACCTNO'].astype(str).str.rstrip('.0').values,
    'CVAR02': dep_filtered['CVAR02'].values,
    'CVAR03': dep_filtered['NEWIC'].values if 'NEWIC' in dep_filtered.columns else '',
    'CVAR04': dep_filtered['CUSTNAME'].values if 'CUSTNAME' in dep_filtered.columns else '',
    'CVAR05': dep_filtered['LMTSTART'].values if 'LMTSTART' in dep_filtered.columns else pd.NaT,
    'CVAR06': dep_filtered['ACCTNO'].astype(str).str.rstrip('.0').values,
    'CVAR07': 'OD',
    ...
    'CVAR13': dep_filtered['NPLDATE'].dt.strftime('%d/%m/%Y').values if 'NPLDATE' in dep_filtered.columns else '',
    'CVAR14': '0233',
    'CVAR15': dep_filtered['MICRCD'].values if 'MICRCD' in dep_filtered.columns else ''
}
...
if not npla_df.empty:
    ...
    npgs = npgs.merge(npla_df[['CVAR06', 'CVAR01', 'STATUS', 'NDATE']], 
...
print(f"Output columns: {list(npgs.columns)}")

# Remove NPLA-specific columns if they exist in output
