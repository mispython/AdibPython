...
def load_saca_trustee():
    """Load SA/CA/FD data for TRUSTEE processing (WITH PURPOSE filter)
       DATA SA; SET SACA.SAVING; WHERE PURPOSE IN ('5','6'); KEEP...; RUN;
       DATA CA; SET SACA.CURRENT; WHERE PURPOSE IN ('5','6'); IF CURCODE NE 'MYR' THEN...; KEEP...; RUN;
       DATA FD; SET SACA.FD; WHERE PURPOSE IN ('5','6'); IF CURCODE NE 'MYR' THEN...; KEEP...; RUN;"""
    dfs = []
...
def load_saca_client():
    ...
    try:
        ...
        # KEEP ACCTNO BRANCH PRODUCT PURPOSE CURBAL INTPAYBL CURCODE FORATE
