...
def eibrtlio():
    ...
    if 'cvar13' in combined_df.columns:
        ...
        # Write TL as sas7bdat using pyreadstat (more reliable than saspy)
        if not tl_df.is_empty():
            try:
                tl_pandas = tl_df.to_pandas()
                pyreadstat.write_sas7bdat(
                    tl_pandas, 
                    str(output_path / "tl.sas7bdat"),
                    encoding='utf-8'
                )
                print(f"TL sas7bdat written successfully")
            except Exception as e:
                print(f"Error writing TL to sas7bdat via pyreadstat: {e}")
        else:
    ...
    # Write NPGS3 as sas7bdat using pyreadstat (more reliable than saspy)
    if not npgs3_df.is_empty():
        try:
            npgs3_pandas = npgs3_df.to_pandas()
            pyreadstat.write_sas7bdat(
                npgs3_pandas, 
                str(output_path / "npgs3.sas7bdat"),
                encoding='utf-8'
            )
            print(f"NPGS3 sas7bdat written successfully")
        except Exception as e:
            print(f"Error writing NPGS3 to sas7bdat via pyreadstat: {e}")
            # Alternative: Try using saspy with correct syntax
            try:
                npgs3_pandas = npgs3_df.to_pandas()
                sas.df2sd(npgs3_pandas, table='npgs3', libref='work')
                # Use PROC DATASETS to copy to a permanent library
                sas.submit(f"""
                ...
                """)
                print(f"NPGS3 sas7bdat written successfully via saspy")
            except Exception as e2:
                print(f"Error writing NPGS3 to sas7bdat via saspy: {e2}")
    else:
