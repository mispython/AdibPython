# eibdcitx.py - FIXED CRA PARSING AND FORMATTING
import polars as pl
...
def main():
    ...
    with open(text_path, "w") as f:
        ...
        for row in cusmyr.iter_rows(named=True):
            ...
            # Get values and ensure they are numeric
            ...
            try:
                accint = float(accint) if accint else 0
            except:
                accint = 0
            try:
                accintrm = float(accintrm) if accintrm else 0
            except:
                accintrm = 0
            try:
                prempaid = float(prempaid) if prempaid else 0
            except:
                prempaid = 0
            try:
                prempaidrm = float(prempaidrm) if prempaidrm else 0
            except:
                prempaidrm = 0
        ...
        if len(cusmyr) > 0:
            ...
            for row in cusmyr.iter_rows(named=True):
                try:
                    total_accint += float(row.get('ACCINT', 0))
                except:
                    pass
                try:
                    total_accintrm += float(row.get('ACCINTRM', 0))
                except:
                    pass
                try:
                    total_prempaid += float(row.get('PREMPAID', 0))
                except:
                    pass
                try:
                    total_prempaidrm += float(row.get('PREMPAIDRM', 0))
                except:
                    pass
