...
if CISDP_DF.exists():
    ...
    # Read only accounts that match CA accounts
    # First, get list of CA account numbers
    ca_accounts_temp = current_df[current_df['PRODUCT'].isin([108, 112, 114])]['ACCTNO'].unique()
    print(f"CA account numbers to match: {len(ca_accounts_temp)}")
    ...
    while True:
        try:
            ...
            # Filter for SECCUST == '901' AND ACCTNO in CA accounts
            if 'SECCUST' in chunk.columns:
                filtered_chunk = chunk[
                    (chunk['SECCUST'] == '901') & 
                    (chunk['ACCTNO'].isin(ca_accounts_temp))
                ][required_cols].copy()
    ...
    if cisdp_chunks:
        ...
        print(f"CISDP dataset: {cisdp_df.shape[0]} matching rows for CA accounts")
        
        if not cisdp_df.empty:
            print(f"CISDP sample matching data:")
            print(cisdp_df.head())
        
        del cisdp_chunks
    else:
        cisdp_df = pd.DataFrame(columns=required_cols)
        print("Warning: No matching CISDP data found for CA accounts")
else:
...
# STEP 4C: CISDP MERGE
# =========================
# Convert ACCTNO to same type for merging
ca['ACCTNO_MERGE'] = ca['ACCTNO'].astype(float)
if not cisdp_df.empty:
    cisdp_df['ACCTNO_MERGE'] = cisdp_df['ACCTNO'].astype(float)
    
    # Merge using the float key
    ca = ca.merge(
        cisdp_df[['ACCTNO_MERGE', 'NEWIC', 'CUSTNAME']].rename(columns={'ACCTNO_MERGE': 'ACCTNO_MERGE'}),
        on='ACCTNO_MERGE',
        how='left'
    )
else:
...
output_data = {
    ...
    'CVAR03': dep_filtered['NEWIC'].fillna('').astype(str).str.rstrip('.0').str.rstrip('.').values if 'NEWIC' in dep_filtered.columns else '',
    ...
    'CVAR14': '0233',
    'CVAR15': dep_filtered['MICRCD'].fillna('').astype(str).str.rstrip('.0').str.rstrip('.').values if 'MICRCD' in dep_filtered.columns else ''
}
...
if not npla_df.empty and 'CVAR06' in npla_df.columns and 'CVAR01' in npla_df.columns:
    npla_merge = npla_df.copy()
    npla_merge['CVAR06'] = npla_merge['CVAR06'].astype(str).str.rstrip('.0').str.rstrip('.')
    npla_merge['CVAR01'] = npla_merge['CVAR01'].astype(str).str.rstrip('.0').str.rstrip('.')
    
    # Keep only needed columns
    merge_cols = ['CVAR06', 'CVAR01']
    if 'STATUS' in npla_merge.columns:
        merge_cols.append('STATUS')
    if 'NDATE' in npla_merge.columns:
        merge_cols.append('NDATE')
    
    npgs = npgs.merge(
        npla_merge[merge_cols].drop_duplicates(subset=['CVAR06', 'CVAR01']),
        on=['CVAR06', 'CVAR01'], 
        how='left'
    )
