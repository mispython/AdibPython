...
DATA ALLEQU LCR.EQU&REPTMON;
   ...
   *NSFR;
   ...
   ELSE IF CUSTFISS IN (00,17,45,57,59,60,61,62,63,64,
   ...
   *18-1761;
   ...
   NSFCODE = BIC||CUSX||PUT(REM30D,REMFMX.)||'00'||DLTYPE||'Y';
...
PROC SORT DATA=SME.IBASELSME&REPTMON&REPTYEAR
          OUT=LCR.SME; BY ACCTNO; RUN;
   ...
   *16-4512;
   ...
   IF TOTICBAL > 250000 THEN DO; /* PROPORTION INSURED/UNINSURED */
      ...
      ELSE DO;
         ...
         IF SUBSTR(BNMCODE,6,2) IN ('49') AND ECP NE '01' THEN
             NSFCODE = SUBSTR(NSFCODE,1,7)||'10'||SUBSTR(NSFCODE,10,5);
         OUTPUT;                    /* INSURED   */
...
DATA LCR.NSFR&REPTMON(KEEP=BIC BNMCODE CMMCODE BRANCH ACCTNO CUSTCD
                          PRODUCT CURCODE AMOUNT CUSTNO NEWIC OLDIC
                          CUSTNAME REM30D REMMTH ECP CDNO MATDT
                          BILLERIND TOTDPBAL TOTICBAL TOTICEQBAL
                          SME_TAG PBMERCH NSFCODE INTPLAN);
   SET ALLMNI;
   IF BIC IN ('95315','95317') THEN DO;
      NSFCODE = SUBSTR(NSFCODE,1,9)||PUT(REMMTH,REMFMX.)||'00Y';
      IF FDHOLD = 'Y' THEN DO;
...
DATA REPORT SREPORT;
   ...
   %IF "&CCY" EQ "SKIPCUR" %THEN %DO;
       IF GL AND CURCODE IN ('USD','SGD') THEN OTHSOURCE = 0; *MAIN;
   %END;
   %ELSE %DO;
...
%LET CCY=('MYR');       %LCRPRINT(MYR,LCRMYR);
