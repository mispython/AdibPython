...
# ============================================
# OPTION: Choose which type to process
# ============================================
# Set to 'BOTH', 'CONVENTIONAL', or 'ISLAMIC'
PROCESS_TYPE = 'CONVENTIONAL'  # Change this to match production
...
if PROCESS_TYPE in ['BOTH', 'ISLAMIC']:
    ...
    if df is not None:
        ...
        print(f"  - IMNI FDMTHLY not found")
...
if PROCESS_TYPE in ['BOTH', 'ISLAMIC']:
    ...
    if df is not None:
        ...
        print(f"  - IMNI CURN124 not found")
...
if PROCESS_TYPE in ['BOTH', 'CONVENTIONAL']:
    ...
    if df is not None:
        ...
        print(f"  - MNI SAVG124 not loaded")
...
if PROCESS_TYPE in ['BOTH', 'ISLAMIC']:
    ...
    if df is not None:
        ...
        print(f"  - IMNI SAVG124 not loaded")
...
if datasets_to_combine:
    ...
    if 'progcd' not in deposit_combined.columns:
        ...
        # Convert prodcd to string for proper filtering
        ...
        if not deposit_filtered.is_empty():
            ...
            # DEBUG: Show branch distribution
            print("\n  - DEBUG: DEPOSIT branch distribution:")
            branch_dist = deposit_filtered.group_by('branch').agg(pl.len().alias('count')).sort('branch')
            print(branch_dist)
            
            print("\n  - DEBUG: Sample DEPOSIT ACCTNOs with branches (first 10):")
            sample = deposit_filtered.select(['acctno', 'branch']).head(10).rows()
            for row in sample:
                print(f"    ACCTNO: {row[0]}, BRANCH: {row[1]}")
        else:
...
if float_df is not None:
    ...
    if not float_df.is_empty() and 'float' in float_df.columns and 'acctno' in float_df.columns:
        ...
        # DEBUG: Show FLOAT branch distribution
        print("\n  - DEBUG: FLOAT branch distribution:")
        float_branch_dist = float_df.group_by('branch').agg(pl.len().alias('count')).sort('branch')
        print(float_branch_dist)
        
        print("\n  - DEBUG: Sample FLOAT ACCTNOs with branches (first 10):")
        sample = float_df.select(['acctno', 'branch']).head(10).rows()
        for row in sample:
            print(f"    ACCTNO: {row[0]}, BRANCH: {row[1]}")
    else:
