...
def read_coll_binary_efficient(file_path: Path, max_rows: int = None) -> pl.DataFrame:
    ...
    if all_data:
        ...
        print(f"COLL acctno range: {df['acctno'].min()} to {df['acctno'].max()}")
        print(f"COLL sample acctno values: {df['acctno'].head(10).to_list()}")
    else:
        df = pl.DataFrame({
...
def read_desc_ebcdic_efficient(file_path: Path, max_rows: int = None) -> pl.DataFrame:
    ...
    if all_data:
        df = pl.DataFrame(all_data)
        print(f"DESC sample ccollno values: {df['ccollno'].head(10).to_list()}")
    else:
        df = pl.DataFrame({
            'ccollno': pl.Series([], dtype=pl.Int64),
...
print(f"CRFT acctno range: {crft['acctno'].min()} to {crft['acctno'].max()}")
...
print(f"CA acctno range: {ca['acctno'].min()} to {ca['acctno'].max()}")
...
print(f"LN acctno range: {ln['acctno'].min()} to {ln['acctno'].max()}")
...
if coll.height > 0 and desc.height > 0:
    ...
    print(f"COLL acctno range: {coll_acctno['acctno'].min()} to {coll_acctno['acctno'].max()}")
    print(f"COLL sample acctno: {coll_acctno['acctno'].head(20).to_list()}")
else:
...
print(f"AAA unique acctno: {aaa['acctno'].n_unique()}")
print(f"AAA acctno range: {aaa['acctno'].min()} to {aaa['acctno'].max()}")
print(f"AAA sample acctno: {aaa['acctno'].head(20).to_list()}")
...
if coll_acctno.height > 0:
    # Check overlap
    aaa_unique_acctno = aaa.select(["acctno"]).unique(subset=["acctno"], keep="first")
    overlap = aaa_unique_acctno.join(coll_acctno, on="acctno", how="inner")
    print(f"Overlap between AAA and COLL: {overlap.height} accounts")
    
    if overlap.height > 0:
        excp = aaa.join(coll_acctno, on="acctno", how="inner")
        print(f"EXCP final records: {excp.height}")
    else:
        print("No overlap found between AAA and COLL")
        print("This is likely because AAA is limited to 50,000 rows per file,")
        print("while COLL contains all accounts from the full dataset.")
        print("To get results, either:")
        print("  1. Increase MAX_ROWS_SAS to read more data")
        print("  2. Set MAX_ROWS_SAS = None to read all data")
...
if excp.height > 0:
    ...
    print("To get results, set MAX_ROWS_SAS = None to read all data")
