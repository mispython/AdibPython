...
def read_sas7bdat(filepath: Path, chunk_size: int = None) -> pl.DataFrame:
    """Read SAS dataset using pyreadstat with optional chunking for large files"""
    if chunk_size is None:
        ...
        chunks = []
        reader = pyreadstat.read_sas7bdat(str(filepath), row_limit=chunk_size)
        # Note: pyreadstat doesn't support direct chunked reading
        # We'll use a different approach for chunking
        df, meta = pyreadstat.read_sas7bdat(str(filepath))
        df.columns = [col.lower() for col in df.columns]
        return pl.from_pandas(df)


def read_sas7bdat_chunked(filepath: Path, chunk_size: int = CHUNK_SIZE, 
                          entity_filter: str = None) -> pl.DataFrame:
    """
    Read SAS dataset in chunks using pyreadstat's row_offset and row_limit
    This is more memory efficient for large files
    """
    # First, get metadata to know total rows
    _, meta = pyreadstat.read_sas7bdat(str(filepath), row_limit=1)
    total_rows = meta.row_count if hasattr(meta, 'row_count') else None
...
# SCH mapping rules (IF/ELSE logic)
