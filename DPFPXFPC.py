...
# ============================================================================
# FIX: these reads had no usecols, so they were loading EVERY column of
# what's very likely a large customer-master file (CIS = Customer
# Information System, typically hundreds of columns bank-wide) even though
# only 5 columns are ever used. This was almost certainly the actual 70-
# minute bottleneck, not the join itself. Same fix pattern as LN_NOTE:
# prune columns + read in parallel across cores.

CIS_COLS = ["ACCTNO", "SECCUST", "CUSTNAM1", "NEWIC", "OLDIC"]

CISCA = (
    read_sas_parallel(CISCA_DEP, usecols=CIS_COLS)
...
CISDP = (
    read_sas_parallel(CISDP_DEP, usecols=CIS_COLS)
    .filter(pl.col("SECCUST") == "901")
    .select(["ACCTNO", "CUSTNAM1", "NEWIC", "OLDIC"])
    .with_columns(pl.col("ACCTNO").cast(pl.Int64, strict=False))
    .unique(subset=["ACCTNO"], keep="first")
)
_tick("read+filter CIS tables (CISCA/CISDP)")
