...
def generate_text_report(deposit_merged, float_only, output_path):
    ...
    with open(report_path, 'w') as f:
        ...
        if not deposit_merged.is_empty():
            # Filter for B AND NOT A (records in FLOAT but not in DEPOSIT)
            float_only_records = deposit_merged.filter(
                pl.col('float').is_not_null() & 
                (pl.col('curbal').is_null() | pl.col('product').is_null())
            )
            
            # Sort by FLOAT amount (descending) to match SAS output
            float_only_records = float_only_records.sort('float', descending=True)
            
            # Select and format records
            obs_count = 0
            total_float = 0
            
            for row in float_only_records.rows():
                obs_count += 1
                # Extract values with defaults
                prodcd = row[float_only_records.columns.index('progcd')] if 'progcd' in float_only_records.columns else ''
                amtind = row[float_only_records.columns.index('amtind')] if 'amtind' in float_only_records.columns else ''
                branch = row[float_only_records.columns.index('branch')] if 'branch' in float_only_records.columns else ''
                acctno = row[float_only_records.columns.index('acctno')] if 'acctno' in float_only_records.columns else ''
                ledgbal = row[float_only_records.columns.index('ledgbal')] if 'ledgbal' in float_only_records.columns else 0
                product = row[float_only_records.columns.index('product')] if 'product' in float_only_records.columns else ''
                curbal = row[float_only_records.columns.index('curbal')] if 'curbal' in float_only_records.columns else 0
                intpaybl = row[float_only_records.columns.index('intpaybl')] if 'intpaybl' in float_only_records.columns else 0
                float_val = row[float_only_records.columns.index('float')] if 'float' in float_only_records.columns else 0
                avbal = row[float_only_records.columns.index('avbal')] if 'avbal' in float_only_records.columns else 0
                avbaltt = row[float_only_records.columns.index('avbaltt')] if 'avbaltt' in float_only_records.columns else 0
                curbaltt = row[float_only_records.columns.index('curbaltt')] if 'curbaltt' in float_only_records.columns else 0
                
                # Format values
                prodcd_str = str(prodcd) if prodcd is not None else ''
                amtind_str = str(amtind) if amtind is not None else ''
                branch_str = str(branch) if branch is not None else ''
                acctno_str = str(int(acctno)) if acctno is not None and not pd.isna(acctno) else ''
                ledgbal_str = f"{ledgbal:.2f}" if ledgbal is not None else ''
                product_str = str(product) if product is not None else ''
                curbal_str = f"{curbal:.2f}" if curbal is not None else ''
                intpaybl_str = f"{intpaybl:.2f}" if intpaybl is not None else ''
                float_str = f"{float_val:.2f}" if float_val is not None else ''
                avbal_str = f"{avbal:.2f}" if avbal is not None else ''
                avbaltt_str = f"{avbaltt:.2f}" if avbaltt is not None else ''
                curbaltt_str = f"{curbaltt:.2f}" if curbaltt is not None else ''
                
                # Write formatted row (matching SAS output spacing)
                ...
                total_float += float_val if float_val is not None else 0
...
def standardize_dataframe(df, columns_to_keep, keep_string_cols=None):
    ...
    # Standardize curbal, ledgbal, intpaybl to Float64 (skip amtind as it may contain strings)
...
def standardize_schema_for_concat(dfs):
    ...
    for df in dfs:
        ...
        # Ensure numeric columns are Float64 (skip amtind)
...
if fdmthly_dfs:
    ...
    # Rename columns according to SAS logic
