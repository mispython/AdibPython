...
try:
    ...
    print(f"  LOANTEMP columns: {df_loantemp.columns}")
...
try:
    ...
    print(f"  LNNOTE columns: {df_lnnote.columns}")
...
try:
    ...
    print(f"  Merged columns: {df_hploan.columns}")
...
try:
    ...
    with open(detail_file, 'w') as f:
        ...
        for row in df_detail_pd.itertuples():
            ...
            line = f"{row.ACCTNO:<12} {row.NOTENO:<8} {name_str:<30} {str(row.BRABBR):<6} {row.PRODUCT:<5} {str(row.BORSTAT):<8} {row.NETPROC:<12,.2f} {row.BALANCE:<12,.2f} {row.MTHARR:<7} {row.MARGINF:<10.1f} {row.NOTETERM:<5} {str(row.STATENM):<15} {str(row.MAKE):<15} {str(row.NEWSEC):<10} {str(row.SOURCE):<12} {str(row.SCORE2):<6} {row.ISTLPD:<8.1f} {issuedt_str:<10}\n"
            f.write(line)
    ...
    with open(detail_file, 'a') as f:
        f.write("-" * 150 + "\n")
        f.write(f"TOTAL NPL ACCOUNTS: {tot_acc:,}\n")
        f.write(f"TOTAL NPL BALANCE: {tot_amt:,.2f}\n")
        f.write("=" * 150 + "\n")
        f.write("END OF REPORT\n")
