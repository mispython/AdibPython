...
def main():
    ...
    try:
        ...
        # Check if CRA has data
        if len(cra) == 0:
            print("  Warning: CRA file is empty, creating empty DataFrame with required columns")
            cra = pl.DataFrame({
                'BRANCH': [],
                'CUSTICKETNO': [],
                'INVCURAC': [],
    ...
    if len(dp_cra) > 0:
        ...
        # Empty DataFrame with all needed columns
        dp_cra = pl.DataFrame({
            'BRANCH': [],
            'CUSTICKETNO': [],
            'INVCURAC': [],
    ...
    # If eqdci is empty after all joins, create minimal structure
    if len(eqdci) == 0:
        print("  Warning: No data in EQDCI after joins, creating empty structure")
        eqdci = pl.DataFrame({
            'TICKETNO': [],
            'CUSTCODE': [],
            'INVCURR': [],
    ...
    # Round based on currency (only if data exists)
    if len(eqdci) > 0:
    ...
    if len(cusmyr) > 0 and len(ibnmyr) > 0:
        ...
        dcimyr = pl.DataFrame()
        print("  Warning: No data available to build DCI")
