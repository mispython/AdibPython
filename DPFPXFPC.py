...
if len(dcid) > 0:
    ...
    dcid2.write_csv(csv_file)
    print(f"✓ Saved CSV: {csv_file}")
    
    # Save as SAS7BDAT using saspy (with better error handling)
    try:
        import saspy
        import pandas as pd
        ...
        result = sas.submit(f"""
            LIBNAME DCI '/sas/python/virt_edw/Data_Warehouse/MIS/XMIS/output/EIBDDCIA';
        ...
        # Save to TEMP library
        print("Saving to TEMP library...")
        result = sas.submit(f"""
            LIBNAME TEMP '/sas/python/virt_edw/Data_Warehouse/MIS/XMIS/output/EIBDDCIA/temp';
        ...
        print("⚠ saspy not available. Only Parquet and CSV files created.")
    except Exception as e:
        print(f"⚠ Error saving SAS file: {e}")
        print("Only Parquet and CSV files created.")
