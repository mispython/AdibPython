...
def main():
    ...
    if loan_base.height == 0:
        ...
        print("The current month's file does not contain the required loan type.")
    ...
    # STEP 2: Read LNCOMM and calculate NETPROC
    print("\n" + "="*60)
    ...
    print(f"LNCOMM rows: {lncomm.height}")
    ...
    if lncomm.height > 0:
        ...
        # Handle INTAMT or use NETPROC directly
        if "NETPROC" in lncomm.columns:
            print("Using existing NETPROC from LNCOMM")
        elif "INTAMT" in lncomm.columns:
            lncomm = lncomm.with_columns([
            ...
            lncomm = lncomm.with_columns(pl.col("CORGAMT_CLEAN").alias("NETPROC"))
        ...
        comm = lncomm.select(select_cols) if len(select_cols) > 1 else pl.DataFrame()
        if comm.height > 0 and "ACCTNO" in comm.columns and "COMMNO" in comm.columns:
    ...
    # STEP 3: Merge LOAN1 with COMM
    if loan1.height > 0 and comm.height > 0:
        print("\n" + "="*60)
        print("STEP 3: Merging LOAN1 with COMM")
        print("="*60)
        
        if "COMMNO" in loan1.columns and "COMMNO" in comm.columns:
        ...
        loan = pl.concat([loan0, loan1], how="vertical", rechunk=True) if loan1.height > 0 else loan0
        del loan0, loan1
        gc.collect()
    
    # Continue with remaining processing steps...
    print("\n" + "="*60)
    print("Data found. Processing would continue here...")
    print("="*60)
    
    # Read COLL file
    print("\nReading COLL file (EBCDIC)...")
    coll = read_fixed_length_ebcdic(COLL_FILE, COLL_RECORD_LENGTH, parse_coll_record)
    
    # Read DESC file
    print("\nReading DESC file (EBCDIC)...")
    desc = read_fixed_length_ebcdic(DESC_FILE, DESC_RECORD_LENGTH, parse_desc_record)
    ...
    if loan.height > 0 and coll.height > 0:
        ...
        print(f"\nNPGS rows: {npgs.height}")
        
        if npgs.height > 0:
            print("SUCCESS: NPGS data created!")
            # Continue with CVAR fields, MICR, etc.
        else:
            print("No matching records between LOAN and COLL")
