...
def read_lnnote_filtered(path, usecols, hp_products, chunk_size,
    ...
    # read_file_in_chunks returns a single generator that yields
    # (chunk_df, chunk_meta) tuples - it does NOT return (iterator, meta) itself.
...
try:
    ...
    # --- Diagnostics: confirm join keys actually line up before merging ---
    print(f"  ACCTNO dtype  -> lnnote: {df_lnnote['ACCTNO'].dtype}, loantemp: {df_loantemp['ACCTNO'].dtype}")
    print(f"  NOTENO dtype  -> lnnote: {df_lnnote['NOTENO'].dtype}, loantemp: {df_loantemp['NOTENO'].dtype}")
    print(f"  ACCTNO sample -> lnnote: {df_lnnote['ACCTNO'].head(3).to_list()}, "
          f"loantemp: {df_loantemp['ACCTNO'].head(3).to_list()}")
    print(f"  NOTENO sample -> lnnote: {df_lnnote['NOTENO'].head(3).to_list()}, "
          f"loantemp: {df_loantemp['NOTENO'].head(3).to_list()}")

    # Force both sides onto an identical, unambiguous key type. SAS numerics
    # read through pyreadstat land as float64; chunked vs non-chunked reads
    # can occasionally differ in dtype/precision, which silently produces
    # ZERO join matches instead of an error. Round-tripping through a
    # rounded Int64 makes the keys directly comparable on both sides.
    df_lnnote = df_lnnote.with_columns([
        pl.col('ACCTNO').cast(pl.Float64).round(0).cast(pl.Int64).alias('ACCTNO'),
        pl.col('NOTENO').cast(pl.Float64).round(0).cast(pl.Int64).alias('NOTENO'),
    ])
    ...
    if len(df_hploan) == 0:
        # Extra diagnostics: is this a NOTENO alignment issue, or do the
        # two files simply not share any ACCTNO universe at all?
        acct_lnnote = set(df_lnnote['ACCTNO'].to_list())
        acct_loantemp = set(df_loantemp['ACCTNO'].to_list())
        acct_overlap = acct_lnnote & acct_loantemp
        print(f"  [diag] lnnote ACCTNO range:   min={min(acct_lnnote):.0f}, max={max(acct_lnnote):.0f}, unique={len(acct_lnnote):,}")
        print(f"  [diag] loantemp ACCTNO range: min={min(acct_loantemp):.0f}, max={max(acct_loantemp):.0f}, unique={len(acct_loantemp):,}")
        print(f"  [diag] ACCTNO overlap (ignoring NOTENO): {len(acct_overlap):,} accounts in common")
        if len(acct_overlap) == 0:
            print("  [diag] ZERO ACCTNO overlap -> the two files do not share any account "
                  "keys at all. This points to the files being from different sources, "
                  "snapshot dates, or environments (e.g. test/dummy data), rather than a "
                  "code/dtype problem. Verify LOAN_DIR and CCDTEMP_DIR point at the correct, "
                  "matching production extracts for this report date.")
        else:
            print(f"  [diag] Some ACCTNO overlap exists but NOTENO combinations don't align -> "
                  f"check whether NOTENO needs its own type/format reconciliation.")
...
df_hploan = df_hploan.with_columns([
    ...
    # SAS logic: zero-pad ISSUEDT to 11 digits, take the FIRST 8 characters.
    # Because of left-padding, this is equivalent to: drop the LAST 3 digits
    # of the number, then left-pad the remainder to 8 digits, then parse
    # as MMDDYYYY. (Confirmed against real sample values: 8052015217 -> drop
    # last 3 -> 8052015 -> pad -> "08052015" -> 05 Aug 2015, a valid date.)
    # Earlier version incorrectly sliced the first 8 chars of the plain
    # string instead of dropping the trailing 3 digits - that was the bug.
    (pl.col('ISSUEDT').cast(pl.Float64).round(0).cast(pl.Int64) // 1000)
      .cast(pl.Utf8).str.zfill(8)
...
_unparsed_dates = df_hploan.filter(pl.col('ISSDTE').is_null()).height
if _unparsed_dates > 0:
    _sample_raw = (
        df_hploan.filter(pl.col('ISSDTE').is_null())
        .select(pl.col('ISSUEDT').cast(pl.Utf8))
        .head(5)['ISSUEDT'].to_list()
    )
    print(f"  WARNING: {_unparsed_dates:,} of {len(df_hploan):,} rows had an ISSUEDT value "
          f"that didn't parse as MMDDYYYY (ISSUE DATE will be blank for these). "
          f"Sample raw ISSUEDT values: {_sample_raw}")
    print("  If this dataset encodes dates differently (e.g. this Islamic/Aitab "
          "extract uses a different layout than the conventional file), the date "
          "format string on the ISSDTE line may need adjusting for this source.")

# Categorize fields (matches SAS logic)
# NOTE: split into two with_columns() calls. Polars evaluates every
# expression in a single with_columns([...]) against the ORIGINAL frame,
# not sequentially - so NATIONAL (which depends on STATENM) and CARS/GOODS
# (which depend on MAKE) must go in a second call, after STATENM and MAKE
# actually exist in the frame. Doing them all in one call previously caused
# ColumnNotFoundError on STATENM.
df_hploan = df_hploan.with_columns([
...
# Second pass: derivations that depend on columns created above
df_hploan = df_hploan.with_columns([
    # NATIONAL - East/West Malaysia (matches SAS IF/ELSE) - depends on STATENM
    pl.when(pl.col('STATENM').is_in(['SABAH', 'SARAWAK', 'LABUAN']))
      .then(pl.lit('EAST MALAYSIA'))
      .otherwise(pl.lit('WEST MALAYSIA'))
      .alias('NATIONAL'),

    # CARS - National/Non-National (matches SAS IF/ELSE) - depends on MAKE
    pl.when(pl.col('MAKE').is_in(['PROTON', 'PERODUA']))
      .then(pl.lit('NATIONAL'))
      .otherwise(pl.lit('NON NATIONAL'))
      .alias('CARS'),

    # GOODS - Schedule/Unschedule (matches SAS IF/ELSE) - depends on MAKE
