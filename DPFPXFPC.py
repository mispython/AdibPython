    ...
    try:
        ...
        warn_missing_columns(
            ...
            ['gwmvt', 'gwccy', 'gwocy', 'gwmpts', 'gwctp', 'gwdlp', 'gwmdt',
             'gwsdt', 'gwbalc', 'gwhsn', 'gwc2r', 'gwdlr'],
            'K1TBL'
        )
...
def process_k3tbl(rep_date):
    ...
    try:
        ...
        warn_missing_columns(
            df,
            ['utref', 'utsty', 'utdlp', 'utcus', 'utclc', 'utctp', 'utmat',
...
def process_dci(rep_date):
    ...
    try:
        ...
        for row in df.iter_rows(named=True):
            matdt = row.get(matdt_col)
            startdt = row.get(startdt_col)
...
def process_cis_equity():
    ...
    try:
        ...
        warn_missing_columns(df, ['acctcode', 'prisec', 'newic', 'aliaskey',
        ...
        # cast defensively in case prisec is stored as float/str
        df = df.with_columns(pl.col('prisec').cast(pl.Utf8).str.strip_chars())
    ...
    try:
        for tbl in ['fd', 'sa', 'ca', 'fcyca']:
            ...
            for filepath in files:
                ...
                warn_missing_columns(
                    df,
                    ['bnmcode', 'amount', 'curcode', 'custcd', 'custcdx', 'acctno',
    ...
    try:
        for deptype in ['cisdp', 'cisca']:
            ...
            for filepath in files:
                ...
                if df is not None:
                    df = df.filter(pl.col('seccust').cast(pl.Utf8).str.strip_chars() == '901')
