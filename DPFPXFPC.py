...
if df_loan.height > 0:
    ...
    # ECSRRSRV - using .map_elements with a function to avoid column "Y" issue
    try:
...
# ============================================================================
# STEP 9: Read customer names - try multiple possible file names
# ============================================================================
print("STEP 9: Reading customer names...")

# Try multiple possible file names and locations for CISNAME.LOAN
cname_patterns = [
    f'{CISNAME_DIR}loan.sas7bdat',
    f'{CISNAME_DIR}LOAN.sas7bdat',
    f'{CISNAME_DIR}cisloan.sas7bdat',
    f'{CISNAME_DIR}CISLOAN.sas7bdat',
    f'{LOAN_DIR}loan.sas7bdat',
    f'{LOAN_DIR}LOAN.sas7bdat',
]
...
for pattern in cname_patterns:
    ...
    if df_cname is not None:
        print(f"  Found customer name file: {os.path.basename(pattern)}")
        break

if df_cname is not None:
    # Check if SECCUST column exists, if not, try CUSTTYPE or similar
    if 'SECCUST' not in df_cname.columns:
        # Try to find a column that might contain customer type
        for col in ['CUSTTYPE', 'CUST_TYPE', 'CUSTCD', 'CUST_CD']:
            if col in df_cname.columns:
                df_cname = df_cname.filter(pl.col(col) == '901')
                break
        else:
            # If no customer type column, keep all rows
            print("  Warning: No SECCUST column found, keeping all customer records")
    else:
        df_cname = df_cname.filter(pl.col('SECCUST') == '901')
    
    df_cname = df_cname.select(['ACCTNO', 'CUSTNAM1', 'OCCUPAT', 'BGC']).unique(subset=['ACCTNO'])
else:
    print("  Warning: Could not find customer name file")
...
if df_woff.height > 0:
    ...
    # Convert types
    for col in ['BORSTAT', 'PAIDIND']:
        if col in df_woff.columns:
            df_woff = df_woff.with_columns(pl.col(col).cast(pl.Utf8))
    
    for col in ['DAYS', 'LOANTYPE', 'TOTAL']:
        if col in df_woff.columns:
