...
def write_sas_dataset(df: pl.DataFrame, file_path: Path):
    ...
    # Handle datetime columns - convert to proper datetime64
...
def process_pa_data(input_dir: Path, output_dir: Path, reptyear4: int):
    ...
    # Parse dates and create new columns
    df = df.with_columns([
    ...
    # Product dataset - select only the columns that exist
...
def merge_all_data(output_dir: Path):
    ...
    if (output_dir / "pacust.sas7bdat").exists():
        pacust_df, _ = pyreadstat.read_sas7bdat(str(output_dir / "pacust.sas7bdat"))
        pacust = pl.from_pandas(pacust_df)
    
    if (output_dir / "motorcust.sas7bdat").exists():
        motorcust_df, _ = pyreadstat.read_sas7bdat(str(output_dir / "motorcust.sas7bdat"))
        motorcust = pl.from_pandas(motorcust_df)
    
    if (output_dir / "firecust.sas7bdat").exists():
        firecust_df, _ = pyreadstat.read_sas7bdat(str(output_dir / "firecust.sas7bdat"))
        firecust = pl.from_pandas(firecust_df)
    
    if (output_dir / "hirecust.sas7bdat").exists():
        hirecust_df, _ = pyreadstat.read_sas7bdat(str(output_dir / "hirecust.sas7bdat"))
        hirecust = pl.from_pandas(hirecust_df)
    
    if (output_dir / "nonhirecust.sas7bdat").exists():
        nonhirecust_df, _ = pyreadstat.read_sas7bdat(str(output_dir / "nonhirecust.sas7bdat"))
        nonhirecust = pl.from_pandas(nonhirecust_df)
    ...
    if (output_dir / "paprod.sas7bdat").exists():
        paprod_df, _ = pyreadstat.read_sas7bdat(str(output_dir / "paprod.sas7bdat"))
        paprod = pl.from_pandas(paprod_df)
    
    if (output_dir / "motorprod.sas7bdat").exists():
        motorprod_df, _ = pyreadstat.read_sas7bdat(str(output_dir / "motorprod.sas7bdat"))
        motorprod = pl.from_pandas(motorprod_df)
    
    if (output_dir / "fireprod.sas7bdat").exists():
        fireprod_df, _ = pyreadstat.read_sas7bdat(str(output_dir / "fireprod.sas7bdat"))
        fireprod = pl.from_pandas(fireprod_df)
