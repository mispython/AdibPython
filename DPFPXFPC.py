...
if not deposit_filtered.is_empty() and not float_summary.is_empty():
    ...
    # First, find which FLOAT records match DEPOSIT on last 6 digits
    # But we need to avoid duplicates - each DEPOSIT should match to the FIRST FLOAT record found
    # or we need to aggregate FLOAT by the last 6 digits
    ...
    print(f"    FLOAT aggregated: {float_aggregated.height} unique keys")
    
    # Merge DEPOSIT with aggregated FLOAT
    ...
    if float_not_null.height > 0:
        ...
        for row in float_not_null.select(['acctno', 'acctno_original', 'float']).head(10).rows():
        ...
        # Since we're doing a left join, B AND NOT A would be FLOAT records not in DEPOSIT
        # But we already aggregated FLOAT, so we need to find FLOAT keys not in DEPOSIT
        deposit_keys = set(deposit_with_key['acctno_key'].unique().to_list())
        float_keys = set(float_aggregated['acctno_key'].unique().to_list())
        float_only_keys = float_keys - deposit_keys
        
        print(f"\n  - FLOAT keys not in DEPOSIT: {len(float_only_keys)}")
        
        # Create float_only records from aggregated FLOAT
        if float_only_keys:
            float_only = float_aggregated.filter(
                pl.col('acctno_key').is_in(list(float_only_keys))
            ).with_columns([
                pl.lit(None).cast(pl.Float64).alias('curbal'),
        ...
        if not float_only.is_empty():
            ...
            for row in float_only.select(['acctno_original', 'float']).head(10).rows():
        ...
        print("  Please check the FLOAT file for the correct period.")
