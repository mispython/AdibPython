...
def format_date_sas(val):
    """Format SAS date to DD/MM/YYYY"""
    if val is None or (isinstance(val, float) and val != val):  # Check for NaN
        return ""
...
def eibsnpgs():
    ...
    # Define required columns for processing (includes branch and filter columns)
    required_cols = ['cvar01','cvar02','cvar03','cvar04','cvar05','cvar06',
                    'cvar07','cvar08','cvar09','cvar10','cvar11','cvar12',
                    'cvar13','cvar14','cvar15','cvarxx','natguar','cinstcl','branch']
    ...
    # Define final output columns for COMBT.txt (WITHOUT cvarxx and branch)
    final_cols = ['cvar01','cvar02','cvar03','cvar04','cvar05','cvar06',
                 'cvar07','cvar08','cvar09','cvar10','cvar11','cvar12',
                 'cvar13','cvar14','cvar15']
    
    # Define columns for report (WITH cvarxx and branch)
    report_cols = final_cols + ['cvarxx', 'branch']
    ...
    # Output columns for COMBT.txt (final_cols + lastcol, WITHOUT cvarxx and branch)
    ...
    if 'cvar02' in npgs_df.columns:
        counts = npgs_df.group_by("cvar02").agg(pl.len().alias("records"))
        print("\nRecords by CVAR02:")
