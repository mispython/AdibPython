...
df_list = read_sas7bdat(f'{NPL_DIR}LIST.sas7bdat', columns=['ACCTNO'])
if df_list is not None:
    ...
    df_list = create_empty_dataframe_with_schema({'ACCTNO': pl.Float64})
...
if df_woff.height > 0:
    # Save to NPL.LIST2 (or WOFFTXT)
    write_sas7bdat(df_woff, f'{NPL_DIR}WOFFTXT.sas7bdat')
    ...
    if text_records:
        ...
        write_sas7bdat(df_text, f'{NPL_DIR}WOFFTXT_FORMATTED.sas7bdat')
...
if df_woff.height > 0:
    ...
    print(f"  {NPL_DIR}WOFFTXT_FORMATTED.sas7bdat (Formatted dataset)")
