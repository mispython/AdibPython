...
def write_summary_to_text(df_pivot, group_cols, title, subtitle, filename):
    ...
    with open(filename, 'w') as f:
        ...
        total_count = df_pivot.select(pl.sum_horizontal(count_cols)).item() if count_cols else 0
        total_amount = df_pivot.select(pl.sum_horizontal(amount_cols)).item() if amount_cols else 0
