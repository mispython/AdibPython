...
VOSTRO_PATH = INPUT_DIR / "VOSTRO.parquet"
CISINFO_PATH = INPUT_DIR / "CISINFO.parquet"

# List files
INTRA_GROUP_PATH = LIST_DIR / "COF_MNI_INTRA_GROUP.parquet"
RELATED_PARTY_PATH = LIST_DIR / "COF_MNI_RELATED_PARTY.parquet"
EQU_INTRA_GROUP_PATH = LIST_DIR / "COF_EQU_INTRA_GROUP.parquet"
EQU_RELATED_PARTY_PATH = LIST_DIR / "COF_EQU_RELATED_PARTY.parquet"
...
def parse_amount_with_sign(amount_str, sign_str):
    """Parse amount considering sign column (space = negative)."""
    try:
...
def create_cof23(reptmon, lists):
    ...
    # Add CUSTEQNO column to CMM records (fill with None)
...
def process_vostro(lists):
    ...
    df_vostro = pl.read_parquet(VOSTRO_PATH)
    df_cisinfo = pl.read_parquet(CISINFO_PATH)
...
if __name__ == "__main__":
  ...
  all inputs are in sas7bdat sas dataset. except for walk.txt and templ.txt. use pyreadstat to read. all inputs are in lowercase! remove reptdate, use datetime timedelta - 1 instead. output in text files. 
