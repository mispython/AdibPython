...
try:
    ...
    # The first record may contain TBDATE (report date) in a specific field
    # Assuming there's a TBDATE or REPTDATE column in the Parquet file
    if 'TBDATE' in df_dptrbl.columns:
        tbdate_val = df_dptrbl.select('TBDATE').first().item()
        reptdate = pd.to_datetime(tbdate_val).date() if hasattr(tbdate_val, 'strftime') else tbdate_val
    elif 'REPTDATE' in df_dptrbl.columns:
        reptdate_val = df_dptrbl.select('REPTDATE').first().item()
        reptdate = reptdate_val if hasattr(reptdate_val, 'date') else datetime.strptime(str(reptdate_val), '%Y%m%d').date()
    else:
        # If no date column, use current date or last date in data
        print("  Warning: No TBDATE/REPTDATE found, using system date")
...
df_with_range = df_filtered.with_columns([
    pl.when(
...
branch_totals = {
    'TOTSAVG': df_savings.filter(pl.col('PRODCD') == '42120')['CURBAL'].sum(),
    'TOTSAVGI': df_savings.filter(pl.col('PRODCD') == '42320')['CURBAL'].sum(),
    'TOTDMND': df_demand.filter(pl.col('PRODCD') == '42110')['CURBAL'].sum(),
    'TOTDMNDI': df_demand.filter(pl.col('PRODCD') == '42310')['CURBAL'].sum(),
    ...
    'TOTOVFT': df_overdrafts['CURBAL'].abs().sum(),
    'TOTFD': df_fixed.filter(pl.col('PRODCD') == '42130')['CURBAL'].sum(),
    'TOTFDI': df_fixed.filter(pl.col('PRODCD') == '42132')['CURBAL'].sum(),
    'ACESA': df_demand.filter(pl.col('PRODUCT').is_in(ACE_PRODUCTS))['CURBAL'].sum(),
    'ACECA': df_demand.filter(pl.col('PRODUCT').is_in(ACE_PRODUCTS))['CURBAL'].sum(),
    'TOTMBSA': df_savings.filter(pl.col('PRODUCT') == 214)['CURBAL'].sum()
}
