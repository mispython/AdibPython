...
def read_sas(filepath, cols=None):
    ...
    try:
        ...
        # Convert column names to lowercase (all columns, not just selected)
        df = df.rename({c: c.lower() for c in df.columns})
        
        # Debug: Show available columns
        print(f"      Available columns: {df.columns[:10]}{'...' if len(df.columns) > 10 else ''}")
        
        # Select only needed columns (now using lowercase)
        if cols:
            # Convert input cols to lowercase for matching
            ...
            if available_cols:
                ...
                print(f"      WARNING: No matching columns found in {filepath.name}")
                print(f"      Expected: {cols_lower}")
                return pl.DataFrame()
...
# Load all source datasets with lowercase column names
print("  Loading MNI SAVG124...")
savg = read_sas(mni_path / "savg124.sas7bdat", ['acctno','product','curbal','ledgbal','progcd','amtind','intpaybl','branch'])

print("  Loading IMNI SAVG124...")
isavg = read_sas(imni_path / "savg124.sas7bdat", ['acctno','product','curbal','ledgbal','progcd','amtind','intpaybl','branch'])

print("  Loading MNI CURN124...")
curn = read_sas(mni_path / "curn124.sas7bdat", ['acctno','product','curbal','ledgbal','progcd','amtind','intpaybl','branch'])

print("  Loading IMNI CURN124...")
icurn = read_sas(imni_path / "curn124.sas7bdat", ['acctno','product','curbal','ledgbal','progcd','amtind','intpaybl','branch'])
...
for df in [savg, isavg, curn, icurn, fdm, ifdm]:
    if not df.is_empty():
        ...
        if not standardized.is_empty():
            ...
            print(f"  Added dataset: {standardized.height:,} records")
...
deposit.write_parquet(output_path / "DEPOSIT_COMBINED.parquet")
...
if not float_df.is_empty():
    # Convert columns (columns are already lowercase)
