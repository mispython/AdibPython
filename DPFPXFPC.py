...
# Input files (macro vars substituted at runtime for the FEE/ACCT files)
...
# SAS: INPUT @01 EXTDATE 11.;  -> first 11 chars
EXTDATE     = int(first_line[0:11].strip())
extdate_str = str(EXTDATE).zfill(11)[:8]                # YYMMDDXX -> YYMMDD

# SAS MMDDYY8. with YEARCUTOFF=1950 -> parse as %m%d%y
...
#   @075 FEEAMTB   PD8.2
#
# NOTE: Assumes the file is ASCII fixed-width (not EBCDIC / COMP-3).
#       If the diagnostic script showed EBCDIC or COMP-3 bytes, replace this
#       block with an appropriate decoder.
# ===========================================================================
con.execute(f"""
...
con.execute(f"""
    ...
    SELECT
        ...
        -- BRANCH = COALESCE(nonzero PENDBRH, nonzero NTBRCH, ACCBRCH)
        CASE
