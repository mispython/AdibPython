...
# Check REINPROD distribution
print("\nREINPROD distribution after REPAID filter:")
reinprod_counts = rlslist.group_by('REINPROD').agg(pl.len().alias('count')).sort('count', descending=True)
for row in reinprod_counts.iter_rows(named=True):
    print(f"  REINPROD='{row['REINPROD']}': {row['count']} records")

# Filter based on REINPROD - exclude empty values
# This likely matches production logic where only loans with REINPROD populated are included
rlslist = rlslist.filter(pl.col('REINPROD').is_not_null())
rlslist = rlslist.filter(pl.col('REINPROD').cast(pl.Utf8).str.strip_chars() != '')
print(f"\nRecords after filtering empty REINPROD: {len(rlslist)}")
