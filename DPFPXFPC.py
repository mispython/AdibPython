...
def eibrsmez():
    ...
    # Add missing columns with appropriate types to each DataFrame
    for col in all_columns:
        if col not in ln_df.columns:
            # Determine the type from the other DataFrame
            if col in dp_df.columns:
                col_type = dp_df.schema[col]
                # Create a column of the same type with null values
                if col_type == pl.Utf8:
                    ln_df = ln_df.with_columns(pl.lit(None).cast(pl.Utf8).alias(col))
                elif col_type == pl.Float64:
                    ln_df = ln_df.with_columns(pl.lit(None).cast(pl.Float64).alias(col))
                elif col_type == pl.Int64:
                    ln_df = ln_df.with_columns(pl.lit(None).cast(pl.Int64).alias(col))
                else:
                    ln_df = ln_df.with_columns(pl.lit(None).cast(col_type).alias(col))
            else:
        ...
        if col not in dp_df.columns:
            # Determine the type from the other DataFrame
            if col in ln_df.columns:
                col_type = ln_df.schema[col]
                # Create a column of the same type with null values
                if col_type == pl.Utf8:
                    dp_df = dp_df.with_columns(pl.lit(None).cast(pl.Utf8).alias(col))
                elif col_type == pl.Float64:
                    dp_df = dp_df.with_columns(pl.lit(None).cast(pl.Float64).alias(col))
                elif col_type == pl.Int64:
                    dp_df = dp_df.with_columns(pl.lit(None).cast(pl.Int64).alias(col))
                else:
                    dp_df = dp_df.with_columns(pl.lit(None).cast(col_type).alias(col))
            else:
                # Default to String type if not found
                dp_df = dp_df.with_columns(pl.lit(None).cast(pl.Utf8).alias(col))
    ...
    npgs5_df = npgs5_df.with_columns(
        ...
        # Convert SAS date to DDMMYY10 format (DD/MM/YYYY)
        pl.when((pl.col("cvar05") != 0) & (pl.col("cvar05").is_not_null()))
        .then(
            pl.col("cvar05")
            .cast(pl.Int64)
            .map_elements(
                lambda x: (datetime(1960, 1, 1) + timedelta(days=x)).strftime("%d/%m/%Y"),
        ...
        pl.when(pl.col("cvar17").is_null())
        .then(pl.lit(0.0))
        .otherwise(pl.col("cvar17"))
        .alias("cvar17"),
        
        pl.when(pl.col("accrual").is_null())
        .then(pl.lit(0.0))
        .otherwise(pl.col("accrual"))
        .alias("accrualx")
    )
