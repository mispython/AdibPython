...
if loan_base.height > 0 and "COMMNO" in loan_base.columns:
    ...
    print("Checking first 100 rows for LOANTYPE and CENSUS values...")
    ...
    if sample.height > 0:
        if "LOANTYPE" in sample.columns:
            print(f"LOANTYPE values: {sample['LOANTYPE'].unique().to_list()[:10]}")
        if "CENSUS" in sample.columns:
            print(f"CENSUS values: {sample['CENSUS'].unique().to_list()[:10]}")
        if "ENTITY_CD" in sample.columns:
            print(f"ENTITY_CD values: {sample['ENTITY_CD'].unique().to_list()[:10]}")
