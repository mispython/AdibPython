...
# For testing - set limits per dataset
# Set to None for full production run
TEST_LIMITS = {
    "lnnote": None,  # Full run - no limit
    "sasb": 5000,    # Limit to 5000 rows
    "customer": 5000, # Limit to 5000 rows
    "collateral": 5000, # Limit to 5000 rows
}
...
def eibaabba():
    ...
    print(f"*** TEST MODE - Dataset limits:")
    for dataset, limit in test_limits.items():
        if limit is None:
            print(f"    {dataset}: FULL RUN (no limit)")
        else:
            print(f"    {dataset}: {limit:,} rows")
    print()
    ...
    # Process data - LNNOTE with full run (no limit)
    print("Reading LNNOTE data (FULL RUN - no limit)...")
    abba_df = process_abba_data(input_path, snapshot_date, test_limits["lnnote"])
    if abba_df.is_empty():
    ...
    # SASB data with limit
    print(f"Merging with SASB loan data (limit: {test_limits['sasb']:,} rows)...")
    abba_df = merge_sasb_data(abba_df, input_path, context["month"], context["week"], snapshot_date, test_limits["sasb"])
    print(f"After SASB merge: {len(abba_df):,} records")

    # Customer data with limit
    print(f"Merging with customer data (limit: {test_limits['customer']:,} rows)...")
    abba_df = merge_customer_data(abba_df, input_path, test_limits["customer"])
    print(f"After customer merge: {len(abba_df):,} records")

    # Collateral data with limit
    print(f"Merging with collateral data (limit: {test_limits['collateral']:,} rows)...")
    abba_df = merge_collateral_data(abba_df, input_path, test_limits["collateral"])
