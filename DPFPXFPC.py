...
with open(DESC_FILE, 'r', encoding='cp037', errors='ignore') as f:
    ...
    for i, line in enumerate(f):
        ...
        line = line.rstrip('\n\r')
        if '06' in line[:300]:
            lines_with_06.append((i, line))
        if '18' in line[:300]:
            lines_with_18.append((i, line))
        
        if len(lines_with_06) >= 3 and len(lines_with_18) >= 3:
            break

print(f"\nLines with '06' in first 300 chars: {len(lines_with_06)}")
for i, line in lines_with_06[:3]:
    pos = line.find('06')
    print(f"  Line {i}: '06' at pos {pos+1}")
    print(f"    Context: ...{line[max(0,pos-10):pos+15]}...")
    print(f"    CCOLLNO: '{line[0:11].strip()}'")
    print(f"    Pos 51-52: '{line[50:52]}'")
    print(f"    Pos 55-56: '{line[54:56]}'")

print(f"\nLines with '18' in first 300 chars: {len(lines_with_18)}")
for i, line in lines_with_18[:3]:
    pos = line.find('18')
    print(f"  Line {i}: '18' at pos {pos+1}")
    print(f"    Context: ...{line[max(0,pos-10):pos+15]}...")
    print(f"    CCOLLNO: '{line[0:11].strip()}'")
    print(f"    Pos 51-52: '{line[50:52]}'")
    print(f"    Pos 55-56: '{line[54:56]}'")
...
# Read DESC as line-delimited text
...
with open(DESC_FILE, 'r', encoding='cp037', errors='ignore') as f:
    for line in f:
        ...
        if line.strip():
            ...
            try:
                row['ccollno'] = float(ccollno_str) if ccollno_str else None
            except:
                row['ccollno'] = None
            
            cinstcl_str = line[50:52].strip() if len(line) >= 52 else ""
            row['cinstcl'] = cinstcl_str
            
            natguar_str = line[54:56].strip() if len(line) >= 56 else ""
            row['natguar'] = natguar_str
            
            census_str = line[210:220].strip() if len(line) >= 220 else ""
            try:
                row['census'] = float(census_str) if census_str else None
            except:
                row['census'] = None
            
            tranche_str = line[290:298].strip() if len(line) >= 298 else ""
            row['tranche'] = tranche_str
