...
try:
    import PBBDPFMT
    print(f"PBBDPFMT module loaded successfully")
    print(f"Available functions: {[f for f in dir(PBBDPFMT) if not f.startswith('_') and callable(getattr(PBBDPFMT, f))]}")
except ImportError as e:
    print(f"PBBDPFMT module not found: {e}")
    PBBDPFMT = None
...
if PBBDPFMT is not None:
    try:
        # Try to get data from PBBDPFMT module
        # The module appears to have various format functions, not data retrieval functions
        
        # Check if there's a function that returns data
        data_functions = ['get_data', 'get_pbbdpfmt', 'process_pbbdpfmt', 'main']
        found_func = None
        
        for func_name in data_functions:
            if hasattr(PBBDPFMT, func_name):
                found_func = func_name
                break
        
        if found_func:
            print(f"Calling PBBDPFMT.{found_func}()...")
            result = getattr(PBBDPFMT, found_func)()
            
            if isinstance(result, pl.DataFrame):
                pbbdpfmt_df = result
            elif hasattr(result, 'to_pandas'):
                pbbdpfmt_df = pl.from_pandas(result.to_pandas())
            else:
                print(f"Result type: {type(result)}")
                pbbdpfmt_df = None
            
            if pbbdpfmt_df is not None and len(pbbdpfmt_df) > 0:
                ...
                if "BIC" in pbbdpfmt_df.columns:
                    ...
                    if len(pbbdpfmt_filtered) > 0:
                        ...
                        print("No PBBDPFMT records found for specified BIC prefixes")
                else:
                    print("BIC column not found in PBBDPFMT data")
                    print(f"Available columns: {pbbdpfmt_df.columns}")
            else:
                print("PBBDPFMT returned no data or empty DataFrame")
        else:
            print("PBBDPFMT module contains format functions but no data retrieval functions")
            print("Available functions:", [f for f in dir(PBBDPFMT) if not f.startswith('_') and callable(getattr(PBBDPFMT, f))])
            
            # Check if the module might have data variables
            data_vars = [v for v in dir(PBBDPFMT) if not v.startswith('_') and not callable(getattr(PBBDPFMT, v))]
            if data_vars:
                print(f"Available data variables: {data_vars}")
                
                # Try to get data from variables
                for var in data_vars:
                    value = getattr(PBBDPFMT, var)
                    if isinstance(value, (pl.DataFrame, pd.DataFrame)):
                        print(f"Found DataFrame in variable: {var}")
...
def generate_report(data, bic_prefix, title, report_type="special"):
    ...
    if len(report_data) == 0:
        ...
        return None, 0.0  # Return None and 0 instead of just None
...
with open(OUTPUT_PATH / f"PROCESSING_SUMMARY_{REPTMON}_{NOWK}_{REPTYEAR}.txt", 'w') as f:
    ...
    f.write("=" * 60 + "\n")
    f.write(f"42130 Special Report: Total = {total_42130:>20,.2f}\n" if total_42130 > 0 else "42130 Special Report: No data\n")
    f.write(f"42132 Special Report: Total = {total_42132:>20,.2f}\n" if total_42132 > 0 else "42132 Special Report: No data\n")
    f.write(f"42630 FCY FD Report: Total = {total_42630:>20,.2f}\n" if total_42630 > 0 else "42630 FCY FD Report: No data\n")
...
print("=" * 60)
print(f"42130 Special Report: Total = {total_42130:>20,.2f}" if total_42130 > 0 else "42130 Special Report: No data")
print(f"42132 Special Report: Total = {total_42132:>20,.2f}" if total_42132 > 0 else "42132 Special Report: No data")
print(f"42630 FCY FD Report: Total = {total_42630:>20,.2f}" if total_42630 > 0 else "42630 FCY FD Report: No data")
