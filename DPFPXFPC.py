...
MICR_FILE = Path("/sas/python/virt_edw/Data_Warehouse/MIS/XMIS/input/prod/EIBTNPGS/BOPESS.txt")  # Uppercase BOPESS.txt
...
# Read text file without parsing - just get the raw lines
...
# Based on the error, the data appears to be in a different format
# Let's print first few lines to understand the format
print("First few lines of crftabl.txt:")
for i, line in enumerate(lines[:5]):
    print(f"Line {i}: '{line.rstrip()}'")

# Since we see values like "SGXX" and "SGCX", this might be space or tab delimited
# Let's try splitting by whitespace or fixed positions based on actual format
# For now, let's try to parse it as space-delimited
crft_data = crft_data.with_columns([
    pl.col('data').str.split(' ').alias('fields')
])

# Extract fields - adjust indices based on actual data structure
crft_data = crft_data.with_columns([
    pl.col('fields').list.get(0).alias('RECTYP1'),
    pl.col('fields').list.get(1).alias('TFID'),
    pl.col('fields').list.get(2).alias('SUBACCT'),
    pl.col('fields').list.get(3).alias('PREIND'),
    pl.col('fields').list.get(4).cast(pl.Int64, strict=False).alias('CENSUST'),
    pl.col('fields').list.get(5).cast(pl.Int64, strict=False).alias('ACCTNO')
...
# Parse based on actual format - adjust as needed
micr_data = micr_data.with_columns([
    pl.col('data').str.split(' ').alias('fields')
])

micr_data = micr_data.with_columns([
    pl.col('fields').list.get(0).alias('BRANCH'),
    pl.col('fields').list.get(1).alias('MICRCD')
