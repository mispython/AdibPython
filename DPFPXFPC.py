...
def eimhptop():
    ...
    # From sample values: 5.0, 15.0, 61.0, 70.0, 71.0, 200.0, 205.0, 210.0, 212.0, 216.0
    # HPD products typically include certain codes - adjust as needed
    ...
    try:
        # Read only first 500 rows for testing
    ...
    # 3. Merge with BRANCH abbreviation - try to read LKP_BRANCH
    brhdata_df = None
    try:
        # Try multiple possible locations for LKP_BRANCH
        lkp_branch_paths = [
            base / "LKP_BRANCH",
            base / "LKP_BRANCH.txt",
            base / "LKP_BRANCH.dat",
                            ...
                            for line in lines[start_idx:]:
                                if line.strip():
                                    ...
                                    if parts and len(parts) >= 2:
                                        try:
                                            # Check if first part is numeric (branch code)
        ...
        if not lkp_found or brhdata_df is None or brhdata_df.is_empty():
            ...
            # Create a simple mapping from the branch codes in the data
            unique_branches = hpacc_df["BRANCH"].unique().to_list()
            brh_lines = []
            for branch in unique_branches:
                if branch is not None:
                    brh_lines.append({"BRANCH": branch, "BRABBR": f"BR{branch}"})
            brhdata_df = pl.DataFrame(brh_lines)
    ...
    if brhdata_df is not None and not brhdata_df.is_empty():
        # Convert BRANCH to float64 to match hpacc_df
