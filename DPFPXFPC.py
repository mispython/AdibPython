...
def eibrp159():
    ...
    if len(npgs_df) > 0:
        ...
        try:
            ...
            # Convert polars DataFrame to pandas and ensure clean data types
            pd_df = npgs_df.to_pandas()
            
            # Clean the DataFrame for SAS compatibility
            ...
            for col in pd_df.columns:
                if pd_df[col].dtype == 'object':
                    ...
                    pd_df[col] = pd_df[col].replace('None', '')
                    pd_df[col] = pd_df[col].replace('nan', '')
            
            # Use SASsubmit with datalines approach for small datasets
            # This is more reliable than df2sd for small data
            if len(pd_df) <= 100:  # For small datasets
                # Create SAS data step with datalines
                data_lines = []
                ...
                # Build SAS code with proper variable definitions
                sas_code = "data work.npgs_output;\n"
                sas_code += "    input\n"
                
                # Define variables based on column names and types
                for col in pd_df.columns:
                    if pd_df[col].dtype in ['int64', 'float64']:
                        sas_code += f"        {col}\n"
                    else:
                        sas_code += f"        {col} $\n"
                ...
                # For larger datasets, use df2sd with proper parameters
                sas.df2sd(pd_df, table='npgs_output', libref='work')
