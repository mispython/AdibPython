    ...
    try:
        ...
        if SASPY_AVAILABLE:
            try:
                ...
                # If updating and file exists, append
                if mode == 'update' and os.path.exists(output_path):
                    # Read existing dataset
                    existing_df, meta = pyreadstat.read_sas7bdat(output_path)
                    
                    # Combine existing and new data
                    combined_df = pd.concat([existing_df, df], ignore_index=True)
                    
                    # Write combined dataset
                    sas.dataframe2sasdata(combined_df, table=temp_sas_name)
                
                # Export to file
                sas.sasdata2sd2(temp_sas_name, output_path)
                
                # Clean up
                sas.submit(f'proc datasets lib=WORK; delete {temp_sas_name.split(".")[1]}; run;')
                sas.quit()
