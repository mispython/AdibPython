...
def main(reptdate=None):
    ...
    try:
        ...
        for row in df_note.iter_rows(named=True):
            ...
            # ****************************************************************
            # IMPORTANT: ITEM is ALWAYS '219' for EIBMABTL
            # According to SAS code: PROD = 'BT' (hardcoded)
            # The SELECT statements check PROD='HL', PROD='FL', PROD='RC'
            # But since PROD is always 'BT', none of these match
            # So OTHERWISE -> ITEM = '219' is always executed
            # ****************************************************************
            item = '219'
