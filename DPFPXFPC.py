...
def write_sas_file(df, file_path):
    ...
    try:
        ...
        # saspy writes to the SAS WORK directory, we need to copy it
        sas.submit(f'''
            proc copy in=user out=work;
                select {dataset_name};
            run;
            data _null_;
                infile "{file_path}";
        ...
        # Alternative: Use SAS to copy directly
        sas.submit(f'''
...
def main():
    ...
    dcimyr = pl.concat([cusmyr, ibnmyr]) if len(cusmyr) > 0 or len(ibnmyr) > 0 else pl.DataFrame()
