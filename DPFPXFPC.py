...
def calculate_remmth(matdt: date, reptdate_val: date) -> float:
...
def calculate_next_bldate(bldate: date, issdte: date, payfreq: str, product: int) -> date:
...
def categorize_remmth(remmth: float) -> str:
...
def get_prod_type(product: int) -> str:
...
def get_item_code(prod_type: str, custcd: str) -> str:
...
def main(reptdate: date | None = None) -> int:
    ...
    try:
        ...
        if reptdate is None:
            reptdate = date.today()
            print(f"\nTESTING MODE: Using today's date: {reptdate}")
        ...
        btrad_filename = f"btrad{reptmon}{nowk}{yy_2digit}.sas7bdat"
        btrad_path = INPUT_DIR / btrad_filename
        
        # Also try uppercase
        if not btrad_path.exists():
            btrad_filename_upper = f"BTRAD{reptmon}{nowk}{yy_2digit}.sas7bdat"
        ...
        for row in df_note.iter_rows(named=True):
            ...
            if (exprdate - reptdate).days < 8:
                ...
                while current_bldate <= exprdate:
                    ...
                    # Add payment records
                    records.append({
                        'BNMCODE': f"{prefix1}{item}{cust}{remmth_code}0000Y",
                        'AMOUNT': payamt,
                        'AMTUSD': 0,
                        'AMTSGD': 0
                    })
                    ...
                    records.append({
                        'BNMCODE': f"{prefix2}{item}{cust}{remmth_code_overdue}0000Y",
                        'AMOUNT': payamt,
                        'AMTUSD': 0,
                        'AMTSGD': 0
                    })
            ...
            records.append({
                'BNMCODE': f"{prefix1}{item}{cust}{remmth_code_final}0000Y",
            ...
            records.append({
                'BNMCODE': f"{prefix2}{item}{cust}{remmth_code_overdue_final}0000Y",
                'AMOUNT': current_balance,
                'AMTUSD': amtusd,
                'AMTSGD': amtsgd
            })
        ...
        print(f"  Output records created: {len(records)}")
        ...
        print(f"\nWriting output to: {NLFBT}")
        with open(NLFBT, 'w', encoding='utf-8') as f:
            # Write header
        ...
        print(f"Total amount: {df_summary['AMOUNT'].sum():,.2f}")
