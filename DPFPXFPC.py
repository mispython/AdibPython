...
def load_pbbrdal_data():
    ...
    if PBBMRDLF is not None:
        try:
            ...
            # PBBMRDLF should be a DataFrame with ITCODE, AMTIND, AMOUNT
            df_ref = PBBMRDLF
...
if len(alw) > 0 and len(pbbrdal1) > 0:
    ...
    # Check for column name mismatches (case sensitivity)
    # Convert to uppercase for comparison if needed
...
if len(loan_data) > 0:
    ...
    # Check column names - try different casings
    zipcode_col = None
    loantype_col = None
    balance_col = None
    ...
    for col in loan_data.columns:
        if col.upper() == 'PZIPCODE':
            zipcode_col = col
        elif col.upper() == 'LOANTYPE':
            loantype_col = col
        elif col.upper() == 'BALANCE':
            balance_col = col
