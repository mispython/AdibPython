...
APPLY_PROGCD_FILTER = False  # Set to False to skip PROGCD filter and match production
...
if datasets_to_combine:
    ...
    # DEBUG: Check PROGCD values before filtering
    print("\n  - DEBUG: PROGCD distribution in combined data:")
    if 'progcd' in deposit_combined.columns:
        prodcd_dist = deposit_combined.group_by('progcd').agg(pl.len().alias('count')).sort('count', descending=True)
        print(f"    Total unique PROGCD values: {prodcd_dist.height}")
        print("    Top 20 PROGCD values:")
        for row in prodcd_dist.head(20).rows():
            print(f"      {row[0]}: {row[1]} records")
    
    # DEBUG: Check ACCTNO first digit by source
    print("\n  - DEBUG: ACCTNO first digit distribution:")
    first_digit_all = deposit_combined.with_columns([
        pl.col('acctno').cast(pl.Utf8).str.slice(0, 1).cast(pl.Int64).alias('first_digit')
    ])
    first_digit_dist = first_digit_all.group_by('first_digit').agg(pl.len().alias('count')).sort('first_digit')
    for row in first_digit_dist.rows():
        print(f"      {row[0]}: {row[1]} records")
    ...
    if 'progcd' not in deposit_combined.columns:
        ...
        # Apply PROGCD filter only if enabled
        if APPLY_PROGCD_FILTER:
            valid_progcd = [
            ...
            deposit_filtered = deposit_combined
            print(f"  - PROGCD filter SKIPPED (all records kept): {deposit_filtered.height} records")
        
        # Apply PRODUCT filters
        if 'product' in deposit_filtered.columns and not deposit_filtered.is_empty():
            print(f"\n  - DEBUG: Before PRODUCT filters: {deposit_filtered.height} records")
            
            # Check PRODUCT distribution
            product_dist = deposit_filtered.group_by('product').agg(pl.len().alias('count')).sort('count', descending=True)
            print("    Product distribution (top 10):")
            for row in product_dist.head(10).rows():
                print(f"      {row[0]}: {row[1]} records")
            ...
            before_special = deposit_filtered.height
            ...
            print(f"  - After PROGCD special: {deposit_filtered.height} records (removed {before_special - deposit_filtered.height})")
            
            # IF PRODUCT IN (30,31,32,33,34) THEN DELETE
            before_product_filter = deposit_filtered.height
            deposit_filtered = deposit_filtered.filter(
                ~pl.col('product').is_in([30.0, 31.0, 32.0, 33.0, 34.0])
            )
            print(f"  - After PRODUCT filter: {deposit_filtered.height} records (removed {before_product_filter - deposit_filtered.height})")
        ...
        if not deposit_filtered.is_empty():
            ...
            # ============================================
            # DEBUG: Analyze ACCTNO ranges after filters
            # ============================================
            print("\n  - DEBUG: DEPOSIT ACCTNO analysis after filters:")
            
            deposit_first_digit = deposit_filtered.with_columns([
                pl.col('acctno').cast(pl.Utf8).str.slice(0, 1).cast(pl.Int64).alias('first_digit')
            ])
            first_digit_dist = deposit_first_digit.group_by('first_digit').agg(pl.len().alias('count')).sort('first_digit')
...
if float_df is not None:
    ...
    if not float_df.is_empty() and 'float' in float_df.columns and 'acctno' in float_df.columns:
        ...
        # ============================================
        # DEBUG: Analyze FLOAT ACCTNO ranges
        # ============================================
        print("\n  - DEBUG: FLOAT ACCTNO analysis:")
        
        float_first_digit = float_summary.with_columns([
            pl.col('acctno').cast(pl.Utf8).str.slice(0, 1).cast(pl.Int64).alias('first_digit')
        ])
        first_digit_dist = float_first_digit.group_by('first_digit').agg(pl.len().alias('count')).sort('first_digit')
        print("    First digit distribution:")
        for row in first_digit_dist.rows():
            print(f"      {row[0]}: {row[1]} records")
    else:
...
if not deposit_filtered.is_empty() and not float_summary.is_empty():
    ...
    # Show sample of overlapping ACCTNOs
    if overlap:
        print("\n  - DEBUG: Sample overlapping ACCTNOs (first 10):")
        for acct in list(overlap)[:10]:
            print(f"    {acct}")
