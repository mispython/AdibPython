...
# Input SAS files - pattern: BTRAD{MM}{WK}.sas7bdat (without year)
# Example: btrad061.sas7bdat (Month=06, Week=1)
BTRAD_FILE_TEMPLATE = INPUT_DIR / "btrad{mm}{wk}.sas7bdat"
...
def process_loan_data(macro_vars: dict) -> pl.DataFrame:
    ...
    # Build filename: btrad{MM}{WK}.sas7bdat (without year)
    # Example: btrad061.sas7bdat (Month=06, Week=1)
    btrad_filename = f"btrad{mm}{wk}.sas7bdat"
    btrad_path = INPUT_DIR / btrad_filename
    
    # Also try uppercase version if lowercase not found
    if not btrad_path.exists():
        btrad_filename_upper = f"BTRAD{mm}{wk}.sas7bdat"
...
def main(reptdate: date | None = None) -> int:
    ...
    try:
        ...
        print(f"Expected BTRAD file: btrad{macro_vars['REPTMON']}{macro_vars['NOWK']}.sas7bdat")
