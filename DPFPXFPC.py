...
# Show combined conditions
if 'BORSTAT' in df_woff.columns and 'DAYS' in df_woff.columns and 'LOANTYPE' in df_woff.columns and 'PAIDIND' in df_woff.columns and 'TOTAL' in df_woff.columns:
    excluded_loans = [983, 993, 678, 679, 698, 699]
    combined = df_woff.filter(
        (
    ...
    print(f"\n  Combined (all criteria): {combined.height} rows")
    
    if combined.height > 0:
        print(f"\n  Sample rows meeting criteria:")
        print(combined.select(['ACCTNO', 'NOTENO', 'BORSTAT', 'DAYS', 'LOANTYPE', 'PAIDIND', 'TOTAL']).head(10))
