...
# ==================== REMMTH CALCULATION ====================
def calculate_remmth_from_sas_date(matdate_val, reptdate_val):
    """Calculate remaining months from SAS date value"""
    ...
    try:
        ...
        if isinstance(matdate_val, (int, float)):
            fddt = sas_date_to_python(matdate_val)
        elif isinstance(matdate_val, (datetime, date)):
            fddt = matdate_val
        else:
...
def calculate_remmth(row, reptdate_val):
    ...
    return calculate_remmth_from_sas_date(matdate_val, reptdate_val)
...
if "MATDATE" in fdmthly.columns:
    ...
    try:
        ...
        if fdmthly['MATDATE'].dtype in [pl.Int32, pl.Int64, pl.Float32, pl.Float64]:
            ...
            print(f"MATDATE range: {min_val} to {max_val}")
            
            # Convert to dates for understanding
            min_date = sas_date_to_python(min_val)
            max_date = sas_date_to_python(max_val)
...
# For better performance, handle MATDATE conversion in bulk where possible
if "MATDATE" in fdmthly.columns:
    # If MATDATE is numeric (SAS date), convert it directly
    if fdmthly['MATDATE'].dtype in [pl.Int32, pl.Int64, pl.Float32, pl.Float64]:
        print("MATDATE is numeric - converting SAS dates directly...")
        # Convert SAS numeric date to date string for easier processing
        fdmthly = fdmthly.with_columns([
            pl.col("MATDATE").map_elements(
                lambda x: sas_date_to_python(x),
        ...
        fdmthly = fdmthly.with_columns([
            pl.struct(["OPENIND", "MATDATE_DATE"]).map_elements(
                lambda x: calculate_remmth_from_sas_date(x.get("MATDATE_DATE"), reptdate_val) if x.get("OPENIND") == "O" else (-1.0 if x.get("OPENIND") == "D" else None),
