...
def process_k1tbl(rep_date):
    ...
    try:
        ...
        # DEBUG: Show unique values of key columns
        if 'gwmvt' in df.columns:
            unique_gwmvt = df['gwmvt'].unique().to_list()
            print(f"    Unique values in GWMVT: {unique_gwmvt}")
        else:
            print(f"    Column 'gwmvt' not found! Available columns: {df.columns}")
            # Try case variations
            for col in df.columns:
                if 'gwmvt' in col.lower():
        ...
        for row in df.iter_rows(named=True):
            ...
            if gwmvt != 'P':
                ...
                if filtered_out <= 3:
                    print(f"    Filtered row {filtered_out}: gwmvt='{gwmvt}' (expected 'P')")
                continue
...
def process_k3tbl(rep_date):
    ...
    try:
        ...
        # DEBUG: Show unique values of key columns
        if 'utref' in df.columns:
            unique_utref = df['utref'].unique().to_list()
            print(f"    Unique values in UTREF: {unique_utref[:20]}")
        else:
            print(f"    Column 'utref' not found! Available columns: {df.columns}")
            # Try case variations
            for col in df.columns:
        ...
        # DEBUG: Show unique values of UTSTY
        if 'utsty' in df.columns:
            unique_utsty = df['utsty'].unique().to_list()
