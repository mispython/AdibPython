...
def apply_format_mappings(df: pl.DataFrame) -> pl.DataFrame:
    ...
    if 'PRODCD' in df.columns:
        ...
        # Using map_elements to apply the format function from PBBDPFMT
        logger.info("Applying product format mappings from PBBDPFMT")
        
        # Determine which format to use based on product type
...
def _get_product_format(prodcd: int) -> str:
    """
    ...
    if prodcd in ProductLists.CURX_PRODUCTS:
        return CAProductFormat.format(prodcd)
    
    # Check if it's a Savings product (typically 200-300 range)
    if 200 <= prodcd <= 300:
        return SAProductFormat.format(prodcd)
    
    # Check if it's a Fixed Deposit product (typically 229-997 range)
    if 229 <= prodcd <= 997:
        return FDProductFormat.format(prodcd)
    
    # Default - return the product code as string
    return str(prodcd)

def _get_denomination(prodcd: int) -> str:
    """
    ...
    # Check if it's a Current Account product
    if prodcd in ProductLists.CURX_PRODUCTS:
        return CADenomFormat.format(prodcd)
    
    # Check if it's a Savings product
    if 200 <= prodcd <= 300:
        return SADenomFormat.format(prodcd)
    
    # Check if it's a Fixed Deposit product
    if 229 <= prodcd <= 997:
        return FDDenomFormat.format(prodcd)
