...
# Python conversion of SAS job EIIDBT12 (Islamic version)
...
# (Conventional version DELETES them, Islamic version KEEPS them)
# --------------------------------------------------------------------
# IF BRANCH > 3000 AND (2850000000<=ACCTNO<=2859999999) THEN OUTPUT;
# This means we KEEP records that match this condition
btdtl = btdtl.filter(
    (pl.col("BRANCH") > 3000) &
...
def read_base_sas(prevmon):
    ...
    # Column mapping (same structure as BTBASE, but with IBTBASE)
