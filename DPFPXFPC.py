...
LOANTEMP_PATH = INPUT_PATH / "loantemp.sas7bdat"
LKP_BRANCH_PATH = INPUT_PATH / "LKP_BRANCH"   # flat file - adjust extension/name as needed
...
def load_branch_data() -> pl.DataFrame:
    ...
    Parsing strategy (in priority order):
      1. If USE_FIXED_WIDTH is True, use the exact column positions in
         FIXED_WIDTH_SPECS (only needed if a true fixed-width layout is
         later confirmed; not required for the current file).
      2. Otherwise, try a small set of single-character delimiters.
      3. Otherwise, fall back to manual line parsing based on the confirmed
         layout, e.g.:
             'B001 PCS   BANK-ATMC                       C   O   ...'
         Tokens (whitespace-split): token[0] is the branch code as the
         letter "B" plus a zero-padded number (e.g. "B001"), which is
         stripped down to a plain integer (1, 2, 3, ...) to match the
         loan file's numeric BRANCH column; token[1] is BRHCODE (e.g.
         "PCS"); anything after that (branch name, region code, office
         type, etc.) is ignored.
    """
    ...
    if df is None:
        # Confirmed layout (from sample lines):
        #   'B001 PCS   BANK-ATMC                             C   O   ...'
        # Whitespace-delimited tokens:
        #   token[0] = branch code as "B" + zero-padded number, e.g. "B001"
        #              -> must strip the letter prefix/leading zeros to get
        #                 a plain integer (1, 2, 3, ...) matching the loan
        #                 file's numeric BRANCH column.
        #   token[1] = BRHCODE, e.g. "PCS"
        #   token[2:] = branch name / region code / office type etc. (unused)
        import re
        ...
        with open(LKP_BRANCH_PATH, "r", encoding="utf-8", errors="replace") as f:
            for i, raw_line in enumerate(f):
                if i < 8:
                    raw_lines_sample.append(raw_line.rstrip("\n"))
                line = raw_line.strip()
                if not line:
                    continue
                tokens = line.split()
                if len(tokens) < 2:
                    continue
                branch_raw = tokens[0]              # e.g. "B001"
                brhcode = tokens[1]                 # e.g. "PCS"
                digits = re.sub(r"\D", "", branch_raw)  # e.g. "001"
                if digits == "":
                    continue
                branch_num = int(digits)            # e.g. 1
                rows.append({"BRANCH": branch_num, "BRHCODE": brhcode})

        print("   [debug] LKP_BRANCH first 8 raw lines (repr):")
        for rl in raw_lines_sample:
            print(f"   [debug]   {rl!r}")
        print(f"   [debug] first 8 parsed rows: {rows[:8]}")
...
def load_and_filter_loans(hpd_list: List[str], variables: Dict) -> pl.DataFrame:
    ...
    # SAS numeric columns come back as float64 via pyreadstat/pandas even
    # when they hold whole numbers (e.g. BRANCH). Cast to Int64 so the join
    # key type matches load_branch_data()'s BRANCH column (also Int64) -
    # otherwise polars raises a SchemaError (f64 vs i64) on the join below.
    loan_df = loan_df.with_columns(pl.col("BRANCH").cast(pl.Int64, strict=False))

    print(f"   [debug] raw LOANTEMP rows: {len(loan_df)}")
    print(f"   [debug] BALANCE>0 rows: {len(loan_df.filter(pl.col('BALANCE') > 0))}")
    print(f"   [debug] BORSTAT != 'Z' rows: {len(loan_df.filter(pl.col('BORSTAT') != 'Z'))}")
    print(f"   [debug] sample PRODUCT values: {loan_df['PRODUCT'].unique().to_list()[:15]}")
    print(f"   [debug] sample BORSTAT values: {loan_df['BORSTAT'].unique().to_list()[:15]}")

    # Convert HPD list to numbers
    hpd_numbers = [int(x.strip("'")) for x in hpd_list]
    print(f"   [debug] HPD filter numbers: {hpd_numbers}")
    print(f"   [debug] PRODUCT in HPD rows: {len(loan_df.filter(pl.col('PRODUCT').is_in(hpd_numbers)))}")
    ...
    print(f"   [debug] rows after all three filters (pre-join): {len(filtered)}")

    # Load and merge branch data
    branch_df = load_branch_data()
    print(f"   [debug] LKP_BRANCH rows loaded: {len(branch_df)}")
    print(f"   [debug] sample branch_df BRANCH values: {branch_df['BRANCH'].unique().to_list()[:15]}")
    print(f"   [debug] sample loan BRANCH values: {filtered['BRANCH'].unique().to_list()[:15]}")
    ...
    print(f"   [debug] rows after branch join: {len(merged)}")
