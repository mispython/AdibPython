...
MAX_ROWS_SAS = 50000  # Limit rows for SAS7BDAT files (testing)
MAX_ROWS_TEXT = 50000  # Limit rows for CRFTABL text file (testing)
# COLL and DESC files should be read completely to ensure proper joins
# MAX_ROWS_COLL_DESC = None  # Read all rows for COLL and DESC
...
def read_coll_file(file_path: Path, max_rows: int = None) -> pl.DataFrame:
    ...
    Note: For proper joins, this file should be read completely without row limits.
    """
    if not file_path.exists():
        raise FileNotFoundError(f"File not found: {file_path}")
...
def read_coll_fixed_width(file_path: Path, max_rows: int = None) -> pl.DataFrame:
    ...
    if parsed_data:
        df = pl.DataFrame(parsed_data)
        print(f"Read {len(df)} rows from COLL file" + (f" (limited to {max_rows})" if max_rows else ""))
    else:
        df = pl.DataFrame({
            'ccollno': pl.Series([], dtype=pl.Int64),
            'acctno': pl.Series([], dtype=pl.Int64)
...
def read_coll_binary_format(file_path: Path, max_rows: int = None) -> pl.DataFrame:
    ...
    if parsed_data:
        df = pl.DataFrame(parsed_data)
        print(f"Read {len(df)} rows from COLL binary file" + (f" (limited to {max_rows})" if max_rows else ""))
    else:
        df = pl.DataFrame({
            'ccollno': pl.Series([], dtype=pl.Int64),
            'acctno': pl.Series([], dtype=pl.Int64)
...
def read_desc_fixed_width(file_path: Path, max_rows: int = None) -> pl.DataFrame:
    ...
    if parsed_data:
        df = pl.DataFrame(parsed_data)
        print(f"Read {len(df)} rows from DESC file" + (f" (limited to {max_rows})" if max_rows else ""))
    else:
        df = pl.DataFrame({
            'ccollno': pl.Series([], dtype=pl.Int64),
            'cinstcl': pl.Series([], dtype=pl.Utf8),
...
print(f"  - COLL/DESC files: reading ALL rows (no limit for proper joins)")
...
# Read COLL and DESC files WITHOUT row limits for proper joins
coll = read_coll_file(COLL_FILE, max_rows=None)  # Read all rows
desc = read_desc_file(DESC_FILE, max_rows=None)  # Read all rows
