...
if data.get('ibtrad') is not None and cred is not None:
    # Debug: Check IBTrad columns and repaid values
    print("\n--- DEBUG: IBTrad Data ---")
    print(f"IBTrad columns: {data['ibtrad'].columns}")
    
    if 'repaid' in data['ibtrad'].columns:
        ibtrad_repaid_stats = data['ibtrad'].select([
            ...
            (pl.col("repaid").is_null()).sum().alias("count_null")
        ])
        print(f"IBTrad REPAID stats: {ibtrad_repaid_stats.to_dicts()}")
        
        # Show sample of non-zero repaid
        sample_repaid = data['ibtrad'].filter(
            pl.col("repaid").cast(pl.Float64, strict=False) > 0
        ).select(["acctnox", "transref", "repaid"]).head(5)
        print(f"Sample IBTrad records with repaid > 0:")
        print(sample_repaid)
    else:
        print("WARNING: 'repaid' column not found in IBTrad")
    
    # Check if there's a different column name for repaid
    repaid_cols = [c for c in data['ibtrad'].columns if 'rep' in c.lower() or 'pay' in c.lower()]
    print(f"Columns related to repayment: {repaid_cols}")
    
    # BTRAX - Repaid/Disburse
    ...
    # Debug: Check BTRAX repaid stats
    btrax_repaid_stats = btrax.select([
        pl.col("repaid").cast(pl.Float64, strict=False).sum().alias("total_repaid"),
        pl.col("repaid").cast(pl.Float64, strict=False).max().alias("max_repaid"),
        (pl.col("repaid").cast(pl.Float64, strict=False) > 0).sum().alias("count_repaid_gt_0")
    ])
    print(f"\nBTRAX REPAID stats: {btrax_repaid_stats.to_dicts()}")
    
    # BTRAX unique count
    print(f"BTRAX unique records: {btrax.unique(subset=['acctnox', 'transrex']).height}")
    print(f"BTRAX total records: {btrax.height}")
    ...
    # Debug: Check CRED columns before join
    print(f"\nCRED columns before join: {[c for c in cred.columns if 'repaid' in c.lower()]}")
    print(f"BTRAX columns: {btrax.columns}")
...
if cred is not None and suba_main is not None:
    ...
    # Debug: Check repaid in BTR2
    if 'repaid' in btr2.columns:
        btr2_repaid_stats = btr2.select([
            pl.col("repaid").cast(pl.Float64, strict=False).sum().alias("total_repaid"),
            pl.col("repaid").cast(pl.Float64, strict=False).max().alias("max_repaid"),
            (pl.col("repaid").cast(pl.Float64, strict=False) > 0).sum().alias("count_repaid_gt_0")
        ])
        print(f"\nBTR2 REPAID stats: {btr2_repaid_stats.to_dicts()}")
...
if btr2 is not None:
    ...
    if 'repaid' in btr2.columns:
        ...
        print(f"BTR2 REPAID stats: {repaid_stats.to_dicts()}")
    else:
        print("WARNING: 'repaid' column not found in BTR2")
    
    # Check repay_source and repay_type_cd
    if 'repay_source' in btr2.columns:
        repay_source_values = btr2['repay_source'].unique().to_list()
        print(f"REPAY_SOURCE values: {repay_source_values[:10]}")
    else:
        print("REPAY_SOURCE column not found in BTR2")
    
    if 'repay_type_cd' in btr2.columns:
        repay_type_values = btr2['repay_type_cd'].unique().to_list()
        print(f"REPAY_TYPE_CD values: {repay_type_values[:10]}")
    else:
