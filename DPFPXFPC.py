...
# ============================================================================
# STEP 7: Calculate derived fields (using polars for speed)
# ============================================================================
...
if df_loan.height > 0:
    ...
    df_extra = read_sas7bdat_fast(
        ...
        columns=['ACCTNO', 'NOTENO', 'FEETOTAL', 'NFEEAMT5', 'FEEAMT3', 'FEETOT2',
                 ...
                 'CUSTCODE', 'LSTTRNCD', 'MARKETVL', 'POSTAMT', 'OTHERAMT', 'OIFEEAMT',
                 'ECSRIND', 'BILPAID', 'PAY75PCT', 'NACODATE', 'MTHPDUE', 'CRRGRADE',
                 'MARGINFI', 'DOBMNI'],
        row_limit=TEST_LIMIT if TEST_MODE else None
    )
    ...
    df_loan = df_loan.with_columns([
        ...
        pl.when(pl.col('ECSRRSRV').fill_null(0) <= 0).then(0).otherwise(pl.col('ECSRRSRV')).alias('ECSRRSRV'),
    ...
    # CRRGRADE
    df_loan = df_loan.with_columns([
        (pl.col('SCORE2').fill_null('') + pl.col('CONTRTYPE').fill_null('')).alias('CRRGRADE')
    ])
