...
def write_sas7bdat(df: pl.DataFrame, filepath: Path, table_name: str):
    ...
    sas_df = sas.df2sd(pd_df, table=f'_temp_{table_name}', libref='WORK')
    
    # Use PROC EXPORT to write sas7bdat file
    # Note: Using PUT to avoid quotes in the file path
    sas_code = f"""
        PROC EXPORT DATA=WORK._temp_{table_name}
            OUTFILE="{sas_path}"
...
# Write each table to the IPC file with proper schema
with pa.ipc.new_file(pa.OSFile(str(ipc_path), 'wb'), TRUST.to_arrow().schema) as writer:
