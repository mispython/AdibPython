...
def apply_format_mappings(df: pl.DataFrame) -> pl.DataFrame:
    ...
    Uses replace() method which is available in Polars
    """
    if 'PRODCD' in df.columns:
        try:
            ...
            # Method 3: Using a Series-based approach
            logger.info(f"Falling back to Series-based mapping: {e}")
            # Convert to series, apply mapping, then back to column
            df = df.with_columns([
                pl.Series(
                    df['PRODCD'].map_elements(lambda x: PRODBRH.get(x, str(x)), return_dtype=pl.String)
                ).alias('PRODCD_FORMATTED')
            ])
