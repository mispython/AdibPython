...
def process_conventional_float():
    ...
    if not fdmthly_df.is_empty():
        ...
        required_cols = ['acctno', 'branch', 'intplan', 'curbal', 'bic', 'amtind', 'intpay']
        available_cols = [col for col in required_cols if col in fdmthly_df.columns]
        if available_cols:
            fdmthly_df = fdmthly_df.select(available_cols).sort('acctno')
        else:
            print("NOTE: Required columns not found in MNI.fdmthly")
    ...
    if not fdmthly_df.is_empty() and 'curbal' in fdmthly_df.columns:
    ...
    if not savg_df.is_empty():
        required_cols = ['acctno', 'product', 'curbal', 'ledgbal', 'progcd', 'amtind', 'intpaybl', 'branch']
        available_cols = [col for col in required_cols if col in savg_df.columns]
        if available_cols:
            savg_df = savg_df.select(available_cols)
    ...
    if not curn_filtered.is_empty():
        required_cols = ['acctno', 'product', 'curbal', 'ledgbal', 'progcd', 'amtind', 'intpaybl', 'branch']
        available_cols = [col for col in required_cols if col in curn_filtered.columns]
        if available_cols:
            curn_selected = curn_filtered.select(available_cols)
            datasets_to_combine.append(curn_selected)

    # FDMTHLY with renames
    if not fdmthly_processed.is_empty():
        # Check if all required columns exist
        if all(col in fdmthly_processed.columns for col in ['acctno', 'branch', 'curbal', 'ledgbal', 'amtind', 'intplan', 'bic', 'intpay']):
    ...
    if datasets_to_combine:
        deposit_combined = pl.concat(datasets_to_combine, how="diagonal_relaxed")
...
def process_islamic_float():
    ...
    if not fdmthly_df.is_empty():
        ...
        if available_cols:
            ...
            print("NOTE: Required columns not found in IMNI.fdmthly")
            fdmthly_df = pl.DataFrame()
    else:
