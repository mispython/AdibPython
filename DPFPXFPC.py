...
branch_abbr = {
    ...
    804: 'H47', 808: 'H04', 117: '117', 812: '812'
}
...
# Apply branch mappings and SUBSTR extractions
repo_df = repo_df.with_columns([
...
# Function to write SAS-style fixed-width output
def write_sas_fixed_width(filename, data, title):
    """Write fixed-width text file matching SAS PUT @column format exactly"""
    with open(output_path / filename, "w") as f:
        # Write title
        f.write(f"{rdate}-{title}\n")
        
        for row in data.iter_rows(named=True):
            # Build line exactly as SAS PUT statements:
            # PUT @001 BRABBR $3. @009 CAC $20. @029 REGNO $13. @043 MAKE $16. 
            # @060 MODEL $21. @082 ENGINE $40. @123 CHASSIS $40. @164 COLLYEAR $4.
            
            # Create empty line of 167 characters (positions 1-167)
            line = [' '] * 168
            
            # Get values
            brabbr = str(row.get('BRABBR', '')).strip()[:3]
            cac = str(row.get('CAC', '')).strip()[:20]
            regno = str(row.get('REGNO', '')).strip()[:13]
            make = str(row.get('MAKE', '')).strip()[:16]
            model = str(row.get('MODEL', '')).strip()[:21]
            engine = str(row.get('ENGINE', '')).strip()[:40]
            chassis = str(row.get('CHASSIS', '')).strip()[:40]
            collyear = str(row.get('COLLYEAR_STR', '')).strip()[:4]
            
            # Place values at exact SAS column positions (0-indexed)
            # @001 = position 0, @009 = position 8, etc.
            for i, char in enumerate(brabbr):
                if i < 3: line[i] = char  # @001-003
            
            for i, char in enumerate(cac):
                if i < 20: line[8 + i] = char  # @009-028
            
            for i, char in enumerate(regno):
                if i < 13: line[28 + i] = char  # @029-041
            
            for i, char in enumerate(make):
                if i < 16: line[42 + i] = char  # @043-058
            
            for i, char in enumerate(model):
                if i < 21: line[59 + i] = char  # @060-080
            
            for i, char in enumerate(engine):
                if i < 40: line[81 + i] = char  # @082-121
            
            for i, char in enumerate(chassis):
                if i < 40: line[122 + i] = char  # @123-162
            
            for i, char in enumerate(colyear):
                if i < 4: line[163 + i] = char  # @164-167
...
write_sas_fixed_width("repotxt.txt", repo_sorted, "REPOSSESSION LISTING")
print(f"REPOTXT.txt generated with {repo_sorted.height} records")

# Generate REPOTXT1
print("Generating REPOTXT1.txt...")
write_sas_fixed_width("repotxt1.txt", repo1_sorted, "REPOSSESSION LISTING (983,993)")
