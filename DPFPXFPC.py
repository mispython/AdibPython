...
# ----------------------------------------------------------------------
# Load PBBLNFMT formats (with fallback)
# ----------------------------------------------------------------------
...
try:
    ...
    elif hasattr(PBBLNFMT, 'sectcd'):
        SECTCD = PBBLNFMT.sectcd
    elif hasattr(PBBLNFMT, 'format_dict'):
        # Might be a dict with all formats
        fmt_dict = PBBLNFMT.format_dict
        if 'SECTCD' in fmt_dict:
            SECTCD = fmt_dict['SECTCD']
        if 'SECTA' in fmt_dict:
            SECTA = fmt_dict['SECTA']
        if 'SECTB' in fmt_dict:
            SECTB = fmt_dict['SECTB']
    
    if hasattr(PBBLNFMT, 'SECTA'):
        SECTA = PBBLNFMT.SECTA
    elif hasattr(PBBLNFMT, 'secta'):
        SECTA = PBBLNFMT.secta
    
    if hasattr(PBBLNFMT, 'SECTB'):
        SECTB = PBBLNFMT.SECTB
    elif hasattr(PBBLNFMT, 'sectb'):
        SECTB = PBBLNFMT.sectb
        
    print(f"[FMT] Loaded SECTCD: {len(SECTCD)} entries, SECTA: {len(SECTA)} entries, SECTB: {len(SECTB)} entries")
    
except ImportError:
    print("[WARN] PBBLNFMT not found – using fallback sector mappings.")
except Exception as e:
    print(f"[WARN] Error loading PBBLNFMT: {e}")

# If formats are empty, use fallback mappings based on SAS program
if not SECTCD:
    print("[FMT] Using fallback SECTCD mapping (numeric sector -> code)")
    # From the SAS program: SECTORCD=PUT(SECTOR, $SECTCD.)
    # We don't know the exact mapping, but we can use a default
    # Common sectors based on typical banking data
    SECTCD = {
...
if not SECTA:
    print("[FMT] Using fallback SECTA mapping (sector code -> A code)")
    # From SAS: SECTCD = PUT(SECTORCD,$SECTA.)
    # This creates a different categorization
    SECTA = {
        '01': 'A1', '02': 'A2', '03': 'A3', '04': 'A4', '05': 'A5',
        '06': 'A6', '07': 'A7', '08': 'A8', '09': 'A9', '10': 'A0'
    }

if not SECTB:
    print("[FMT] Using fallback SECTB mapping (sector code -> B code)")
    # From SAS: SECTCD = PUT(SECTORCD,$SECTB.)
    # Another categorization
    SECTB = {
        '01': 'B1', '02': 'B2', '03': 'B3', '04': 'B4', '05': 'B5',
        '06': 'B6', '07': 'B7', '08': 'B8', '09': 'B9', '10': 'B0'
    }
...
def read_sas_cached(sas_path):
    ...
    if cache_fresh(sas_path, cache_path):
        ...
        try:
            ...
            # If cache corrupt, delete and reconvert
            cache_path.unlink()
...
def build_expanded(lnnote, curr, prev):
    ...
    if SECTA or SECTB:
        for _, row in merged.iterrows():
            ...
            # If no mapping, use the original sector code
            if not a and not b:
                a = str(sectcd)
        ...
        # No formats available, just use SECTORCD as-is
        for _, row in merged.iterrows():
            rows.append({
                'SECTCD': row['SECTORCD'],
                'DISBURSE': row['DISBURSE'],
...
def summarise(expanded, label, custcd_filter=None):
    ...
    summary['WEIGHTED'] = summary['PRODUCT'] / summary['DISBURSE'].replace(0, np.nan)
...
def main():
    ...
    if not prev_path.exists():
        ...
        # Try to find a file with the previous pattern
        candidates = list(INPUT_DIR.glob("loan*.sas7bdat"))
        if candidates:
            # Exclude the current file if it's the only one
            for c in candidates:
                if c != curr_path:
                    prev_path = c
                ...
                # If only one file exists, use the same file (not ideal but prevents crash)
                prev_path = curr_path
