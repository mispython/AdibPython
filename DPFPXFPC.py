...
def safe_float(val, default=0.0):
    """
    Coerce a value to float for numeric fixed-width fields, mapping NaN/None
    to 0.0. Without this, Python's format() happily renders NaN as the
    literal string 'nan' (e.g. f'{float("nan"):>16.2f}' -> '             nan'),
    which silently corrupts the fixed-width output rather than raising -
    much harder to catch than a crash. Common after outer merges leave a
    column unmatched for a given row.
    """
    try:
        if val is None:
            return default
