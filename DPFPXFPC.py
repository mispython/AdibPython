...
def calculate_remmth(row, reptdate_val):
    ...
    Where:
    - REMY = FDYR - RPYR (year difference)
    - REMM = FDMTH - RPMTH (month difference)
    - REMD = FDDAY - RPDAY (day difference)
    - RPDAYS(RPMTH) = days in report month
    """
...
def generate_combined_report(data, report_date):
    ...
    with open(report_file, 'w') as f:
        for section_idx, section in enumerate(sections):
            ...
            # Data rows with observation numbers - NO extra blank lines like production
            obs_num = 1
...
print(f"Records processed: {2_756_145:,}")
