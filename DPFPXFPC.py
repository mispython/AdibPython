...
if datasets_to_combine:
    ...
    if 'progcd' not in deposit_combined.columns:
        ...
        if not deposit_filtered.is_empty():
            ...
            # Show sample ACCTNOs from each range
            print("\n    Sample ACCTNOs from DEPOSIT (first 5 of each first digit):")
            for digit in range(10):
                sample = deposit_first_digit.filter(pl.col('first_digit') == digit).select('acctno').head(5).rows()
                if sample:
                    print(f"      {digit}xxx: {[row[0] for row in sample]}")
        else:
...
if float_df is not None:
    ...
    if not float_df.is_empty() and 'float' in float_df.columns and 'acctno' in float_df.columns:
        ...
        for digit in range(10):
            sample = float_first_digit.filter(pl.col('first_digit') == digit).select('acctno').head(5).rows()
            if sample:
                print(f"      {digit}xxx: {[row[0] for row in sample]}")
    else:
