...
mni_path = Path("/sas/python/virt_edw/Data_Warimport polars as pl
from pathlib import Path
import datetime
import pyreadstat
import re
...
if any([savg, curn, isavg, icurn]):
    dep_df = pl.concat([d for d in [savg, curn, isavg, icurn] if d is not None], how="diagonal")
...
print("="*80)ehouse/MIS/XMIS/input/prod/EIBQREMT")
imni_path = Path("/sas/python/virt_edw/Data_Warehouse/MIS/XMIS/input/prod/EIIQREMT")
output_path = Path("/sas/python/virt_edw/Data_Warehouse/MIS/XMIS/output/EIBQREMT")
output_path.mkdir(exist_ok=True)
deposit_path.mkdir(exist_ok=True)
...
def parse_remit_file(filepath):
        ...
        with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
            ...
            for line in f:
                ...
                try:
                    ...
                    # @003 ACCTNO PD6. - positions 3-8 (0-indexed: 2-7)
                    # @009 CHEQNO 6. - positions 9-14 (0-indexed: 8-13)
                    # @027 ISSYY $4. - positions 27-30 (0-indexed: 26-29)
                    # @032 ISSMM $2. - positions 32-33 (0-indexed: 31-32)
                    # @035 ISSDD $2. - positions 35-36 (0-indexed: 34-35)
                    # @037 ISSBRANCH PD3. - positions 37-39 (0-indexed: 36-38)
                    # @040 LEDGBAL PD7.2 - positions 40-48 (0-indexed: 39-47)
                    # @047 STATUS $2. - positions 47-48 (0-indexed: 46-47)
                    # @055 PAYMODE $10. - positions 55-64 (0-indexed: 54-63)
                    # @075 NAME $40. - positions 75-114 (0-indexed: 74-113)
                    
                    acctno_raw = raw[2:8] if line_len > 8 else ""  # PD6. - 6 digits
                    cheqno_raw = raw[8:14] if line_len > 14 else ""  # 6 digits
                    issyy_raw = raw[26:30] if line_len > 30 else ""  # 4 chars
                    issmm_raw = raw[31:33] if line_len > 33 else ""  # 2 chars
                    issdd_raw = raw[34:36] if line_len > 36 else ""  # 2 chars
                    issbranch_raw = raw[36:39] if line_len > 39 else ""  # 3 digits
                    ledgbal_raw = raw[39:48] if line_len > 48 else ""  # 7.2 format (9 chars including decimal)
                    status_raw = raw[46:48] if line_len > 48 else ""  # 2 chars
                    paymode_raw = raw[54:64] if line_len > 64 else ""  # 10 chars
                    name_raw = raw[74:114] if line_len > 114 else ""  # 40 chars
                    ...
                    # Parse LEDGBAL - handle PD7.2 format (might have decimal point or implied decimal)
                    ledgbal_clean = ledgbal.replace(',', '').strip()
                    try:
                        if ledgbal_clean:
                            # If it looks like a number with decimal
                            if '.' in ledgbal_clean:
                                ledgbal_val = float(ledgbal_clean)
                            else:
                                # PD7.2 means 7 digits with 2 decimal places implied
                                # But it might also be stored with decimal point
                                # Try to parse as is
                                ledgbal_val = float(ledgbal_clean) / 100 if len(ledgbal_clean) <= 7 else float(ledgbal_clean)
                        else:
                            ledgbal_val = 0.0
                    except:
                        ledgbal_val = 0.0
    ...
    # Show some sample LEDGBAL values for debugging
    if df.height > 0:
        sample = df.select(['ACCTNO', 'LEDGBAL', 'PAYMODE', 'NAME']).head(5)
...
if not remit_final.is_empty() and not dep_deduped.is_empty():
    ...
    # For remit_final, PAYMODE is already the ACCTNO in the SAS code
    # We need to use PAYMODE as the join key
...
if not nondebit_invalid.is_empty():
    ...
    # For nondebit records, PAYMODE may not be numeric, so we don't convert to integer
    # We just keep it as string for the report
    nondebit_processed = nondebit_invalid.with_columns([
        pl.lit('NON_DEBIT').alias('BC'),
        pl.col('PAYMODE').alias('ACCTNO')  # Keep as string
    ])
