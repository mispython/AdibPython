...
def main():
    ...
    eq = eqfl.with_columns([
        ...
        pl.col("PREMPAID").abs(),
        pl.col("PREMREC").abs(),
        # Convert STARTDT and MATDT from SAS date to string for comparison
    ...
    # -------------------------------------------------------------------
    # FX enrichment - FIXED: Use correct spot rate
    # -------------------------------------------------------------------
    eqrt = eqrt.rename({"CURRENCY": "INVCURR", "SPOTRATE": "SPOTRT"})
    
    # For MYR, set SPOTRT to 1.0000000
    # Create a copy of eqrt with MYR = 1.0
    myr_row = pl.DataFrame({'INVCURR': ['MYR'], 'SPOTRT': [1.0000000]})
    ...
    # Convert dates from SAS numeric to display format
    def format_date(val):
        if val is None or val == '':
            return ''
        try:
            # If it's a string, try to parse as SAS date
            if isinstance(val, str):
                # Check if it looks like a SAS date (digits only)
                if val.isdigit():
                    return convert_sas_date(float(val))
                return val
            # If it's numeric, convert
            if isinstance(val, (int, float)):
                return convert_sas_date(val)
            return str(val)
        except:
            return str(val)
    ...
    with open(text_path, "w") as f:
        ...
        for row in cusmyr.iter_rows(named=True):
            ...
            startdt = format_date(row.get('STARTDT', ''))
            matdt = format_date(row.get('MATDT', ''))
