...
def write_output(df, base_path, suffix=""):
    ...
    try:
        ...
        pd_df = df.to_pandas()
        # pyreadstat uses write_sas, not write_sas7bdat
...
if current.is_empty():
    ...
    # Convert PRODUCT back to numeric for filtering
    current = current.with_columns([
        pl.col("PRODUCT").cast(pl.Int64).alias("PRODUCT_NUM")
    ])
...
if not saving.is_empty():
    ...
    if not current.is_empty():
        ...
        curr293 = curr293.filter(pl.col("BRANCH").cast(pl.Int64) <= 900)
