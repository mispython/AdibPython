...
def read_sas7bdat(file_path: Path) -> pl.DataFrame:
    ...
    # Convert acctno to integer if it exists and is numeric
    if 'acctno' in pl_df.columns:
        # Try to convert to integer if it's string or float
        try:
            if pl_df['acctno'].dtype == pl.Utf8:
                pl_df = pl_df.with_columns([
                    pl.col('acctno').str.strip().cast(pl.Int64, strict=False).alias('acctno')
                ])
...
if 'acctno' in mast.columns:
    # Convert MAST acctno to Int64
    mast = mast.with_columns([
        pl.col('acctno').cast(pl.Int64).alias('acctno')
    ])

# Also ensure CRFT acctno is Int64
