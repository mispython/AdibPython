...
def generate_islamic_hp_report(df, rdate, output_path):
    ...
    with open(output_path, 'w') as f:
        ...
        for branch in branches:
            ...
            # Header - exactly matching production
            ...
            for icno in icnos:
                ...
                for row in ic_df.iter_rows(named=True):
                    ...
                    # Balance - right aligned with spaces to match production
                    # Production: spaces before the number, then comma formatted
                    balance_val = row.get('BALANCE', 0) or 0
                    # Format with commas and 2 decimals, then right-align to 14 chars
                    balance = f"{balance_val:,.2f}".rjust(14)
                    
                    # MTHARR (Pass Due) - right aligned with spaces
                    # Production has spaces before the number, then the number
                    mtharr_val = row.get('MTHARR', 0) or 0
                    mtharr = str(int(mtharr_val)).rjust(6)  # Right align to 6 chars with spaces
                    ...
                    # Format: space + ACCTNO(12) + NOTENO(6) + NAME(30) + TYPE(4) + STATUS(6) + balance(14) + mtharr(6) + DATE(8)
