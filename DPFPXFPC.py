...
# ============================================
# STEP 1: Load FDMTHLY from MNI and IMNI (lowercase)
# ============================================
...
if fdmthly_df is not None:
    ...
    print(f"  - MNI FDMTHLY dtypes: {fdmthly_df.dtypes[:5]}")
else:
...
if ifdmthly_df is not None:
    ...
    print(f"  - IMNI FDMTHLY dtypes: {ifdmthly_df.dtypes[:5]}")
else:
    ifdmthly_df = pl.DataFrame()

# Ensure both FDMTHLY dataframes have the same schema before concatenating
if not fdmthly_df.is_empty() and not ifdmthly_df.is_empty():
    # Get all columns from both dataframes
    all_cols = set(fdmthly_df.columns) | set(ifdmthly_df.columns)
    
    # Add missing columns to both
    for col in all_cols:
        if col not in fdmthly_df.columns:
            fdmthly_df = fdmthly_df.with_columns([
                pl.lit(None).cast(pl.Utf8).alias(col)
            ])
        if col not in ifdmthly_df.columns:
            ifdmthly_df = ifdmthly_df.with_columns([
                pl.lit(None).cast(pl.Utf8).alias(col)
            ])
    
    # Reorder columns to match
    cols_order = sorted(all_cols)
    fdmthly_df = fdmthly_df.select(cols_order)
    ifdmthly_df = ifdmthly_df.select(cols_order)
...
if not fdmthly_combined.is_empty():
    # Add LEDGBAL as CURBAL if CURBAL exists
    if 'curbal' in fdmthly_combined.columns:
        fdmthly_processed = fdmthly_combined.with_columns([
            pl.col('curbal').alias('ledgbal')
        ])
    else:
        fdmthly_processed = fdmthly_combined
    ...
    print(f"  - FDMTHLY dtypes: {fdmthly_processed.dtypes}")
else:
    fdmthly_processed = pl.DataFrame()
    print("  - No FDMTHLY data to save")

# ============================================
# STEP 2: Load CURN from MNI and IMNI (lowercase)
# ============================================
...
# Ensure both CURN dataframes have the same schema
if not curn1_df.is_empty() and not curn2_df.is_empty():
    all_cols = set(curn1_df.columns) | set(curn2_df.columns)
    for col in all_cols:
        if col not in curn1_df.columns:
            curn1_df = curn1_df.with_columns([
                pl.lit(None).cast(pl.Utf8).alias(col)
            ])
        if col not in curn2_df.columns:
            curn2_df = curn2_df.with_columns([
                pl.lit(None).cast(pl.Utf8).alias(col)
            ])
    cols_order = sorted(all_cols)
    curn1_df = curn1_df.select(cols_order)
    curn2_df = curn2_df.select(cols_order)
...
if not curn_combined.is_empty():
    ...
    if 'curbal' not in curn_combined.columns and 'ledgbal' in curn_combined.columns:
        curn_combined = curn_combined.with_columns([
...
# ============================================
# STEP 3: Load SAVG data and combine (lowercase)
# ============================================
...
for i, name in enumerate(dataset_names):
    ...
    print(f"       Columns: {datasets_to_combine[i].columns}")
    print(f"       Dtypes: {[str(dt) for dt in datasets_to_combine[i].dtypes]}")
...
if datasets_to_combine:
    ...
    deposit_combined = pl.concat(standardized_datasets, how="diagonal")
...
# ============================================
# STEP 8: Load FLOAT data (lowercase)
# ============================================
