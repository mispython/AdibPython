...
def load_remit(d):
    ...
    try:
        ...
        if 'paymode' in remit.columns:
            remit = remit.with_columns(pl.col('paymode').cast(pl.Utf8))
        if 'paymode' in unclaim.columns:
            unclaim = unclaim.with_columns(pl.col('paymode').cast(pl.Utf8))
...
def main():
    ...
    if not saca_df.is_empty():
        ...
        output_cols = ['branch', 'acctno', 'name', 'purpose', 'avbal', 'intpaybl', 
                      'product', 'amtind', 'plusbal', 'unclaim', 'si', 'ibgamt', 'avbaltt']
