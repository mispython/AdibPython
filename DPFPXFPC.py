...
def read_sas7bdat_in_chunks(file_path, chunk_size=CHUNK_SIZE, filter_func=None, keep_columns=None):
    ...
    Returns:
        Polars DataFrame with all chunks concatenated, or empty DataFrame with proper schema
    """
    chunks = []
    row_offset = 0
...
print(f"Available columns: {loan_base.columns[:10]}...")  # Show first 10 columns
...
if loan_base.height > 0 and "COMMNO" in loan_base.columns:
    ...
    print("Check if filters are correct or if file has data")
...
# Process COMM if we have data
if lncomm.height > 0 and "COMMNO" in lncomm.columns:
    comm = (
        lncomm
        .with_columns([
...
if loan1.height > 0 and comm.height > 0:
    ...
    print("WARNING: Cannot merge LOAN1 with COMM due to empty data")
    loan = loan0
...
if loan.height > 0:
    ...
    if coll.height > 0 and desc.height > 0:
        ...
        print("WARNING: Cannot merge COLL with DESC due to empty data")
        coll = pl.DataFrame()
    ...
    if loan.height > 0 and coll.height > 0:
        ...
        print("WARNING: Cannot create NPGS due to empty data")
        npgs = pl.DataFrame()
    ...
    # Continue only if we have NPGS data
    if npgs.height > 0:
        # =========================
