...
if data.get('imast') is not None:
    ...
    # Step 1: Filter and create basic columns
    ...
    # Step 4: Apply special sector override
    mast = mast.with_columns([
        pl.when(
...
if data.get('isuba') is not None:
    ...
    suba = suba.with_columns([
        pl.col(liabcode_col).replace_strict(LIAB_FORMAT, default=None).alias("facility"),
        pl.col(liabcode_col).replace_strict(NSRSLIAB_FORMAT, default=None).alias("faccode"),
        pl.col(liabcode_col).replace_strict(PRCTYPE_FORMAT, default="99").alias("typeprc"),
        pl.col(liabcode_col).replace_strict(PRCTYPESFS_FORMAT, default="99").alias("typeprc_sfs"),
        pl.col(liabcode_col).replace_strict(BTFCONCEPT_FORMAT, default=99).alias("fconcept")
    ])
...
if acct is not None:
    ...
    rename_dict = {k: v for k, v in output_columns.items() if k in acct.columns}
    acct_output = acct.rename(rename_dict)
