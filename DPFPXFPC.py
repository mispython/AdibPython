...
for line_num, line in enumerate(data_lines, 1):
    ...
    if len(parts) >= 3:
        glitem = parts[0]
        date_str = parts[1]  # This will be in DD/MM/YY format
        ...
        try:
            ...
            records.append({
                'GLITEM': glitem,
                'DATEX': date_str,  # Keep original format for parsing
                'SIGN': sign,
                'BALANCE': balance
            })
        except ValueError as e:
...
DETAIL = df_gl.with_columns([
    pl.col('DATEX').map_elements(lambda x: parse_date_dmy(str(x)), return_dtype=pl.Datetime).alias('DATE'),
    pl.when(pl.col('SIGN') == '-')
      .then(-pl.col('BALANCE'))
...
def build_pass(detail: pl.DataFrame, variant: int) -> pl.DataFrame:
    ...
    df = detail.with_columns([
        pl.when(pl.col("GLITEM").is_in(["49120", "49120NLF"])).then(pl.lit("A1.20"))
         ...
         .when(pl.col("GLITEM") == "137070").then(pl.lit("A2.08"))  # Islamic version
         .otherwise(pl.lit(" ")).alias("ITEM"),
        pl.when(pl.col("GLITEM").is_in([
            "49120", "49120NLF", "F143120ODNCB", "F143120ODNIB",
            "F13312002CB", "F132121BBNM", "37070", "137070"
    ...
    df = df.with_columns(
        pl.sum_horizontal([
            ...
            pl.col("MONTH").fill_null(0.0) if "MONTH" in df.columns else pl.lit(0.0),
            pl.col("QTR").fill_null(0.0) if "QTR" in df.columns else pl.lit(0.0),
            pl.col("HALFYR").fill_null(0.0) if "HALFYR" in df.columns else pl.lit(0.0),
            pl.col("YEAR").fill_null(0.0) if "YEAR" in df.columns else pl.lit(0.0),
            pl.col("LAST").fill_null(0.0) if "LAST" in df.columns else pl.lit(0.0),
            pl.col("TOTAL").fill_null(0.0) if "TOTAL" in df.columns else pl.lit(0.0),
        ]).alias("BALANCE")
    )
