...
SAS_CONFIG = {
    "sascfg": {
        "saspath": "/usr/local/SASHome/SASFoundation/9.4/bin/sas",  # Adjust path as needed
...
# Common columns needed for DCI processing
DCI_COMMON_COLS = [
    'TICKETNO', 'CUSTCODE', 'BRANCH', 'INVCURR', 'ALTCURR',
    'INVAMT', 'ALTAMT', 'TENOR', 'STATUSIND', 'DCIRT', 
    'STARTDT', 'MATDT', 'TYPE', 'ACCINTRM', 'PREMIUM'
]
...
def main():
    ...
    try:
        ...
        # Ensure INVCURAC is Int64 type for joining
    ...
    try:
        # Load MNITB Saving with Parquet caching
        ...
        print(f"  ✓ Loaded MNITB Saving: {len(mnitb_saving):,} rows (2 columns)")
        
        # Load MNITB Current with Parquet caching
        ...
        print(f"  ✓ Loaded MNITB Current: {len(mnitb_current):,} rows (2 columns)")
    except Exception as e:
    ...
    # If dp_cra is empty, create empty DataFrame with required columns
    if len(dp_cra) == 0:
        print("  Note: No CRA records with valid status, creating empty CRA DataFrame")
        dp_cra = pl.DataFrame({
            'BRANCH': [],
            'CUSTICKETNO': [],
            'INVCURAC': [],
    ...
    # Add required columns if we have data
    if len(dp_cra) > 0:
    ...
    # Join CRA with DEPO - only if both have data
    if len(dp_cra) > 0 and len(depo) > 0:
        ...
        print("  Note: No CRA or DEPO data to join, skipping CRA processing")
        dp_cra = pl.DataFrame()
    ...
    # Define common columns for DCI processing
    dci_cols = ['TICKETNO', 'CUSTCODE', 'BRANCH', 'INVCURR', 'ALTCURR',
                'INVAMT', 'ALTAMT', 'TENOR', 'STATUSIND', 'DCIRT', 
                'STARTDT', 'MATDT', 'TYPE', 'ACCINTRM', 'PREMPAID', 'PREMREC']
    
    # Align columns for both dataframes before concatenating
    if len(cusmyr) > 0 and len(ibnmyr) > 0:
        ...
        essential_cols = ['TICKETNO', 'INVCURR', 'TYPE', 'ACCINTRM', 'PREMPAID', 'PREMREC']
        cols_to_keep = [col for col in essential_cols if col in common_cols]
        
        # Select only common columns
        cusmyr_aligned = cusmyr.select(cols_to_keep)
        ibnmyr_aligned = ibnmyr.select(cols_to_keep)
    ...
    if len(dcimyr) > 0:
        # Add PREMIUM column (use PREMPAID for customer, PREMREC for interbank)
