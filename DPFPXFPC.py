...
def read_sas_columns(path, wanted_cols, required=None, disable_datetime_conversion=False,
    ...
    - Matches `wanted_cols` against the file's ACTUAL column names
      case-insensitively (fixes the silent-empty-frame bug from before).
    - Returns a DataFrame with all column names lowercased, containing only
      the requested columns that actually exist in the file.
    - `required` (default: []) lists columns that MUST exist - if any of
      those are missing, raises a clear RuntimeError listing the file's
      available columns. Any OTHER requested column that doesn't exist is
      simply skipped (with a printed notice), not treated as fatal.
      FIX: the previous version defaulted `required` to ALL of wanted_cols,
      so e.g. requesting ['acctno','noteno','sp','marketvl'] from a file
      that has acctno/noteno/iis/oi/totiis but genuinely has no sp/marketvl
      columns would abort the ENTIRE read and fall back to a completely
      empty frame - throwing away perfectly good iis/oi/totiis data along
      with the two columns that were actually absent. Now only columns you
      explicitly mark as `required` (typically join keys) can do that;
    ...
    if hard_missing:
        raise RuntimeError(
            f"Required column(s) {hard_missing} not found in {path}.\n"
    ...
    if skipped:
        print(f"  Note: {path} has no column(s) {skipped} - continuing "
              f"without them (not marked required).")

    if not resolved_cols:
        # Nothing requested actually exists in the file at all.
        return pd.DataFrame(columns=[c.lower() for c in wanted_cols])
...
def get_branch_name(branch_code):
    ...
    # FIX: pyreadstat always returns SAS numeric columns as float64, even
    # whole-number codes like branch numbers (e.g. 3.0, not 3), and missing
    # values come through as NaN. Coerce safely to int before the dict
    # lookup so branch_map.get(3.0, ...) doesn't silently miss (dict keys
    # are int 3, not float 3.0) and NaN doesn't blow up downstream formatting.
    try:
        if branch_code is None or (isinstance(branch_code, float) and pd.isna(branch_code)):
            return 'UNK'
...
def safe_float(val, default=0.0):
    """
    Coerce a value to float for numeric fixed-width fields, mapping
    NaN/inf/None to 0.0. Without this, Python's format() happily renders
    NaN as 'nan' and +/-inf as 'inf'/'-inf' (e.g.
    f'{float("inf"):>16.2f}' -> '             inf'), which silently
    corrupts the fixed-width output rather than raising - much harder to
    catch than a crash. NaN commonly appears after outer merges leave a
    column unmatched; inf commonly appears from a division by a zero
    denominator upstream (e.g. margin-of-financing when appvalue is 0).
    """
    try:
        if val is None:
            return default
...
def safe_int(val, default=0):
    """
    Coerce a value to int, tolerating the two failure modes that show up
    constantly with SAS-derived data:
    - NaN (very common after outer merges leave some columns unmatched)
    - values already typed as numpy float64 from pyreadstat
    Returns `default` (0) instead of raising, since a raised ValueError here
    would abort output generation for an otherwise-valid row over a single
    blank field.
    """
    try:
        if val is None:
...
def create_fixed_width_line(row):
    ...
    acctno = safe_float(row.get('acctno', 0))
    noteno = safe_float(row.get('noteno', 0))
    borstat = str(row.get('borstat', '') or '')[:1]
    iis = safe_float(row.get('iis', 0))
    oi = safe_float(row.get('oi', 0))
    totiis = safe_float(row.get('totiis', 0))
    sp = safe_float(row.get('sp', 0))
    curbal = safe_float(row.get('curbal', 0))
    prevbal = safe_float(row.get('prevbal', 0))
    payment = safe_float(row.get('payment', 0))
    ecsrrsrv = safe_float(row.get('ecsrrsrv', 0))
    postamt = safe_float(row.get('postamt', 0))
    otheramt = safe_float(row.get('otheramt', 0))
    ...
    marginfi = safe_float(row.get('marginfi', 0))
    noteterm = row.get('noteterm', 0) or 0
    payamt = safe_float(row.get('payamt', 0))
    ...
    line += f"{matdate:<10}{safe_int(loantype):>3d}{intamt:>16.2f}{postntrn:1}"
    line += f"{marketvl:>16.2f}{intearn4:>16.2f}{safe_int(days):>6d}{safe_int(custcode):>3d}{rind:1}"
    line += f"{oifeeamt:>16.2f}{lasttra1:<10}{safe_int(lsttrncd):>3d}{safe_int(mthpdue):>3d}"
    line += f"{balance:>16.2f}{guarend:<20}{guarnam1:<40}{guarnam2:<40}"
    line += f"{issxdte_str:<10}{netproc:>16.2f}{colldesc:<70}{safe_int(collyear):>4d}"
    line += f"{safe_int(bilpaid):>3d}{crrgrade:<5}{marginfi:>16.2f}{safe_int(noteterm):>3d}"
...
try:
    df_lnnote = read_sas_columns(
        ...
        LNNOTE_COLS_NEEDED,
        required=['borstat', 'loantype', 'acctno', 'noteno'],
...
# PERF + FIX: fully vectorized branch-string construction.
# - pyreadstat returns SAS numerics as float64 (e.g. 3.0) with NaN for
#   missing values, so we coerce safely to int first (fillna(0) -> 'UNK').
# - Series.map(dict) is a vectorized dict lookup (much faster than
#   .apply(lambda ...) row-by-row), and zero-padding is done with the
#   vectorized .str.zfill() instead of an f-string per row.
branch_map = {1: 'KL', 2: 'PJ', 3: 'JB', 4: 'PG', 5: 'IP'}
ntbrch_int = pd.to_numeric(df_npla['ntbrch'], errors='coerce').fillna(0).astype(int)
branch_abbr = ntbrch_int.map(branch_map).fillna('UNK')
branch_code_str = ntbrch_int.astype(str).str.zfill(3)
df_npla['branch'] = branch_abbr + ' ' + branch_code_str
...
if os.path.exists(credmsubac_file):
    try:
        df_credsub = read_sas_columns(
            ...
            required=['facility', 'acctnum', 'daysarr', 'noteno'],
        )
...
# FIX: np.where() evaluates BOTH branches eagerly before choosing one, so
# even for rows where payamt <= 0 (and thus we intend to use the "0" branch)
# pandas still computes (orgbal - curbal) / payamt for every row - producing
# +/-inf wherever payamt is exactly 0. The old code then called .astype(int)
# on that *entire* array before np.where discarded the unwanted values,
# so a single inf/NaN anywhere blew up the whole cast with
# "IntCastingNaNError: Cannot convert non-finite values (NA or inf) to
# integer" - even on rows we never intended to use that value for.
# Fix: compute the ratio with divide-by-zero warnings suppressed, replace
# any resulting inf/-inf/NaN with 0 BEFORE casting to int, then apply the
# payamt>0 mask.
with np.errstate(divide='ignore', invalid='ignore'):
    bilpaid_ratio = (df_loan['orgbal'] - df_loan['curbal']) / df_loan['payamt']
bilpaid_ratio = bilpaid_ratio.replace([np.inf, -np.inf], 0).fillna(0)
df_loan['bilpaid'] = np.where(
    df_loan['payamt'] > 0,
    bilpaid_ratio.astype(int),
    0
)
...
# FIX: the actual file naming convention is lnliab{reptmon}{nowk}{reptyear}.sas7bdat
# (e.g. lnliab070326.sas7bdat for month=07, week=3, year=26), not a static
# "liab.sas7bdat" - it follows the same weekly/monthly pattern as the other
# period-stamped inputs (credmsubac, loan{reptmon1}{nowks}, etc.).
liab_file = f'{LOAN_DIR}lnliab{reptmon}{nowk}{reptyear}.sas7bdat'
try:
    df_liab = read_sas_columns(
        liab_file,
        ['acctno', 'noteno', 'liabacct', 'liabname'],
        required=['acctno', 'noteno', 'liabacct'],
    )
    ...
    print(f"Error reading liability data ({liab_file}): {e}")
    guarantor_data = {}
