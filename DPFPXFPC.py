...
def eimhptop():
    ...
    try:
        ...
        print(f"HPACC columns: {hpacc_df.columns}")
        print(f"PRODUCT column type: {hpacc_df['PRODUCT'].dtype}")
        print(f"PRODUCT sample values: {hpacc_df['PRODUCT'].head(5).to_list()}")
    except Exception as e:
        print(f"File not found or error reading: LOAN/LOAN{reptmon}{wk}.sas7bdat - {e}")
        return
    
    # Convert PRODUCT to string if it's not already
    if hpacc_df["PRODUCT"].dtype != pl.Utf8:
        hpacc_df = hpacc_df.with_columns(
            pl.col("PRODUCT").cast(pl.Utf8).alias("PRODUCT")
        )
