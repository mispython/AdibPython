...
if datasets_to_combine:
    ...
    if 'progcd' not in deposit_combined.columns:
        ...
        if not deposit_filtered.is_empty():
            # IMPORTANT: Keep all records, don't deduplicate
            # The SAS code doesn't deduplicate DEPOSIT
...
if not deposit_filtered.is_empty() and not float_summary.is_empty():
    ...
    merged = deposit_sorted.join(
        float_summary, 
        on='acctno', 
        how='outer',  # Equivalent to SAS MERGE with IN= options
        suffix='_float'
    )
    ...
    # B = float is not null, A = deposit is not null (check curbal or product)
