...
def eiihptop():
    ...
    reptmon = f"{mm:02d}"
    reptmon1 = f"{mm1:02d}"
    rdate = reptdate.strftime("%d/%m/%y")  # Changed to match production format
    ...
    # 7. Generate Islamic bank report with production formatting
...
def generate_islamic_hp_report(df, rdate, output_path):
    """Generate formatted report matching production output"""
    if df.is_empty():
    ...
    with open(output_path, 'w') as f:
        ...
        for branch in branches:
            ...
            # Header - matching production exactly
            f.write(f" PUBLIC ISLAMIC BANK BERHAD                                                      {rdate}  PAGE NO : {page_num}\n")
            f.write(f" TOP TEN LARGE ACCOUNTS FOR HPD (CONVENTIONAL & AITAB) AS AT {rdate}\n")
            f.write(f" REPORT ID: EIMHPTOP\n")
            
            # Branch code - with 0 prefix as in production
            if isinstance(branch, float) and branch.is_integer():
                branch_code = int(branch)
            else:
                branch_code = int(branch) if branch is not None else 0
            
            f.write(f"0BRANCH CODE= {branch_code:03d}\n")
            
            # Column headers - matching production spacing
            f.write(f"0            NOTE                                 LOAN  BORROWER                  MONTH      ISSUE\n")
            f.write(f" MNI NO      NO     NAME                          TYPE  STATUS        NET BALANCE PASS DUE   DATE\n")
            f.write(f" --------------------------------------------------------------------------------------------------------\n")
            ...
            for icno in icnos:
                ...
                for row in ic_df.iter_rows(named=True):
                    # Format values - removing decimals from account numbers
                    acctno_raw = str(row.get('ACCTNO', '') or '')
                    # Remove decimal if present (e.g., "2000079202.0" -> "2000079202")
                    if '.' in acctno_raw:
                        acctno = acctno_raw.split('.')[0].ljust(12)
                    else:
                        acctno = acctno_raw.ljust(12)
                    
                    noteno_raw = str(row.get('NOTENO', '') or '')
                    if '.' in noteno_raw:
                        noteno = noteno_raw.split('.')[0].ljust(6)
                    else:
                        noteno = noteno_raw.ljust(6)
                    ...
                    if '.' in product_raw:
                        product = product_raw.split('.')[0].ljust(4)
                    else:
                        product = product_raw.ljust(4)
                    ...
                    # MTHARR (Pass Due) - integer, no decimals
                    mtharr_val = row.get('MTHARR', 0) or 0
                    mtharr = f"{int(mtharr_val):2d}"
                    ...
                    # Write the detail line - matching production spacing
                ...
                # ICNO total - matching production
                f.write(f"                                                                 ----------------\n")
                f.write(f"                                                          TOTAL: {ic_total:13,.2f}\n")
                f.write(f"                                                                 ================\n")
            
            # Branch total - matching production
            f.write(f"                                                                 ----------------\n")
