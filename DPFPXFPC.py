...
def format_acctno(acctno):
    """Format ACCTNO to a consistent string format for matching"""
    if acctno is None or (isinstance(acctno, float) and pd.isna(acctno)):
        return None
    # Convert to string, remove decimal if present
    if isinstance(acctno, float):
        if acctno.is_integer():
            return str(int(acctno))
        else:
            return str(acctno).replace('.', '')
    return str(acctno)
...
if float_df is not None:
    ...
    print(f"  - FLOAT sample ACCTNO: {float_df['acctno'].head(10)}")
    ...
    if not float_df.is_empty() and 'float' in float_df.columns and 'acctno' in float_df.columns:
        # Create FLOAT_SUMMARY with ACCTNO as string for better matching
        ...
        if not deposit_filtered.is_empty():
            deposit_acctnos = deposit_filtered['acctno'].unique().head(10)
            float_acctnos = float_summary['acctno'].unique().head(10)
            print(f"  - Sample DEPOSIT ACCTNO: {deposit_acctnos}")
            print(f"  - Sample FLOAT ACCTNO: {float_acctnos}")
            ...
            if len(common) > 0:
                print(f"  - Sample common ACCTNO: {list(common)[:5]}")
    else:
...
if not deposit_filtered.is_empty():
    ...
    if not float_summary.is_empty():
        ...
        if float_not_null.height > 0:
            print(f"  - Sample FLOAT matches: {float_not_null.select(['acctno', 'float']).head(10)}")
