...
# ==================================================
# Import PBBDPFMT.py and inspect available formats
# ==================================================
def load_pbbdpfmt():
    ...
    print("Available attributes in PBBDPFMT:")
    for attr in dir(pbbdpfmt):
        if not attr.startswith('_'):
            obj = getattr(pbbdpfmt, attr)
            if isinstance(obj, dict):
                print(f"  {attr}: dict with {len(obj)} items")
            elif callable(obj):
...
# Look for format dictionaries - they might be nested or named differently
# Based on the output we saw, try to find the format mappings
def find_format_dict(module, format_name):
    """Find a format dictionary in the module, checking various possibilities"""
    # Check direct attribute
    if hasattr(module, format_name):
        return getattr(module, format_name)
    
    # Check if there's a formats dictionary
    if hasattr(module, 'formats') and isinstance(module.formats, dict):
        if format_name in module.formats:
            return module.formats[format_name]
    
    # Check if there's a get_format function
    if hasattr(module, 'get_format') and callable(module.get_format):
        try:
            return module.get_format(format_name)
        except:
            pass
    
    # Check if apply_format function exists with different signature
    if hasattr(module, 'apply_format') and callable(module.apply_format):
        # Try to use apply_format directly
        return module.apply_format
    
    # Print all dict attributes for debugging
    print(f"\nCould not find {format_name}. Available dict attributes:")
    for attr in dir(module):
        if not attr.startswith('_'):
            obj = getattr(module, attr)
            if isinstance(obj, dict):
                print(f"  {attr}: {list(obj.keys())[:5]}...")
    
    return None


# Try to get format dictionaries - adapt based on what's available
SAPROD = find_format_dict(pbbdpfmt, 'SAPROD')
CAPROD = find_format_dict(pbbdpfmt, 'CAPROD')
FDPROD = find_format_dict(pbbdpfmt, 'FDPROD')

# If formats are callable (functions), we'll use them directly
# If they're dictionaries, we'll use the lookup method
...
print(f"\nProcessing for month: {REPTMON} (Report Date: {REPTDATE.strftime('%Y-%m-%d')})")
...
# Apply SAPROD format
if SAPROD is not None:
    SA = apply_sas_format(SA, source_col="PRODUCT", format_dict=SAPROD, out_col="PRODCD")
else:
    print("WARNING: SAPROD format not found, creating PRODCD as empty string")
    SA = SA.with_columns(pl.lit("").alias("PRODCD"))
...
# Apply CAPROD format
if CAPROD is not None:
    CA = apply_sas_format(CA, source_col="PRODUCT", format_dict=CAPROD, out_col="PRODCD")
else:
    print("WARNING: CAPROD format not found, creating PRODCD as empty string")
    CA = CA.with_columns(pl.lit("").alias("PRODCD"))
...
# Apply FDPROD format
if FDPROD is not None:
    FDCD = apply_sas_format(FDCD, source_col="INTPLAN", format_dict=FDPROD, out_col="PRODCD")
else:
    print("WARNING: FDPROD format not found, creating PRODCD as empty string")
    FDCD = FDCD.with_columns(pl.lit("").alias("PRODCD"))
