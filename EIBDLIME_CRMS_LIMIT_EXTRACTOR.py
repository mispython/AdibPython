...
# Step 4: Create final limit datasets
limit_columns = [
...
# Save outputs
pq.write_table(final_df, limit_path / f"LM_{rept_dt}.parquet")
csv.write_csv(final_df, limit_path / f"LM_{rept_dt}.csv")
pq.write_table(final_df, limit_path / f"LM{rdate}.parquet")
csv.write_csv(final_df, limit_path / f"LM{rdate}.csv")

# Distribute to CRMS and PBB
pq.write_table(final_df, crms_output_path / f"LM_{rept_dt}.parquet")
pq.write_table(final_df, pbb_output_path / f"LM{rdate}.parquet")

print(f"Completed: {len(final_df)} limit records processed")
conn.close()
