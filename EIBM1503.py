"""
EIMB1503 - NPL Financial Management Report (Production Ready)

Purpose:
- Generate Financial Management (FM) NPL reports for PBB
- Process current and previous month NPL data
- Calculate averages and accumulate totals
- Produce 3 output files: Count, Amount, Average

Key Processing:
1. Read current month NPL data (NPL file)
2. Read previous month NPL data (XNPL file)
3. Map products to LOB (Line of Business) via ODFMT/LNFMT
4. Handle special mappings (PCCC → DRFMT, TF accounts, HPOP)
5. Calculate averages: (Current + Previous) / 2
6. Generate 3 FM files: NPLC (count), NPLA (amount), NPLV (average)

Account Types:
- OD: Overdraft (3000000000-3999999999)
- LN: Term Loans (others)
- TF: Trade Finance (2500000000-2599999999)
...
Output Files:
- NPLC: NPL count by branch/product/LOB
- NPLA: NPL amount by branch/product/LOB
- NPLV: NPL average (current + previous)/2 + accumulative average
...
print("EIMB1503 - NPL Financial Management Report")
print("=" * 60)
...
try:
    ...
    # If current month = today's month, use previous month
...
print("\nReading previous month NPL (XNPL)...")
try:
    xnpl_data = []
...
try:
    ...
    with open(f'{INPUT_DIR}NPL', 'r') as f:
        ...
        for line in f:
            ...
            # Skip if amount=0 or class='P'
            if amount == 0 or class_code == 'P':
                continue
            
            # Adjust branch (subtract 3000 if > 3000)
            if branch > 3000:
                branch = branch - 3000
            
            # Determine BRANCX
            if 800 <= branch < 900:
                # Use HPCC format (HP cost center)
...
# Read product mapping (ODFMT, LNFMT)
print("\nReading product mappings...")

# ODFMT (Overdraft products)
try:
    odfmt_data = []
    ...
    print(f"  ✓ ODFMT: {len(df_odfmt):,} products")
except:
...
# Read DRFMT (branch LOB mapping for PCCC)
print("\nReading DRFMT (branch LOB mapping)...")
try:
    drfmt_data = []
    ...
    print(f"  ✓ DRFMT: {len(df_drfmt):,} branch mappings")
except:
    df_drfmt = pl.DataFrame([])

# Split NPL into PCCC and others
...
# Summarize by BRANCX, PRODUCX, LOB
print("\nSummarizing NPL data...")
...
print(f"  ✓ NPL summary: {len(df_npls):,} records")

# Generate NPLC (Count file)
print("\nGenerating NPLC (count file)...")
...
with open(f'{OUTPUT_DIR}NPLC', 'w') as f:
    for row in df_npls.iter_rows(named=True):
        line = (
            f"D,PBB{'':65}C_NPL{'':197}{row['YM']}{row['NOACC']:>15}"
...
print(f"  ✓ NPLC: {lcnt:,} records, Total Count: {totcnt:,}")

# Generate NPLA (Amount file)
print("\nGenerating NPLA (amount file)...")
totamt = df_npls['AMOUNT'].sum()

with open(f'{OUTPUT_DIR}NPLA', 'w') as f:
    for row in df_npls.iter_rows(named=True):
        line = (
            f"D,PBB{'':65}B_NPL{'':197}{row['YM']}{row['AMOUNT']:>15.2f}"
            f"{row['LOB']:>4}{row['PRODUCX']:>6}EXT{'':65}ACTUAL{'':65}"
            f"{row['BRANCX']:>4},{'':66},{'':32},MYR{'':32},"
            f"{'':32},{'':32},{'':32},Y,{'':32},{'':32},{'':32},;"
        )
...
print(f"  ✓ NPLA: {lcnt:,} records, Total Amount: RM {totamt:,.2f}")

# Generate NPLV (Average file)
print("\nGenerating NPLV (average file)...")
...
with open(f'{OUTPUT_DIR}NPLV', 'w') as f:
    for row in df_nplv.iter_rows(named=True):
        # AVG line
        line1 = (
            f"D,PBB{'':65}B_NPL{'':197}{row['YM']}{row['AVGAMR']:>15.2f}"
...
print(f"  ✓ NPLV: {lcnt_v:,} records, Total Average: RM {tavg:,.2f}")

print(f"\n{'='*60}")
print(f"✓ EIMB1503 Complete!")
print(f"{'='*60}")
print(f"\nFinancial Management NPL Report Summary:")
...
print(f"\nFile Format: FM Standard (comma-separated, fixed positions)")
print(f"  Entity: PBB")
print(f"  Measure: C_NPL (count), B_NPL (amount/average)")
print(f"  Period: {ym}")
print(f"  Currency: MYR")
