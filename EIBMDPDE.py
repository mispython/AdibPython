...
# Extract and convert packed decimal at position 34 (1-indexed) = 33 (0-indexed)
try:
    # Get 6 bytes at position 34
    tbdate_bytes = record[33:39]
    
    # Unpack packed decimal (PD6.) - each nibble is a digit except last low nibble (sign)
    digits = []
    for i, b in enumerate(tbdate_bytes):
        high, low = (b >> 4) & 0x0F, b & 0x0F
        digits.append(str(high))  # High nibble always digit
        if i < 5:  # First 5 bytes: low nibble is digit
            digits.append(str(low))
    
    # Convert to Z11. format (11 digits, zero-filled)
    tbdate_z11 = ''.join(digits)[:11].zfill(11)
    
    # Parse first 8 chars as MMDDYYYY
