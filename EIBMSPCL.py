"""
EIBMSPCL - Special Mention (IL) Financial Management Report (Production Ready)

Purpose:
- Generate Financial Management (FM) Impaired Loans (IL) reports for PBB
- Same as EIMB1503 but for Special Mention (IL) instead of NPL
- Process current and previous month IL data
- Calculate averages and accumulate totals
- Produce 3 output files: Count, Amount, Average

Key Differences from EIMB1503:
- Measure codes: C_IL (count), B_IL (balance) instead of C_NPL, B_NPL
- Same processing logic, different measure type
- IL = Impaired Loans (Special Mention accounts)
...
Output Files:
- NPLC: IL count by branch/product/LOB (C_IL measure)
- NPLA: IL amount by branch/product/LOB (B_IL measure)
- NPLV: IL average (current + previous)/2 + accumulative average
...
print("EIBMSPCL - Special Mention (IL) Financial Management Report")
print("=" * 60)
...
try:
    ...
    print(f"Measure: IL (Impaired Loans - Special Mention)")
...
# Read previous month IL (XNPL)
print("\nReading previous month IL (XNPL)...")
try:
    xnpl_data = []
    ...
    print(f"  ✓ Previous month IL: {len(df_xnpl):,} records")

except Exception as e:
    print(f"  ⚠ XNPL: {e}")
    df_xnpl = pl.DataFrame([])

# Read current month IL
print("\nReading current month IL...")
try:
    npl_data = []
    ...
    print(f"  ✓ Current month IL: {len(df_npl):,} records")
...
print(f"  ✓ IL summary: {len(df_npls):,} records")

# Generate NPLC (Count - C_IL)
print("\nGenerating NPLC (count file - C_IL)...")
...
with open(f'{OUTPUT_DIR}NPLC', 'w') as f:
    for row in df_npls.iter_rows(named=True):
        line = (
            f"D,PBB{'':65}C_IL{'':196}{row['YM']}{row['NOACC']:>15}"
...
# Generate NPLA (Amount - B_IL)
print("\nGenerating NPLA (amount file - B_IL)...")
totamt = df_npls['AMOUNT'].sum()

with open(f'{OUTPUT_DIR}NPLA', 'w') as f:
    for row in df_npls.iter_rows(named=True):
        line = (
            f"D,PBB{'':65}B_IL{'':196}{row['YM']}{row['AMOUNT']:>15.2f}"
            f"{row['LOB']:>4}{row['PRODUCX']:>6}EXT{'':65}ACTUAL{'':65}"
            f"{row['BRANCX']:>4},{'':66},{'':32},MYR{'':32},"
            f"{'':32},{'':32},{'':32},Y,{'':32},{'':32},{'':32},;"
        )
...
# Generate NPLV (Average - B_IL)
print("\nGenerating NPLV (average file - B_IL)...")
...
with open(f'{OUTPUT_DIR}NPLV', 'w') as f:
    for row in df_nplv.iter_rows(named=True):
        # AVG
        line1 = (
            f"D,PBB{'':65}B_IL{'':196}{row['YM']}{row['AVGAMR']:>15.2f}"
...
print(f"\n{'='*60}")
print(f"✓ EIBMSPCL Complete!")
print(f"{'='*60}")
print(f"\nSpecial Mention (IL) FM Report Summary:")
print(f"  Period: {rmonth}/{ryear}")
print(f"  Measure: IL (Impaired Loans)")
print(f"\nOutput Files:")
print(f"  NPLC (Count):   {lcnt:,} records (C_IL), Total: {totcnt:,}")
print(f"  NPLA (Amount):  {lcnt:,} records (B_IL), Total: RM {totamt:,.2f}")
print(f"  NPLV (Average): {lcnt_v:,} records (B_IL), Total: RM {tavg:,.2f}")
print(f"\nFile Format: FM Standard")
print(f"  Entity: PBB")
print(f"  Measure: C_IL (count), B_IL (balance)")
print(f"  Period: {ym}")
print(f"  Currency: MYR")
print(f"\nDifference from EIMB1503:")
print(f"  EIMB1503: NPL (C_NPL, B_NPL)")
print(f"  EIBMSPCL: IL (C_IL, B_IL) - Special Mention/Impaired Loans")
