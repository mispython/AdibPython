...
DPLP = INPUT_DIR / 'dplp' / 'dplp{mon}.parquet'
LNLP = INPUT_DIR / 'lnlp{ver}' / 'lntrax{year}{mon}.parquet'
DPCC = INPUT_DIR / 'dpcc' / 'dpcc{mon}.parquet'
CTCS = INPUT_DIR / 'ctcs' / 'ctcs{mon}.parquet'
...
CONV_REPORT = OUTPUT_DIR / 'loan_repayment_conv.txt'
ISLAMIC_REPORT = OUTPUT_DIR / 'loan_repayment_islamic.txt'
...
# DESC format (Conventional loan products)
con.execute("""CREATE TEMP TABLE desc_fmt AS SELECT * FROM (VALUES
    (4,'HL'),(5,'HL'),(6,'HL'),(7,'HL'),(60,'HL'),(61,'HL'),(62,'HL'),(70,'HL'),
    (200,'HL'),(201,'HL'),(202,'HL'),(203,'HL'),(204,'HL'),(205,'HL'),(600,'HL'),(911,'HL'),
    (15,'VL'),(20,'VL'),(71,'VL'),(72,'VL'),(380,'VL'),(381,'VL'),(700,'VL'),(705,'VL'),(720,'VL'),(725,'VL'),
    ...
    (364,'PL'),(365,'PL'),(391,'PL'),(303,'PL'),(306,'PL'),(307,'PL'),(308,'PL'),(311,'PL'),(313,'PL'),(367,'PL'),(369,'PL')
) AS t(loantype, prod)""")

# IDESC format (Islamic loan products)
con.execute("""CREATE TEMP TABLE idesc_fmt AS SELECT * FROM (VALUES
    (102,'HL'),(105,'HL'),(106,'HL'),(110,'HL'),(111,'HL'),(112,'HL'),(113,'HL'),(114,'HL'),(115,'HL'),
    (116,'HL'),(117,'HL'),(118,'HL'),(119,'HL'),(124,'HL'),(139,'HL'),(142,'HL'),(145,'HL'),
    (150,'HL'),(151,'HL'),(152,'HL'),(156,'HL'),(170,'HL'),(175,'HL'),(400,'HL'),(409,'HL'),
    (410,'HL'),(412,'HL'),(413,'HL'),(414,'HL'),(415,'HL'),(423,'HL'),(466,'HL'),(469,'HL'),
    (650,'HL'),(651,'HL'),(664,'HL'),
    (103,'VL'),(104,'VL'),(107,'VL'),(108,'VL'),(128,'VL'),(130,'VL'),(131,'VL'),(132,'VL'),(196,'VL'),
    (122,'PL'),(135,'PL'),(136,'PL'),(137,'PL'),(138,'PL'),(194,'PL'),(195,'PL'),
    (419,'PL'),(420,'PL'),(422,'PL'),(424,'PL'),(426,'PL'),(464,'PL'),(465,'PL'),(468,'PL'),
    (652,'PL'),(653,'PL'),(668,'PL'),(669,'PL'),(672,'PL'),(673,'PL'),(674,'PL'),(675,'PL')
) AS t(loantype, prod)""")

# ============================================================================
# Get REPTDATE and calculate periods
# ============================================================================
reptdate_df = pl.read_parquet(LOAN_REPTDATE)
reptdate = reptdate_df['reptdate'][0]

# Calculate previous months
prvrptdate = reptdate.replace(day=1) - relativedelta(days=1)
pprvrptdate = prvrptdate.replace(day=1) - relativedelta(days=1)

reptmon = f"{reptdate.month:02d}"
reptyear = f"{reptdate.year % 100:02d}"
rdate = reptdate.strftime('%d%m%y')

preptmon = f"{prvrptdate.month:02d}"
preptyear = f"{prvrptdate.year % 100:02d}"

ppreptmon = f"{pprvrptdate.month:02d}"
ppreptyear = f"{pprvrptdate.year % 100:02d}"

print(f"Loan Repayment Report - {rdate}")
print(f"Current: {reptmon}/{reptyear}, Prev: {preptmon}/{preptyear}, Prev2: {ppreptmon}/{ppreptyear}")

# ============================================================================
# Load DPLP (Deposit Loan Payment)
# ============================================================================
dplp_files = [str(DPLP).format(mon=m) for m in [reptmon, preptmon, ppreptmon]]
dplp_query = " UNION ALL ".join([f"SELECT * FROM read_parquet('{f}')" for f in dplp_files if Path(f).exists()])
con.execute(f"CREATE TEMP TABLE dplp AS {dplp_query} ORDER BY acctno, trandt, tranamt")

# ============================================================================
# Load LNLP (Loan Payment transactions)
# ============================================================================
lnlp_files = []
for year, mon_prefix in [(reptyear, reptmon), (preptyear, preptmon), (ppreptyear, ppreptmon)]:
    ver = '1' if year == reptyear else '2' if year == preptyear else '3'
    for week in ['01', '02', '03', '04']:
        lnlp_files.append(str(LNLP).format(ver=ver, year=year, mon=mon_prefix + week))

lnlp_query = " UNION ALL ".join([f"SELECT *, reptdate as trandt FROM read_parquet('{f}')" 
                                  for f in lnlp_files if Path(f).exists()])
con.execute(f"CREATE TEMP TABLE lnlp AS {lnlp_query} ORDER BY acctno, trandt, tranamt")

# ============================================================================
# Match LNLP and DPLP (Cash transactions)
# ============================================================================
con.execute("""
...
# Classify as conventional or Islamic
con.execute("""
...
# ============================================================================
dpcc_files = [str(DPCC).format(mon=m) for m in [reptmon, preptmon, ppreptmon]]
dpcc_query = " UNION ALL ".join([f"SELECT * FROM read_parquet('{f}')" for f in dpcc_files if Path(f).exists()])
con.execute(f"CREATE TEMP TABLE otccc AS SELECT *, 'CC' as prod, 'CASH' as item, 1 as seq FROM ({dpcc_query})")

# ============================================================================
# Load CTCS (Cheque Transactions)
# ============================================================================
ctcs_files = [str(CTCS).format(mon=m) for m in [reptmon, preptmon, ppreptmon]]
ctcs_query = " UNION ALL ".join([f"SELECT * FROM read_parquet('{f}')" for f in ctcs_files if Path(f).exists()])
con.execute(f"CREATE TEMP TABLE ctcs_base AS {ctcs_query} WHERE ind != 'OC' ORDER BY acctno, noteno")
...
if Path(RENLN_TXT).exists():
    con.execute(f"""
        ...
        SELECT 
            ...
            CAST(SUBSTRING(line, 19, 3) AS INTEGER) as loantype,
            TRY_STRPTIME(SUBSTRING(line, 23, 8), '%d%m%y') as trandt,
            CAST(SUBSTRING(line, 29, 11) AS BIGINT) as acctno,
            CAST(SUBSTRING(line, 41, 17) AS DECIMAL(19,2)) as tranamt,
...
con.execute("""
    ...
    SELECT *, tranamt / 1000.0 as tranamt1 FROM (
        ...
        UNION ALL SELECT acctno, trandt, tranamt, prod, item, seq FROM otccc
        UNION ALL SELECT acctno, trandt, tranamt, prod, item, seq FROM ctcs_conv
...
con.execute("""
    ...
    SELECT *, tranamt / 1000.0 as tranamt1 FROM (
        ...
        UNION ALL SELECT acctno, trandt, tranamt, prod, item, seq FROM ctcs_islamic
