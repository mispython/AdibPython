...
# Configuration
mni_path = Path("/sas/python/virt_edw/Data_Warehouse/MIS/XMIS/input/prod/MNI")import polars as pl
import duckdb
from pathlib import Path
import datetime
import pyreadstat 
...
try:
    ...
    if not ibg_file_path.exists():
        ...
        ibg_parsed = ibg_df.with_columns([
            pl.col('raw_line').str.slice(0, 10).str.strip().alias('PAYMODE'),
            pl.col('raw_line').str.slice(11, 16).str.strip().cast(pl.Float64).alias('IBGAMT')
        ]).drop('raw_line')
    ...
    # Fallback if str.strip() doesn't work
    print(f"AttributeError with str.strip(): {e}")
    print("Trying alternative approach without strip...")
    ...
    try:
        ...
        # Alternative: Use str.replace() to remove whitespace
        ibg_parsed = ibg_df.with_columns([
        ...
        print(f"IBG records (alternative): {ibg_valid.height}")
        print(f"NONDEBIT records (alternative): {nondebit_invalid.height}")
        
        if not ibg_parsed.is_empty():
            ibg_parsed.write_csv(output_path / "IBGPIDM_PARSED_ALT.csv")
...
try:
    ...
    if not ibg_file_path.exists():
        ...
        # Parse fixed-width format:
        # @01 PAYMODE $10., @12 IBGAMT 16.2
        # FIX: Use strip_chars() or handle string cleaning differently
        ibg_parsed = ibg_df.with_columns([
            # Method 1: Use str.strip() - if this doesn't work, try str.strip_chars()
            pl.col('raw_line').str.slice(0, 10).str.strip().alias('PAYMODE'),
            # For IBGAMT, cast to float after stripping
            pl.col('raw_line').str.slice(11, 16).str.strip().cast(pl.Float64).alias('IBGAMT')
        ]).drop('raw_line')
    ...
    # If str.strip() doesn't work, try alternative approach
    print(f"AttributeError with str.strip(): {e}")
    print("Trying alternative approach without strip...")
    
    try:
        # Read the file again with a different approach
        ibg_file_path = mni_path / "IBG_YEAREND.txt"
        ibg_df = pl.read_csv(ibg_file_path, 
                            has_header=False,
                            new_columns=['raw_line'])
