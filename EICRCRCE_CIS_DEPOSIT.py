# Program: EICRCRCE_CIS_DEPOSIT
# Purpose: Consolidate CIS Deposit + Customer + Tax + Address datasets
...
print("EICRCRCE Report Date:", batch_date, "Tag:", tag)

# -----------------------------
# Step 2. Register Parquet Inputs
# (Assume fixed-width to Parquet conversion already done upstream)
# -----------------------------
...
# Rename columns per SAS (CUSTCONS → CUSTCONSENT, CONS_OLD → CONSENT_OLD)
duckdb.sql("""
    CREATE TABLE CISDP1 AS
    SELECT *,
           CUSTCONS    AS CUSTCONSENT,
           CONS_OLD    AS CONSENT_OLD
    FROM CISDP0
""")
...
# -----------------------------
duckdb.sql("""
    CREATE TABLE CISDEPOSIT_FINAL AS
...
# Step 6. Save Output
# -----------------------------
result = duckdb.sql("SELECT * FROM CISDEPOSIT_FINAL").arrow()

os.makedirs("output", exist_ok=True)
out_parquet = f"output/CIS_DEPOSIT_{tag}.parquet"
out_csv = f"output/CIS_DEPOSIT_{tag}.csv"

pq.write_table(result, out_parquet)
with open(out_csv, "wb") as f:
    pacsv.write_csv(result, f)

print(f"EICRCRCE_CIS_DEPOSIT completed.\nParquet: {out_parquet}\nCSV: {out_csv}")
