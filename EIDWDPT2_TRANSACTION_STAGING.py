#!/usr/bin/env python3
"""
EIDWDPT2 - TRANSACTION STAGING DATA PROCESSING
1:1 CONVERSION FROM SAS TO PYTHON
PROCESSES DPPSTRG2 TRANSACTION DATA WITH COMPLEX POSITIONAL INPUT LOGIC
...
# INPUT FILES
DPPSTRG2_FILE = INPUT_DIR / "dppstrg2.parquet"

# OUTPUT FILES
OUTPUT_FILE = STG_DIR / "STG_DP_DPPSTRG2.parquet"
OUTPUT_CSV = STG_DIR / "STG_DP_DPPSTRG2.csv"

# CONNECT TO DUCKDB
con = duckdb.connect(":memory:")

print("PROCESSING EIDWDPT2 - TRANSACTION STAGING DATA (DPPSTRG2)")
...
con.execute(f"""
    CREATE OR REPLACE TABLE DPPSTRG2_RAW AS
    SELECT *
    FROM read_parquet('{DPPSTRG2_FILE}')
    WHERE BANKNO = 33 
        AND FMTCODE IN (1, 2, 6, 8)
""")

TOTAL_RECORDS = con.execute("SELECT COUNT(*) FROM DPPSTRG2_RAW").fetchone()[0]
...
# ============================================================================
DF = pl.from_arrow(con.execute("SELECT * FROM DPPSTRG2_RAW").arrow())
...
print("\nNOTE: EIDWDPT2 PROCESSES DPPSTRG2 FILE")
print("      (IDENTICAL LOGIC TO EIDWDPT1 BUT DIFFERENT SOURCE FILE)")
