#!/usr/bin/env python3
"""
EIDWDPT3 - TRANSACTION STAGING DATA PROCESSING
1:1 CONVERSION FROM SAS TO PYTHON
PROCESSES DPPSTRG3 TRANSACTION DATA WITH COMPLEX POSITIONAL INPUT LOGIC
...
# INPUT FILES
DPPSTRG3_FILE = INPUT_DIR / "dppstrg3.parquet"

# OUTPUT FILES
OUTPUT_FILE = STG_DIR / "STG_DP_DPPSTRG3.parquet"
OUTPUT_CSV = STG_DIR / "STG_DP_DPPSTRG3.csv"

# CONNECT TO DUCKDB
con = duckdb.connect(":memory:")

print("PROCESSING EIDWDPT3 - TRANSACTION STAGING DATA (DPPSTRG3)")
...
con.execute(f"""
    CREATE OR REPLACE TABLE DPPSTRG3_RAW AS
    SELECT *
    FROM read_parquet('{DPPSTRG3_FILE}')
    WHERE BANKNO = 33 
        AND FMTCODE IN (1, 2, 6, 8)
""")

TOTAL_RECORDS = con.execute("SELECT COUNT(*) FROM DPPSTRG3_RAW").fetchone()[0]
...
# ============================================================================
DF = pl.from_arrow(con.execute("SELECT * FROM DPPSTRG3_RAW").arrow())
...
print("\nNOTE: EIDWDPT3 PROCESSES DPPSTRG3 FILE")
print("      (IDENTICAL LOGIC TO EIDWDPT1/EIDWDPT2 BUT DIFFERENT SOURCE FILE)")
