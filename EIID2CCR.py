#!/usr/bin/env python3
"""
EIID2CCR - Islamic Daily Credit Report
Generates daily credit file for Islamic loans with overdraft days and payment tracking
"""
...
print(f"Islamic Daily Credit Report - {reptdate.strftime('%d/%m/%Y')}")
...
con.execute(f"""
    ...
    FROM read_parquet('{INPUT_DIR}/loan/iln{reptyear:02d}{reptmon:02d}{reptday:02d}.parquet')
""")
...
with open(OUTPUT_DIR / 'daycred_islamic.txt', 'w') as f:
...
con.execute(f"COPY daycred TO '{OUTPUT_DIR}/daycred_islamic_{rdate}.parquet'")
con.execute(f"COPY daycred TO '{OUTPUT_DIR}/daycred_islamic_{rdate}.csv' (HEADER, DELIMITER ',')")

record_count = con.execute("SELECT COUNT(*) FROM daycred").fetchone()[0]

print(f"""
Islamic Daily Credit Report
Output: {record_count} records
Files: daycred_islamic.txt (127 bytes/record)
       daycred_islamic_{rdate}.parquet
       daycred_islamic_{rdate}.csv
