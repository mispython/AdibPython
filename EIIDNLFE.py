"""
EIIDNLFE - Islamic BNM Behavioral Analysis with Istisna (Production Ready)

Same as EIIMNLFE but with Istisna processing (commented out per ESMR 2013-403)

Key Features:
- EXCLUDES Istisna (9532908/9532909) from behavioral analysis
- CM remapped (9531708/9→9531508/9)
- Istisna remapped to FD in BASE.DEPOSIT (9532908/9→9531508/9)
- Only 6 products (RM only)
- Istisna sections commented out (historical reference)

Note: Code includes commented Istisna sections per ESMR 2013-403 revision.
These are preserved for audit trail but not executed.

6 Product Categories:
- INDRMDD/NONRMDD/INDRMFD/NONRMFD/INDRMSA/NONRMSA (RM only)
...
print("EIIDNLFE - Islamic BNM Behavioral Analysis with Istisna")
print("=" * 60)
...
# Product definitions (6 products - RM only)
PRODUCTS = [
    'INDRMDD', 'INDRMFD', 'INDRMSA',
    'NONRMDD', 'NONRMFD', 'NONRMSA'
]

# BNM code mappings (Islamic)
BNMCODE_MAP = {
    '9531508': 'INDRMFD',  # Islamic FD
    '9531509': 'NONRMFD',
    '9531708': 'INDRMFD',  # CM → merge to FD
    '9531709': 'NONRMFD',
    '9531208': 'INDRMSA',
    '9531209': 'NONRMSA',
    '9531308': 'INDRMDD',
...
# Read NOTE file (exclude Istisna per ESMR 2013-403)
print("\nReading NOTE file (Islamic, excl Istisna)...")
try:
    note_file = f'{STORE_DIR}NOTE{reptyear}{reptmon}{reptday:02d}.parquet'
    df_note = pl.read_parquet(note_file)
    
    # EXCLUDE Istisna (9532908/9532909) - ESMR 2013-403
...
# NOTE: Istisna processing section commented per ESMR 2013-403
print("\n  ℹ Istisna processing disabled (ESMR 2013-403)")

# Create DEPRMP2 (RM only - no FX)
print("\nProcessing deposits...")

if len(df_transpose) > 0:
    df_deprmp2 = df_transpose.with_columns([
    ...
    # Negate (liabilities)
    df_deprmp2 = df_deprmp2.with_columns([
...
# Create BASE.DEPOSIT with CM+Istisna remapping
print("\nCreating BASE.DEPOSIT with remapping...")
try:
    df_note2 = pl.read_parquet(note_file)
    
    # Remap CM (95317xx → 95315xx)
    ...
    # Remap Istisna (9532908/9→9531508/9)
    istisna_map = {
    ...
    df_base = df_base.filter(pl.col('DESC').is_not_null())
    
    # Transpose and finalize
    df_base_trans = df_base.group_by(['PROD', 'DESC']).agg([
        pl.col('AMOUNT').sum().alias('WEEK'),
        pl.lit(0).alias('MONTH'),
        pl.lit(0).alias('QTR'),
        pl.lit(0).alias('HALFYR'),
...
# NOTE: Istisna balance calculation commented per ESMR 2013-403
# Original code computed INDISTI and NONISTI balances for optional use
# This is now disabled

# Process each product (same as EIIMNLFE)
print("\nProcessing products...")
...
for prod in PRODUCTS:
    ...
    try:
        df_prod = df_deprmp2.filter(pl.col('DESC') == prod) if len(df_deprmp2) > 0 else pl.DataFrame([])
        ...
        try:
            ...
            # NOTE: No Istisna adjustment per ESMR 2013-403
            # Original code: if prod == 'INDRMFD': outstanding += INDISTI
...
# NOTE: Istisna behavioral calculation commented per ESMR 2013-403
# Original code computed HIGHLOWINDISTI and HIGHLOWNONISTI
...
# NOTE: No Istisna files added per ESMR 2013-403

if highlow_files:
    df_behavenote = pl.concat(highlow_files)
    
    # Add PROD codes (Islamic 933xxxx)
    prod_code_map = {
...
print(f"\n{'='*60}")
print(f"✓ EIIDNLFE Complete!")
print(f"{'='*60}")
print(f"\nKey Features (ESMR 2013-403):")
print(f"  ✓ EXCLUDES Istisna from behavioral (9532908/9532909)")
print(f"  ✓ CM remapped (9531708/9→9531508/9)")
print(f"  ✓ Istisna remapped in BASE.DEPOSIT")
print(f"  ✓ Istisna sections commented (audit trail)")
print(f"  ✓ Only 6 products (RM, no FX)")
print(f"\n6 Product Categories:")
print(f"  - INDRMDD/NONRMDD/INDRMFD/NONRMFD/INDRMSA/NONRMSA")
print(f"\nOutputs:")
print(f"  - STORE.BEHAVENOTE: 6 products (no Istisna)")
print(f"  - STORE.DEPRMP1: RM only")
print(f"  - STORE.REPORT: Final report")
print(f"  - BASE.DEPOSIT: With CM+Istisna consolidated")
