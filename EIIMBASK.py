"""
EIIMBASK - Islamic CA & NPL Monitoring (PIBB) - Production Ready

Purpose:
- Monitor Islamic banking Close Attention (CA) and Non-Performing Loans (NPL)
- Same logic as EIBMBASK but filtered for Islamic accounts (PIBB)
- Compare current month against baseline
- Identify newly surfaced delinquent Islamic accounts

Key Differences from EIBMBASK:
- COSTCTR filter: (3000-3999) OR (4043, 4048) - Islamic cost centers
- Report labels: "(PIBB)" suffix for Islamic Banking Division
- Same risk categories and logic

Islamic Cost Centers:
- 3000-3999: Islamic banking operations
- 4043, 4048: Specific Islamic units

Risk Categories:
- 0C: Close Attention (60-89 days)
- 1C: Special Mention 1 (90-182 days)
- 2S: Special Mention 2 (>182 days)
- 3D: Doubtful
- 4B: Bad

Account Ranges:
- OD: 3000000000-3999999999 (Islamic OD)
- LN: 2000000000-2999999999 (Islamic term loans)
- Islamic specific: 8000000000-8999999999

Output Reports:
1. Newly surfaced CA (PIBB) this month
2. Newly surfaced NPL (PIBB) this month
3. Newly surfaced this year (PIBB)
4. Current NPL (PIBB) listing
5. Current CA (PIBB) listing
6. NPL reduction (PIBB) report
"""
...
print("EIIMBASK - Islamic CA & NPL Monitoring (PIBB)")
print("=" * 60)
...
try:
    ...
    print(f"Islamic Banking Division (PIBB)")
...
# Create BASE datasets (same as EIBMBASK)
...
try:
    ...
    df_base = pl.concat([df_basecax, df_baseca, df_basenpl])
except:
    df_base = pl.DataFrame([])

# Process OD accounts (ISLAMIC FILTER: COSTCTR 3000-3999 OR 4043/4048)
print("\nProcessing Islamic OD accounts...")
try:
    df_loan3 = pl.read_parquet(f'{BNM_DIR}LOAN{reptmon}{nowk}.parquet').filter(
        ...
        (((pl.col('COSTCTR') >= 3000) & (pl.col('COSTCTR') <= 3999)) | 
         pl.col('COSTCTR').is_in([4043, 4048]))
              ...
              'ISSDTE', 'AMTIND', 'PRODCD', 'ORGCODE', 'ODINTACC', 'COSTCTR'])
    ...
    print(f"  ✓ Islamic OD: {len(df_od):,}")

except Exception as e:
    print(f"  ⚠ Islamic OD: {e}")
    df_od = pl.DataFrame([])

# Process LN accounts (ISLAMIC FILTER: COSTCTR 3000-3999 OR 4043/4048)
print("\nProcessing Islamic LN accounts...")
try:
    df_loans = pl.read_parquet(f'{BNM_DIR}LOAN{reptmon}{nowk}.parquet').filter(
        ...
        pl.when(pl.col('ACCTYIND') == 900)
          ...
          .alias('CORPBIND')
               ...
               'PRODCD', 'CORPBIND', 'BORSTAT', 'COSTCTR'])
    
    print(f"  ✓ Islamic LN: {len(df_loans):,}")

except Exception as e:
    print(f"  ⚠ Islamic LN: {e}")
...
# Islamic specific: AMTIND='I' → CORPBIND='IS'
df_combine = df_combine.with_columns([
    pl.when(pl.col('AMTIND') == 'I')
      .then(pl.lit('IS'))
      .otherwise(pl.col('CORPBIND'))
      .alias('CORPBIND')
])
...
print(f"\n✓ CA{reptmon}{nowk} (PIBB): {len(df_ca):,} Islamic accounts")
...
# Generate CSV reports (PIBB suffix)
print("\nGenerating Islamic (PIBB) CSV reports...")

df_new_npl = df_ca.filter(
    pl.col('RISK').is_in(['1C', '2S', '3D', '4B']) & (pl.col('NEW') == '*')
).sort('BALANCE', descending=True)

df_new_npl.write_csv(f'{OUTPUT_DIR}NEW_NPL_PIBB_{reptmon}.csv', separator=';')
print(f"  ✓ NEW_NPL_PIBB_{reptmon}.csv: {len(df_new_npl):,}")

df_new_ca = df_ca.filter(
    (pl.col('RISK') == '0C') & (pl.col('NEW') == '*')
).sort('BALANCE', descending=True)

df_new_ca.write_csv(f'{OUTPUT_DIR}NEW_CA_PIBB_{reptmon}.csv', separator=';')
print(f"  ✓ NEW_CA_PIBB_{reptmon}.csv: {len(df_new_ca):,}")

df_curr_npl = df_ca.filter(
    pl.col('RISK').is_in(['1C', '2S', '3D', '4B'])
).sort('BALANCE', descending=True)

df_curr_npl.write_csv(f'{OUTPUT_DIR}CURR_NPL_PIBB_{reptmon}.csv', separator=';')
print(f"  ✓ CURR_NPL_PIBB_{reptmon}.csv: {len(df_curr_npl):,}")

df_curr_ca = df_ca.filter(pl.col('RISK') == '0C').sort('BALANCE', descending=True)

df_curr_ca.write_csv(f'{OUTPUT_DIR}CURR_CA_PIBB_{reptmon}.csv', separator=';')
print(f"  ✓ CURR_CA_PIBB_{reptmon}.csv: {len(df_curr_ca):,}")

print(f"\n{'='*60}")
print(f"✓ EIIMBASK Complete!")
print(f"{'='*60}")
print(f"\nIslamic Banking Division (PIBB)")
print(f"Cost Centers: 3000-3999, 4043, 4048")
print(f"\nRisk Categories:")
...
print(f"  4B: Bad")
print(f"\nKey Filters:")
print(f"  - COSTCTR: (3000-3999) OR (4043, 4048)")
print(f"  - AMTIND='I' → CORPBIND='IS'")
print(f"  - Same base comparison logic as EIBMBASK")
print(f"\nOutputs:")
print(f"  - MIS.CA{reptmon}{nowk}: Islamic CA/NPL dataset")
print(f"  - MIS.GETCIS{reptmon}: Customer summary")
print(f"  - CSV reports: NEW_NPL_PIBB, NEW_CA_PIBB, CURR_NPL_PIBB, CURR_CA_PIBB")
