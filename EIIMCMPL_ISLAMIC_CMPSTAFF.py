...
if not dptr_df.is_empty() and not dpstaf_df.is_empty():
    ...
    # Save to parquet
    output_file_sc = output_path / f"islamic_staff_to_customer_transfers_{macro_vars['RPTDT']}.parquet"
    tranz_sc_df.write_parquet(output_file_sc)
    print(f"Islamic Staff to Customer transfers saved to: {output_file_sc}")
...
if not dptr_df.is_empty() and not dpcust_df.is_empty():
    ...
    # Save to parquet
    output_file_cs = output_path / f"islamic_customer_to_staff_transfers_{macro_vars['RPTDT']}.parquet"
    tranz_cs_df.write_parquet(output_file_cs)
    print(f"Islamic Customer to Staff transfers saved to: {output_file_cs}")
