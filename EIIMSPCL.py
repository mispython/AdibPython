"""
EIIMSPCL - Islamic IL Financial Management Report (PIBB) - Production Ready

Purpose:
- Generate Financial Management (FM) IL reports for PIBB (Islamic banking)
- Islamic version of EIBMSPCL (Special Mention/Impaired Loans)
- Process current and previous month Islamic IL data
...
- Measure: C_IL (count), B_IL (balance) - Impaired Loans
- TF Range: 2580000000-2589999999 (Islamic specific)
- Same processing logic as EIBMSPCL

Account Types:
- OD: Overdraft (3000000000-3999999999)
- LN: Term Loans (others)
- TF: Trade Finance (2580000000-2589999999) - Islamic

LOB Special Handling:
- PCCC: Use DRFMT mapping
- HPOP: HP Staff loans (H* + LSTAFF)
- CORP: Corporate TF (product 999)
- RETL: Retail TF (default)

Output Files:
- NPLC: Islamic IL count (C_IL measure)
- NPLA: Islamic IL amount (B_IL measure)
- NPLV: Islamic IL average (B_IL measure)

Format: FM standard
Entity: PIBB (Islamic banking)
Measure: IL (Impaired Loans)
"""
...
print("EIIMSPCL - Islamic IL Financial Management Report (PIBB)")
print("=" * 60)
...
print("\nReading previous month Islamic IL (XNPL)...")
try:
    xnpl_data = []
...
print("\nReading current month Islamic IL...")
try:
    npl_data = []
...
print(f"  ✓ Islamic IL summary: {len(df_npls):,} records")

# Generate NPLC (Count - C_IL - PIBB)
print("\nGenerating NPLC (count file - C_IL - PIBB)...")
...
with open(f'{OUTPUT_DIR}NPLC', 'w') as f:
    for row in df_npls.iter_rows(named=True):
        line = (
            f"D,PIBB{'':64}C_IL{'':196}{row['YM']}{row['NOACC']:>15}"
...
# Generate NPLA (Amount - B_IL - PIBB)
print("\nGenerating NPLA (amount file - B_IL - PIBB)...")
totamt = df_npls['AMOUNT'].sum()

with open(f'{OUTPUT_DIR}NPLA', 'w') as f:
    for row in df_npls.iter_rows(named=True):
        line = (
            f"D,PIBB{'':64}B_IL{'':196}{row['YM']}{row['AMOUNT']:>15.2f}"
            f"{row['LOB']:>4}{row['PRODUCX']:>6}EXT{'':65}ACTUAL{'':65}"
            f"{row['BRANCX']:>4},{'':66},{'':32},MYR{'':32},"
            f"{'':32},{'':32},{'':32},Y,{'':32},{'':32},{'':32},;"
        )
...
# Generate NPLV (Average - B_IL - PIBB)
print("\nGenerating NPLV (average file - B_IL - PIBB)...")
...
with open(f'{OUTPUT_DIR}NPLV', 'w') as f:
    for row in df_nplv.iter_rows(named=True):
        # AVG
        line1 = (
            f"D,PIBB{'':64}B_IL{'':196}{row['YM']}{row['AVGAMR']:>15.2f}"
...
print(f"\n{'='*60}")
print(f"✓ EIIMSPCL Complete!")
print(f"{'='*60}")
print(f"\nIslamic IL FM Report Summary:")
...
print(f"  EIBMSPCL (PBB IL)  → Conventional Special Mention")
print(f"  EIIMSPCL (PIBB IL) → Islamic Special Mention")
print(f"  EIMB1503 (PBB NPL) → Conventional NPL")
print(f"  EIIM1503 (PIBB NPL) → Islamic NPL")
