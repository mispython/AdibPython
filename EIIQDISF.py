"""
EIIQDISF - Islamic Deposit Insurance Framework (MDIC) Reporting
Complete implementation with CIS integration, EQ Master equity accounts,
branch apportionment, and insurance calculations for Islamic banking
"""
...
# =============================================================================
PATHS = {
    'DEPOSIT': 'data/deposit/', 'DEPOSITX': 'data/depositx/', 'MNI': 'data/mni/',
    'PIDMS': 'data/pidms/', 'UNCLAIM': 'data/unclaim/', 'TRUST': 'data/trust/',
    'EQ': 'data/eq/', 'OUTPUT': 'output/'
}
for p in PATHS.values(): Path(p).mkdir(exist_ok=True)

# Formats
JOIN_FMT = {0: 'ORGANISATION', 1: 'PERSONAL', **{i: f'JOIN {i}' for i in range(2, 15)}}
...
PROD_BRH = {'42110':'DDMAND','42310':'DDMAND','34180':'DDMAND','42199':'DDMAND','42120':'DSVING',
            ...
            '42610':'FDMAND','42699':'FDMAND','42630':'FFIXED','46795':'DEBIT CARD (E)','42XXX':'ATM/SI (E)'}
PROD_FMI = {'42110':'CA (A)','42310':'CA (A)','34180':'CA (A)','42120':'SA (D)','42320':'SA (D)',
            '42130':'FD','42610':'FXCA','42630':'FXFD','42132':'GID (B)','42133':'GID (B)',
            '42180':'HOUSING DEV (C)','42XXX':'ATM/SI (E)'}

# ACE products (would come from macro)
ACE_PRODUCTS = []  # Populate from actual macro

# Invalid products
INVALID_RANGES = [(107,107),(126,129),(130,136),(140,149),(171,173),
                  (350,357),(400,410),(500,560)]
...
def load_deposit(d):
    ...
    dep = pl.concat(dfs).filter(pl.col('PRODCD').cast(str).is_in(['42110','42310','42120','42320','42130',
                '42610','42133','42132','42180','42199','42699']))
    ...
    for s,e in INVALID_RANGES:
        invalid.extend(range(s, e+1))
    return dep.filter(~pl.col('PRODUCT').cast(int).is_in(invalid))\
              .with_columns(pl.when(pl.col('INTPAYBL')<0).then(0).otherwise(pl.col('INTPAYBL')).alias('INTPAYBL'))

def load_cisdep():
    dfs = []
    for i in [1,2,3,4,5,6,7,8,11,14]:
        df = load_parquet(f"{PATHS['DEPOSITX']}TRANS{i:02d}.parquet")
        if not df.is_empty(): dfs.append(df)
    for f in ['TRANORG', 'TRAN999']:
        df = load_parquet(f"{PATHS['DEPOSITX']}{f}.parquet")
        if not df.is_empty(): dfs.append(df)
...
def load_eq_master():
    """Load EQ Master accounts with INSURABILITY=1"""
    eq = load_parquet(f"{PATHS['EQ']}MASTER.parquet")
    if eq.is_empty(): return pl.DataFrame()
    return eq.filter(pl.col('INSURABILITY') == 1).with_columns([
        pl.col('REGNUM').alias('CUSTID'),
        pl.lit('I').alias('AMTIND'),
        pl.lit(0).alias('JOIN'),
        pl.lit('IR070').alias('PRODCD'),
        pl.col('AVAILABLEBAL').alias('CURBAL'),
        pl.col('ACCRUEDINT').alias('INTPAYBL'),
        (pl.col('AVAILABLEBAL') + pl.col('ACCRUEDINT')).alias('CURBALTT')
    ])
...
# =============================================================================
def main():
    print("="*60+"\nEIIQDISF - Islamic Deposit Insurance Framework\n"+"="*60)
    d = get_dates()
    ...
    # BC/DD adjustments
    dep = dep.with_columns(
        pl.when((pl.col('PRODUCT').cast(int).is_in(ACE_PRODUCTS)) & (pl.col('PRODCD')=='42110'))
    ...
    # Load CIS and EQ Master
    print("\nLoading CIS and EQ...")
    cisdep, tc, eq = load_cisdep(), load_trust_client(), load_eq_master()
    print(f"  CISDEP:{len(cisdep)} TRUST/CLIENT:{len(tc)} EQ:{len(eq)}")
    ...
    if not cisdep.is_empty() and not dep_final.is_empty():
        ...
        # Extract CUSTID from KEY
        merged = merged.with_columns(pl.col('KEY').str.slice(2,17).alias('CUSTID'))
        
        # Merge with EQ Master
        if not eq.is_empty():
            eq = eq.sort('CUSTID')
            merged_cust = merged.unique('CUSTID').sort('CUSTID')
            eq_merged = eq.join(merged_cust.select(['CUSTID','KEY']), on='CUSTID', how='left')
            eq_merged = eq_merged.with_columns(
                pl.when(pl.col('KEY').is_not_null()).then(pl.col('KEY')).otherwise(pl.col('CUSTID')).alias('KEY')
            )
            
            # Combine EQ and CIS deposits
            all_data = pl.concat([
                merged.with_columns(pl.lit('D').alias('AMTIND_ORIG')),
                eq_merged.select(merged.columns)
            ]).unique(subset=['KEY','AMTIND'])
            
            # Group by KEY and AMTIND
            summary = all_data.group_by(['KEY', 'AMTIND']).agg([
            ...
            for row in summary.sort('KEY').rows(named=True):
                ...
                result.append({
                    ...
                    'INSURED': insured if key != last_key else None,
                    'EXCESS': excess if key != last_key else None,
                    'MORESIX': moresix if key != last_key else None,
                    'LESSSIX': lesssix if key != last_key else None,
                    'INSUREBR': inshare if key == last_key else None,
                    'INSURSUM': (insured if key != last_key else 0) * 0.0006,
                    'TOTALSUM': tot * 0.0002
                })
                last_key = key
            
            ins_df = pl.DataFrame(result).filter(pl.col('AMTIND') == 'I')
            
            # Final reports
            print("\n"+"="*60+"\nDEPOSIT INSURANCE REPORT - ISLAMIC\n"+"="*60)
            ...
            if 'JOIN' in merged.columns and 'PRODCD' in merged.columns:
                tab1 = merged.filter(pl.col('AMTIND')=='I').group_by(['JOIN', 'PRODCD']).agg([
            ...
            if 'BRANCH' in merged.columns:
                branch = merged.filter(pl.col('AMTIND')=='I').group_by('BRANCH').agg([
                    pl.col('CURBALTT').sum().alias('TOTBR'),
                    (pl.col('CURBALTT') / pl.col('CURBALTT').sum() * 250000).sum().alias('INSUREBR')
                ])
                print("\nBranch apportionment (Islamic):")
                for r in branch.sort('BRANCH').rows(named=True):
                    print(f"  Branch {r['BRANCH']}: Insured {r['INSUREBR']:,.2f}")
            
            ins_df.write_parquet(f"{PATHS['OUTPUT']}CISDEPI.parquet")
            print(f"\nSaved: CISDEPI.parquet ({len(ins_df)} records)")
