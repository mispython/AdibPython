...
# DATA IDEPOSIT.REMIT(DROP=ISSYY ISSMM ISSDD) NONDEBIT;
# INFILE REMIT;
try:
    ...
    ).drop(['ISSYY', 'ISSMM', 'ISSDD'])  # DROP=ISSYY ISSMM ISSDD
    ...
    # Save datasets
    remit_valid.write_parquet(ideposit_path / "REMIT.parquet")
    nondebit_invalid.write_parquet(output_path / "NONDEBIT.parquet")
    
    print("Created IDEPOSIT.REMIT and NONDEBIT datasets")
...
if not remit_valid.is_empty():
    ...
    remit_sorted.write_parquet(ideposit_path / "REMIT.parquet")
...
# PROC SUMMARY DATA=IDEPOSIT.REMIT; BY PAYMODE; VAR LEDGBAL;
if not remit_sorted.is_empty():
    ...
    # DATA REMIT; MERGE REMIT(IN=A) IDEPOSIT.REMIT (IN=B DROP=LEDGBAL);
...
if not remit_final.is_empty():
    ...
    print("\n" + "="*80)
    print("BANKERS CHEQUE WITH DEBITTED A/C (ISLAMIC)")
    print("="*80)
    ...
    # PROC SUMMARY DATA=DEP NWAY MISSING;
    # WHERE BC = 'NOT_FOUND' AND complex conditions;
    ...
    if not notfound_filtered.is_empty():
        ...
        # PROC PRINT DATA=XXX LABEL; LABEL LEDGBAL = 'BC/DD AMOUNT'; SUM LEDGBAL;
        print("BC/DD AMOUNT by Category (Not Found in FISS):")
        print(notfound_summary)
        total = notfound_summary.select(pl.col('LEDGBAL').sum()).row(0)[0]
        print(f"TOTAL BC/DD AMOUNT: {total:.2f}")
    else:
        print("No not-found records matching the criteria")
