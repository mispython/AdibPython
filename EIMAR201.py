...
for row in pl.read_parquet(BNM_LOANTEMP).filter(pl.col('CHECKDT') == 1).iter_rows(named=True):
    if row['PRODUCT'] in [380, 381, 700, 705]:
        records.append({**row, 'CAT': 'A', 'TYPE': '(HPD-C)'})
    
    if row['PRODUCT'] in [380, 381]:
        records.append({**row, 'CAT': 'B', 'TYPE': '(HP 380/381)'})
    
    if row['PRODUCT'] in [128, 130]:
        records.append({**row, 'CAT': 'C', 'TYPE': '(AITAB)'})
    
    if row['PRODUCT'] in [128, 130, 380, 381, 700, 705]:
...
for row in df_loantemp.iter_rows(named=True):
    if current_cat != row['CAT']:
        ...
        lines.append(f"PROGRAM-ID : EIMAR201                  P U B L I C   B A N K   B E R H A D                                             PAGE NO.: {page_count}")
        lines.append(f"                               OUTSTANDING LOANS IN ARREARS ISSUED FROM 01 JAN 1998 {row['TYPE']:13} {rdate}")
        lines.append(' ')
