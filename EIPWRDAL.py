"""
EIPWRDAL - RDAL Report Generation for Bank Negara Malaysia
Includes: PBBLNFMT, PBBMRDLF format definitions

Note: This is essentially the same as EIWPRDAL
"""
...
from pbblnfmt import LNPROD, LNDENOM
from pbbelf import LIBRARY_PATHS
...
print(f"Processing RDAL Report Generation for BNM")
print(f"Report Date: {reptdate.strftime('%d/%m/%Y')}")
print(f"Week: {nowk}, Previous Week: {nowk1}")
print(f"Start Date: {sdate.strftime('%d/%m/%Y')}")
print(f"Institution: {sdesc}")

# Step 1: Load PBBMRDLF data (monthly RDAL BICs - from %INC PGM(PBBMRDLF))
# This contains additional BIC codes with zero amounts
...
# Step 5: Process Cagamas loans (specific PZIPCODE values)
...
# Apply LNPROD and LNDENOM formats from PBBLNFMT
cag_records = []
for row in df_cag.iter_rows(named=True):
    loantype = row['LOANTYPE']
    prodcd = LNPROD.get(loantype, f'{loantype:05d}')
    amtind = LNDENOM.get(loantype, 'D')
...
df_cag = pl.DataFrame(cag_records)

# Aggregate Cagamas data
df_cag_agg = df_cag.group_by(['ITCODE', 'AMTIND']).agg([
    pl.col('BALANCE').sum().alias('AMOUNT')
])

# Step 6: Combine RDAL and Cagamas data
df_rdal = pl.concat([df_rdal, df_cag_agg])

# Step 7: Remove specific item and handle special case
df_rdal = df_rdal.filter(
...
# Step 8: Filter out F and # codes
df_rdal_filtered = df_rdal.filter(
    ~pl.col('ITCODE').str.slice(13, 1).is_in(['F', '#'])
)

# Step 9: Classify records as AL, OB, or SP
...
# Step 10: Write RDAL output file
...
# Step 11: Process hash (#) codes and reaggregate
...
# Step 12: Write NSRS output file
...
print(f"\nFormats used from PBBLNFMT:")
print(f"  LNPROD - Loan product codes")
print(f"  LNDENOM - Denomination indicators (D/I/F)")
